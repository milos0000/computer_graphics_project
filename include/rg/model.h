...
class Model
{
	...
	Mesh processMesh(aiMesh *mesh, const aiScene *scene)
	{
		...
		loadTextureMaterial(material, aiTextureType_DIFFUSE,
				    "texture_diffuse", textures);

		loadTextureMaterial(material, aiTextureType_SPECULAR,
				    "texture_specular", textures);

		loadTextureMaterial(material, aiTextureType_NORMALS,
				    "texture_normal", textures);

		loadTextureMaterial(material, aiTextureType_HEIGHT,
				    "texture_height", textures);

		return Mesh(vertices, indices, textures);
	}

	void loadTextureMaterial(aiMaterial *mat, aiTextureType type,
				 std::string typeName,
				 std::vector<Texture> &textures)
	{
		...
		for (unsigned int i = 0; i < mat->GetTextureCount(type); ++i) {
			...
			for (unsigned int j = 0; j < loaded_textures.size();
			     ++j) {
				if (std::strcmp(
					str.C_Str(),
					loaded_textures[j].path.c_str()) == 0) {
...
unsigned int TextureFromFile(const char *filename, std::string directory)
{
	...
	if (data) {
		...
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAX_FILTER,
				GL_LINEAR);
