...
#define LOG(stream)                                                            \
	stream << "[" << __FILE__ << ", " << __func__ << ", " << __LINE__      \
	       << "] "
...
#define GLCALL(x)                                                              \
	do {                                                                   \
		...
		x;                                                             \
		BREAK_IF_FALSE(rg::wasPreviousOpenGLCallSuccessful(            \
		    __FILE__, __LINE__, #x));                                  \
	} while (0)
...
bool wasPreviousOpenGLCallSuccessful(const char *file, int line,
				     const char *call);
