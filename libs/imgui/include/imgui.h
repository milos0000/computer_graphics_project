...
// ImGuiStyle
// ImGuiIO
// Misc data structures (ImGuiInputTextCallbackData, ImGuiSizeCallbackData,
ImGuiPayload)
// Obsolete functions
// Helpers (ImGuiOnceUponAFrame, ImGuiTextFilter, ImGuiTextBuffer, ImGuiStorage,
ImGuiListClipper, ImColor)
// Draw List API (ImDrawCallback, ImDrawCmd, ImDrawIdx, ImDrawVert,
ImDrawChannel, ImDrawListSplitter, ImDrawListFlags, ImDrawList, ImDrawData)
// Font API (ImFontConfig, ImFontGlyph, ImFontGlyphRangesBuilder,
ImFontAtlasFlags, ImFontAtlas, ImFont)

// FIXME-TABLE: Add ImGuiTableSortSpecs and ImGuiTableColumnSortSpecs in "Misc
data structures" section above (we don't do it right now to facilitate merging
various branches)
...
// Configuration file with compile-time options (edit imconfig.h or #define
// IMGUI_USER_CONFIG to your own filename)
...
// progress versions typically starts at XYY99 then bounce up to XYY00, XYY01
// etc. when release tagging happens)
...
// shared library is not recommended, because we don't guarantee backward nor
// forward ABI compatibility (also function call overhead, as dear imgui is a
// call-heavy API)
#ifndef IMGUI_API
#define IMGUI_API
#endif
...
#define IM_ARRAYSIZE(_ARR)                                                     \
	((int)(sizeof(_ARR) /                                                  \
	       sizeof(*(_ARR)))) // Size of a static C-style array. Don't use on
				 // pointers!
#define IM_UNUSED(_VAR)                                                        \
	((void)(_VAR)) // Used to silence "unused variable warnings". Often
		       // useful as asserts may be stripped out from final
		       // builds.
#if (__cplusplus >= 201100)
#define IM_OFFSETOF(_TYPE, _MEMBER)                                            \
	offsetof(_TYPE, _MEMBER) // Offset of _MEMBER within _TYPE. Standardized
				 // as offsetof() in C++11
#else
#define IM_OFFSETOF(_TYPE, _MEMBER)                                            \
	((size_t) &                                                            \
...
#define IM_FMTARGS(FMT)                                                        \
	__attribute__((                                                        \
	    format(printf, FMT, FMT + 1))) // Apply printf-style warnings to our
...
#define IM_FMTARGS(FMT)                                                        \
	__attribute__((                                                        \
	    format(gnu_printf, FMT, FMT + 1))) // Apply printf-style warnings to
...
// Enums/Flags (declared as int for compatibility with old C++, to allow using
// as flags and to not pollute the top of this file)
...
typedef int ImDrawCornerFlags;	// -> enum ImDrawCornerFlags_    // Flags: for
				// ImDrawList::AddRect(), AddRectFilled() etc.
typedef int
...
typedef int ImGuiKeyModFlags;	 // -> enum ImGuiKeyModFlags_     // Flags: for
				 // io.KeyMods (Ctrl/Shift/Alt/Super)
typedef int
...
// Other types
		    ...
		    // with '#define ImTextureID xxx']
typedef void *ImTextureID; // User data for rendering backend to identify a
			   // texture. This is whatever to you want it to be!
			   // read the FAQ about ImTextureID for details.
#endif
typedef unsigned int ImGuiID; // A unique ID used by widgets, typically hashed
...
// (This is a namespace. You can add extra ImGui:: functions in your own
// separate file. Please don't modify imgui source files!)
//-----------------------------------------------------------------------------
...
// contexts. None of those functions is reliant on the current context.
...
IMGUI_API void ShowDemoWindow(
    bool *p_open =
	NULL); // create Demo window (previously called ShowTestWindow).
...
GetVersion(); // get the compiled version string e.g. "1.23" (essentially the
...
// Prefer using SetNextXXX functions (before Begin) rather that SetXXX functions
// (after Begin).
...
IMGUI_API void SetWindowFontScale(
    float scale); // set font scale. Adjust IO.FontGlobalScale if you want to
		  // scale all windows. This is an old API! For correct scaling,
		  // prefer to reload font + rebuild ImFontAtlas + call
		  // style.ScaleAllSizes().
...
// Content region
// - Those functions are bound to be redesigned soon (they are confusing,
// incomplete and return values in local window coordinates which increases
// confusion)
IMGUI_API ImVec2
...
IMGUI_API void PushAllowKeyboardFocus(
    bool allow_keyboard_focus); // allow focusing using TAB/Shift-TAB, enabled
				// by default but you can disable it for certain
				// widgets
...
IMGUI_API ImU32
GetColorU32(const ImVec4 &col); // retrieve given color with style alpha applied
IMGUI_API ImU32
...
IMGUI_API void PushItemWidth(
    float item_width); // push width of items for common large "item+label"
		       ...
		       // to the right side). 0.0f = default to ~2/3 of windows
		       // width,
...
//    PushTextWrapPos() Absolute coordinate:        GetCursorScreenPos(),
...
IMGUI_API void NewLine(); // undo a SameLine() or force a new line when in an
...
IMGUI_API ImVec2 GetCursorPos(); // cursor position in window coordinates
				 // (relative to window position)
IMGUI_API float
GetCursorPosX(); //   (some functions are using window-relative coordinates,
		 //   such as: GetCursorPos, GetCursorStartPos,
		 //   GetContentRegionMax, GetWindowContentRegion* etc.
IMGUI_API float
GetCursorPosY(); //    other functions such as GetCursorScreenPos or everything
		 //    in ImDrawList::
IMGUI_API void SetCursorPos(
    const ImVec2
	&local_pos); //    are using the main, absolute coordinate system.
IMGUI_API void
SetCursorPosX(float local_x); //    GetWindowPos() + GetCursorPos() ==
...
IMGUI_API void
SetCursorScreenPos(const ImVec2 &pos); // cursor position in absolute screen
				       // coordinates [0..io.DisplaySize]
IMGUI_API void
...
// ID stack/scopes
// - Read the FAQ for more details about how ID are handled in dear imgui. If
// you are creating widgets in a loop you most
//   likely want to push a unique identifier (e.g. object pointer, loop index)
...
// string that will be displayed and used as an ID,
...
// functions, note that a 'float v[X]' function argument is the same as 'float*
// v', the array syntax is just a way to document the number of elements that
// are expected to be accessible. You can pass address of your first element out
...
// manual input can override those limits.
...
IMGUI_API bool DragScalar(const char *label, ImGuiDataType data_type,
			  void *p_data, float v_speed, const void *p_min = NULL,
...
// - Format string may also be set to NULL or use the default format ("%f" or
// "%d").
// - Legacy: Pre-1.78 there are SliderXXX() function signatures that takes a
...
IMGUI_API bool
CollapsingHeader(const char *label, bool *p_open,
		 ImGuiTreeNodeFlags flags =
		     0); // when 'p_open' isn't NULL, display an additional
...
// Widgets: List Boxes
// - FIXME: To be consistent with all the newer API, ListBoxHeader/ListBoxFooter
// should in reality be called BeginListBox/EndListBox. Will rename them.
...
IMGUI_API bool ListBoxHeader(const char *label, int items_count,
			     int height_in_items = -1); // "
IMGUI_API void
ListBoxFooter(); // terminate the scrolling region. only call ListBoxFooter() if
...
IMGUI_API bool
MenuItem(const char *label, const char *shortcut = NULL, bool selected = false,
	 bool enabled =
...
IMGUI_API void OpenPopupOnItemClick(
    ...
    ImGuiPopupFlags popup_flags =
	1); // helper to open popup when clicked on last item. return true when
	    // just opened. (note: actually triggers on the mouse _released_
...
//  - IMPORTANT: we exceptionally default their flags to 1 (==
...
IMGUI_API bool BeginPopupContextItem(
    const char *str_id = NULL,
    ImGuiPopupFlags popup_flags =
	1); // open+begin popup when clicked on last item. if you can pass a
	    // NULL str_id only if the previous item had an id. If you want to
	    // use that on a non-interactive item such as Text() you need to
...
// columns/rows
// - 4. Optionally call TableHeadersRow() to submit a header row (names will be
// pulled from data submitted to TableSetupColumns)
// - 5. Populate contents
//    - In most situations you can use TableNextRow() + TableSetColumnIndex(N)
//    to start appending into a column.
//    - If you are using tables as a sort of grid, where every columns is
//    holding the same type of contents,
...
//      columns but one (e.g. the first one).
...
IMGUI_API bool BeginTable(const char *str_id, int columns_count,
...
// - Use TableHeadersRow() to create a row and automatically submit a
// TableHeader() for each column.
//   Headers are required to perform: reordering, sorting, and opening the
//   context menu (but context menu can also be available in columns body using
//   ImGuiTableFlags_ContextMenuInBody).
// - You may manually submit headers using TableNextRow() + TableHeader() calls,
// but this is only useful in some advanced cases (e.g. adding custom widgets in
// header row).
// - Use TableSetupScrollFreeze() to lock columns (from the right) or rows (from
// the top) so they stay visible when scrolled.
...
IMGUI_API void
TableHeadersRow(); // submit all headers cells based on data provided to
		   // TableSetupColumn() + submit context menu
IMGUI_API void
TableHeader(const char *label); // submit one header cell manually (rarely used)
// Tables: Miscellaneous functions
// - Most functions taking 'int column_n' treat the default value of -1 as the
// same as passing the current column index
// - Sorting: call TableGetSortSpecs() to retrieve latest sort specs for the
// table. Return value will be NULL if no sorting.
//   When 'SpecsDirty == true' you should sort your data. It will be true when
//   sorting specs have changed since last call, or the first time. Make sure to
//   set 'SpecsDirty = false' after sorting, else you may wastefully sort your
//   data every frame! Lifetime: don't hold on this pointer over multiple frames
//   or past any subsequent call to BeginTable().
IMGUI_API int
...
IMGUI_API ImGuiTableColumnFlags TableGetColumnFlags(
    int column_n = -1); // return column flags so you can query their
			// Enabled/Visible/Sorted/Hovered status flags.
...
IMGUI_API void TableSetBgColor(
    ImGuiTableBgTarget bg_target, ImU32 color,
...
GetDragDropPayload(); // peek directly into the current payload from anywhere.
		      // may return NULL. use ImGuiPayload::IsDataType() to test
...
IMGUI_API void
SetItemAllowOverlap(); // allow last item to be overlapped by a subsequent item.
...
IMGUI_API ImDrawList *
GetBackgroundDrawList(); // this draw list will be the first rendering one.
...
// - For 'int user_key_index' you can use your own indices/enums according to
// how your backend/engine stored them in io.KeysDown[].
// - We don't know the meaning of those value. You can use GetKeyIndex() to map
// a ImGuiKey_ value into the user index.
IMGUI_API int
GetKeyIndex(ImGuiKey imgui_key); // map ImGuiKey_* values into user's key index.
				 // == io.KeyMap[key]
...
IMGUI_API bool IsMouseDoubleClicked(
    ImGuiMouseButton
	button); // did mouse button double-clicked? (note that a double-click
		 // will also report IsMouseClicked() == true)
...
IMGUI_API void
SetMouseCursor(ImGuiMouseCursor cursor_type); // set desired cursor type
IMGUI_API void CaptureMouseFromApp(
    bool want_capture_mouse_value =
	true); // attention: misleading name! manually override
	       // io.WantCaptureMouse flag next frame (said flag is entirely
	       // left for your application to handle). This is equivalent to
	       // setting "io.WantCaptureMouse = want_capture_mouse_value;"
...
// SetCurrentContext() + SetAllocatorFunctions() again because we use global
// storage for those.
IMGUI_API void
SetAllocatorFunctions(void *(*alloc_func)(size_t sz, void *user_data),
...
enum ImGuiWindowFlags_ {
	...
	ImGuiWindowFlags_NoScrollbar =
	    1 << 3, // Disable scrollbars (window can still scroll with mouse or
		    // programmatically)
	ImGuiWindowFlags_NoScrollWithMouse =
	    1 << 4, // Disable user vertically scrolling with mouse wheel. On
	...
	ImGuiWindowFlags_NoBackground =
	    1 << 7, // Disable drawing background color (WindowBg, etc.) and
		    // outside border. Similar as using
	...
	ImGuiWindowFlags_HorizontalScrollbar =
	    1
	    << 11, // Allow horizontal scrollbar to appear (off by default). You
	...
	ImGuiWindowFlags_AlwaysVerticalScrollbar =
	    1 << 14, // Always show vertical scrollbar (even if ContentSize.y <
		     // Size.y)
	...
	ImGuiWindowFlags_UnsavedDocument =
	    1 << 20, // Append '*' to title without affecting the ID, as a
		     // convenience to avoid using the ### operator. When used
		     // in a tab/docking context, tab is selected on closure and
		     // closure is deferred by one frame to allow code to cancel
		     // the closure (with a confirmation popup, etc.) without
		     // flicker.
	...
	ImGuiWindowFlags_NavFlattened =
	    1 << 23, // [BETA] Allow gamepad/keyboard navigation to cross over
		     // parent border to this child (only use on child that have
		     // no scrolling!)
	...
	// [Obsolete]
	// ImGuiWindowFlags_ResizeFromAnySide    = 1 << 17,  // --> Set
	// io.ConfigWindowsResizeFromEdges=true and make sure mouse cursors are
	// supported by backend (io.BackendFlags &
...
enum ImGuiInputTextFlags_ {
	...
	ImGuiInputTextFlags_EnterReturnsTrue =
	    1 << 5, // Return 'true' when Enter is pressed (as opposed to every
	...
	ImGuiInputTextFlags_CallbackCharFilter =
	    1 << 9, // Callback on character inputs to replace or discard them.
	...
	ImGuiInputTextFlags_CtrlEnterForNewLine =
	    1 << 11, // In multi-line mode, unfocus with Enter, add new line
		     // with Ctrl+Enter (default is opposite: unfocus with
	...
	ImGuiInputTextFlags_NoUndoRedo =
	    1 << 16, // Disable undo/redo. Note that input text owns the text
		     // data while active, if you want to provide your own
		     // undo/redo stack you need e.g. to call ClearActiveID().
	ImGuiInputTextFlags_CharsScientific =
	    1 << 17, // Allow 0123456789.+-*/eE (Scientific notation input)
	ImGuiInputTextFlags_CallbackResize =
	    1 << 18, // Callback on buffer capacity changes request (beyond
		     // 'buf_size' parameter value), allowing the string to
		     // grow. Notify when the string wants to be resized (for
		     // string types which hold a cache of their Size). You will
		     // be provided a new BufSize in the callback and NEED to
		     // honor it. (see misc/cpp/imgui_stdlib.h for an example of
		     // using this)
	ImGuiInputTextFlags_CallbackEdit =
	    1 << 19, // Callback on any edit (note that InputText() already
	...
	ImGuiInputTextFlags_NoMarkEdited =
	    1 << 21 // For internal use by functions using InputText() before
		    // reformatting data
};
...
enum ImGuiTreeNodeFlags_ {
	...
	ImGuiTreeNodeFlags_AllowItemOverlap =
	    1
	    << 2, // Hit testing to allow subsequent widgets to overlap this one
	ImGuiTreeNodeFlags_NoTreePushOnOpen =
	    1
	    << 3, // Don't do a TreePush() when open (e.g. for CollapsingHeader)
		  // = no extra indent nor pushing on ID stack
	ImGuiTreeNodeFlags_NoAutoOpenOnLog =
	    1 << 4, // Don't automatically and temporarily open node when
	...
	ImGuiTreeNodeFlags_Leaf = 1 << 8,   // No collapsing, no arrow (use as a
					    // convenience for leaf nodes).
	...
	ImGuiTreeNodeFlags_SpanAvailWidth =
	    1 << 11, // Extend hit box to the right-most edge, even if not
		     // framed. This is not the default in order to allow adding
		     // other items on the same line. In the future we may
		     // refactor the hit system to be front-to-back, allowing
		     // natural overlaps and then this can become the default.
	ImGuiTreeNodeFlags_SpanFullWidth =
	    1 << 12, // Extend hit box to the left-most and right-most edges
		     // (bypass the indented area).
	ImGuiTreeNodeFlags_NavLeftJumpsBackHere =
	    1 << 13, // (WIP) Nav: left direction may move to this TreeNode()
		     // from any of its child (items submitted between TreeNode
		     // and TreePop)
	// ImGuiTreeNodeFlags_NoScrollOnOpen     = 1 << 14,  // FIXME: TODO:
	// Disable automatic scroll on TreePop() if node got just open and
...
//   and want to another another flag, you need to pass in the
//   ImGuiPopupFlags_MouseButtonRight flag.
// - Multiple buttons currently cannot be combined/or-ed in those functions (we
// could allow it later).
enum ImGuiPopupFlags_ {
	ImGuiPopupFlags_None = 0,
	ImGuiPopupFlags_MouseButtonLeft =
	    0, // For BeginPopupContext*(): open on Left Mouse release.
	       // Guaranteed to always be == 0 (same as ImGuiMouseButton_Left)
	ImGuiPopupFlags_MouseButtonRight =
	    1, // For BeginPopupContext*(): open on Right Mouse release.
	       // Guaranteed to always be == 1 (same as ImGuiMouseButton_Right)
	ImGuiPopupFlags_MouseButtonMiddle =
	    2, // For BeginPopupContext*(): open on Middle Mouse release.
	       // Guaranteed to always be == 2 (same as ImGuiMouseButton_Middle)
	ImGuiPopupFlags_MouseButtonMask_ = 0x1F,
	ImGuiPopupFlags_MouseButtonDefault_ = 1,
	ImGuiPopupFlags_NoOpenOverExistingPopup =
	    1 << 5, // For OpenPopup*(), BeginPopupContext*(): don't open if
		    // there's already a popup at the same level of the popup
		    // stack
	ImGuiPopupFlags_NoOpenOverItems =
	    1 << 6, // For BeginPopupContextWindow(): don't return true when
		    // hovering items, only when hovering empty space
	ImGuiPopupFlags_AnyPopupId =
	    1 << 7, // For IsPopupOpen(): ignore the ImGuiID parameter and test
		    // for any popup.
	ImGuiPopupFlags_AnyPopupLevel =
	    1 << 8, // For IsPopupOpen(): search/test at any level of the popup
...
enum ImGuiSelectableFlags_ {
	...
	ImGuiSelectableFlags_SpanAllColumns =
	    1 << 1, // Selectable frame can span all columns (text will still
	...
	ImGuiSelectableFlags_AllowItemOverlap =
	    1 << 4 // (WIP) Hit testing to allow subsequent widgets to overlap
		   // this one
};
...
enum ImGuiComboFlags_ {
	...
	ImGuiComboFlags_HeightSmall =
	    1 << 1, // Max ~4 items visible. Tip: If you want your combo popup
		    // to be a specific size you can use
		    // SetNextWindowSizeConstraints() prior to calling
		    // BeginCombo()
	ImGuiComboFlags_HeightRegular = 1
					<< 2, // Max ~8 items visible (default)
	ImGuiComboFlags_HeightLarge = 1 << 3, // Max ~20 items visible
	ImGuiComboFlags_HeightLargest =
	    1 << 4, // As many fitting items as possible
	ImGuiComboFlags_NoArrowButton =
	    1
	    << 5, // Display on the preview box without the square arrow button
	ImGuiComboFlags_NoPreview = 1
				    << 6, // Display only a square arrow button
...
enum ImGuiTabBarFlags_ {
	...
	ImGuiTabBarFlags_Reorderable =
	    1 << 0, // Allow manually dragging tabs to re-order them + New tabs
...
enum ImGuiTabItemFlags_ {
	...
	ImGuiTabItemFlags_UnsavedDocument =
	    1 << 0, // Append '*' to title without affecting the ID, as a
	...
	ImGuiTabItemFlags_NoPushId = 1
				     << 3, // Don't call PushID(tab->ID)/PopID()
					   // on BeginTabItem()/EndTabItem()
	ImGuiTabItemFlags_NoTooltip = 1
				      << 4, // Disable tooltip for the given tab
	ImGuiTabItemFlags_NoReorder =
	    1 << 5, // Disable reordering this tab or having another tab cross
		    // over this tab
	ImGuiTabItemFlags_Leading =
	    1 << 6, // Enforce the tab position to the left of the tab bar
		    // (after the tab list popup button)
	ImGuiTabItemFlags_Trailing =
	    1 << 7 // Enforce the tab position to the right of the tab bar
		   // (before the scrolling buttons)
};

// Flags for ImGui::BeginTable()
// - Important! Sizing policies have particularly complex and subtle side
// effects, more so than you would expect.
//   Read comments/demos carefully + experiment with live demos to get
//   acquainted with them.
// - The default sizing policy for columns depends on whether the ScrollX flag
// is set on the table:
//   When ScrollX is off:
//    - Table defaults to ImGuiTableFlags_ColumnsWidthStretch -> all Columns
//    defaults to ImGuiTableColumnFlags_WidthStretch.
//    - Columns sizing policy allowed: Stretch (default) or Fixed/Auto.
//    - Stretch Columns will share the width available in table.
//    - Fixed Columns will generally obtain their requested width unless the
//    Table cannot fit them all.
//   When ScrollX is on:
//    - Table defaults to ImGuiTableFlags_ColumnsWidthFixed -> all Columns
//    defaults to ImGuiTableColumnFlags_WidthFixed.
//    - Columns sizing policy allowed: Fixed/Auto mostly!
//    - Fixed Columns can be enlarged as needed. Table will show an horizontal
//    scrollbar if needed.
...
// - Mixing up columns with different sizing policy is possible BUT can be
// tricky and has some side-effects and restrictions.
//   (their visible order and the scrolling state have subtle but necessary
//   effects on how they can be manually resized). The typical use of mixing
//   sizing policies is to have ScrollX disabled, one or two Stretch Column and
//   many Fixed Columns.
enum ImGuiTableFlags_ {
	...
	ImGuiTableFlags_Reorderable =
	    1
	    << 1, // Allow reordering columns in header row (need calling
		  // TableSetupColumn() + TableHeadersRow() to display headers)
	ImGuiTableFlags_Hideable =
	    1 << 2, // Allow hiding/disabling columns in context menu.
	ImGuiTableFlags_Sortable =
	    1 << 3, // Allow sorting on one column (sort_specs_count will always
		    // be == 1). Call TableGetSortSpecs() to obtain sort specs.
	ImGuiTableFlags_MultiSortable =
	    1 << 4, // Allow sorting on multiple columns by holding Shift
		    // (sort_specs_count may be > 1). Call TableGetSortSpecs()
		    // to obtain sort specs.
	ImGuiTableFlags_NoSavedSettings =
	    1 << 5, // Disable persisting columns order, width and sort settings
		    // in the .ini file.
	ImGuiTableFlags_ContextMenuInBody =
	    1 << 6, // Right-click on columns body/contents will display table
		    // context menu. By default it is available in
		    // TableHeadersRow(). Decorations
	ImGuiTableFlags_RowBg =
	    1 << 7, // Set each RowBg color with ImGuiCol_TableRowBg or
		    // ImGuiCol_TableRowBgAlt (equivalent of calling
		    // TableSetBgColor with ImGuiTableBgFlags_RowBg0 on each row
		    // manually)
	ImGuiTableFlags_BordersInnerH =
	    1 << 8, // Draw horizontal borders between rows.
	ImGuiTableFlags_BordersOuterH =
	    1 << 9, // Draw horizontal borders at the top and bottom.
	ImGuiTableFlags_BordersInnerV =
	    1 << 10, // Draw vertical borders between columns.
	ImGuiTableFlags_BordersOuterV =
	    1 << 11, // Draw vertical borders on the left and right sides.
	...
	ImGuiTableFlags_NoBordersInBody =
	    1 << 12, // Disable vertical borders in columns Body (borders will
		     // always appears in Headers).
	ImGuiTableFlags_NoBordersInBodyUntilResize =
	    1
	    << 13, // Disable vertical borders in columns Body until hovered for
		   // resize (borders will always appears in Headers). Sizing
	ImGuiTableFlags_ColumnsWidthStretch =
	    1 << 14, // Default if ScrollX is off. Columns will default to use
		     // _WidthStretch. Read description above for more details.
	ImGuiTableFlags_ColumnsWidthFixed =
	    1 << 15, // Default if ScrollX is on. Columns will default to use
		     // _WidthFixed or _WidthAutoResize policy (if Resizable is
		     // off). Read description above for more details.
	ImGuiTableFlags_SameWidths =
	    1 << 16, // Make all columns the same widths which is useful with
		     // Fixed columns policy (but granted by default with
		     // Stretch policy + no resize). Implicitly enable
		     // ImGuiTableFlags_NoKeepColumnsVisible and disable
		     // ImGuiTableFlags_Resizable.
	ImGuiTableFlags_NoHeadersWidth =
	    1 << 17, // Disable headers' contribution to automatic width
		     // calculation.
	ImGuiTableFlags_NoHostExtendY =
	    1 << 18, // Disable extending past the limit set by outer_size.y,
		     // only meaningful when neither of ScrollX|ScrollY are set
		     // (data below the limit will be clipped and not visible)
	ImGuiTableFlags_NoKeepColumnsVisible =
	    1 << 19, // Disable keeping column always minimally visible when
		     // ScrollX is off and table gets too small.
	ImGuiTableFlags_PreciseWidths =
	    1 << 20, // Disable distributing remainder width to stretched
		     // columns (width allocation on a 100-wide table with 3
		     // columns: Without this flag: 33,33,34. With this flag:
		     // 33,33,33). With larger number of columns, resizing will
		     // appear to be less smooth.
	ImGuiTableFlags_NoClip =
	    1 << 21, // Disable clipping rectangle for every individual columns
		     // (reduce draw command count, items will be able to
		     // overflow into other columns). Generally incompatible
		     // with TableSetupScrollFreeze(). Padding
	ImGuiTableFlags_PadOuterX =
	    1
	    << 22, // Default if BordersOuterV is on. Enable outer-most padding.
	ImGuiTableFlags_NoPadOuterX = 1
				      << 23, // Default if BordersOuterV is off.
					     // Disable outer-most padding.
	ImGuiTableFlags_NoPadInnerX =
	    1 << 24, // Disable inner padding between columns (double inner
		     // padding if BordersOuterV is on, single inner padding if
		     // BordersOuterV is off). Scrolling
	ImGuiTableFlags_ScrollX =
	    1 << 25, // Enable horizontal scrolling. Require 'outer_size'
		     // parameter of BeginTable() to specify the container size.
		     // Changes default sizing policy. Because this create a
		     // child window, ScrollY is currently generally recommended
		     // when using ScrollX.
	ImGuiTableFlags_ScrollY =
	    1 << 26 // Enable vertical scrolling. Require 'outer_size' parameter
...
enum ImGuiTableColumnFlags_ {
	...
	ImGuiTableColumnFlags_DefaultHide =
	    1 << 0, // Default as a hidden/disabled column.
	ImGuiTableColumnFlags_DefaultSort =
	    1 << 1, // Default as a sorting column.
	ImGuiTableColumnFlags_WidthStretch =
	    1 << 2, // Column will stretch. Preferable with horizontal scrolling
		    // disabled (default if table sizing policy is
		    // _ColumnsWidthStretch).
	ImGuiTableColumnFlags_WidthFixed =
	    1 << 3, // Column will not stretch. Preferable with horizontal
		    // scrolling enabled (default if table sizing policy is
		    // _ColumnsWidthFixed and table is resizable).
	ImGuiTableColumnFlags_WidthAutoResize =
	    1 << 4, // Column will not stretch and keep resizing based on
		    // submitted contents (default if table sizing policy is
		    // _ColumnsWidthFixed and table is not resizable).
	...
	ImGuiTableColumnFlags_NoHeaderWidth =
	    1 << 12, // Header width don't contribute to automatic column width.
	ImGuiTableColumnFlags_PreferSortAscending =
	    1 << 13, // Make the initial sort direction Ascending when first
		     // sorting on this column (default).
	ImGuiTableColumnFlags_PreferSortDescending =
	    1 << 14, // Make the initial sort direction Descending when first
		     // sorting on this column.
	ImGuiTableColumnFlags_IndentEnable =
	    1 << 15, // Use current Indent value when entering cell (default for
		     // column 0).
	ImGuiTableColumnFlags_IndentDisable =
	    1 << 16, // Ignore current Indent value when entering cell (default
		     // for columns > 0). Indentation changes _within_ the cell
		     // will still be honored.

	// Output status flags, read-only via TableGetColumnFlags()
	ImGuiTableColumnFlags_IsEnabled =
	    1 << 20, // Status: is enabled == not hidden by user/api (referred
		     // to as "Hide" in _DefaultHide and _NoHide) flags.
	ImGuiTableColumnFlags_IsVisible =
	    1 << 21, // Status: is visible == is enabled AND not clipped by
		     // scrolling.
	ImGuiTableColumnFlags_IsSorted =
	    1 << 22, // Status: is currently part of the sort specs
	ImGuiTableColumnFlags_IsHovered = 1
					  << 23, // Status: is hovered by mouse
	...
	ImGuiTableColumnFlags_WidthMask_ =
	    ...
	    ImGuiTableColumnFlags_WidthFixed |
	    ImGuiTableColumnFlags_WidthAutoResize,
	ImGuiTableColumnFlags_IndentMask_ = ImGuiTableColumnFlags_IndentEnable |
					    ImGuiTableColumnFlags_IndentDisable,
	...
	ImGuiTableColumnFlags_NoDirectResize_ =
	    1 << 30 // [Internal] Disable user resizing this column directly (it
...
enum ImGuiTableRowFlags_ {
	...
	ImGuiTableRowFlags_Headers =
	    1 << 0 // Identify header row (set default background color + width
		   // of its contents accounted different for auto column width)
};
...
enum ImGuiTableBgTarget_ {
	...
	// ImGuiTableBgTarget_ColumnBg0                  = 1,        //
	// FIXME-TABLE: Todo. Set column background color 0 (generally used for
	// background ImGuiTableBgTarget_ColumnBg1                  = 2, //
	// FIXME-TABLE: Todo. Set column background color 1 (generally used for
	// selection marking)
	ImGuiTableBgTarget_RowBg0 =
	    3, // Set row background color 0 (generally used for background,
	       // automatically set when ImGuiTableFlags_RowBg is used)
	ImGuiTableBgTarget_RowBg1 = 4, // Set row background color 1 (generally
				       // used for selection marking)
	ImGuiTableBgTarget_CellBg =
	    5 // Set cell background color (top-most color)
};
...
enum ImGuiFocusedFlags_ {
	...
	ImGuiFocusedFlags_ChildWindows =
	    1 << 0, // IsWindowFocused(): Return true if any children of the
		    // window is focused
	ImGuiFocusedFlags_RootWindow =
	    1 << 1, // IsWindowFocused(): Test from root window (top most parent
		    // of the current hierarchy)
	ImGuiFocusedFlags_AnyWindow =
	    1 << 2, // IsWindowFocused(): Return true if any window is focused.
		    // Important: If you are trying to tell how to dispatch your
		    // low-level inputs, do NOT use this. Use
...
enum ImGuiHoveredFlags_ {
	ImGuiHoveredFlags_None =
	    0, // Return true if directly over the item/window, not obstructed
	       // by another window, not obstructed by an active popup or modal
	       // blocking inputs under them.
	ImGuiHoveredFlags_ChildWindows =
	    1 << 0, // IsWindowHovered() only: Return true if any children of
		    // the window is hovered
	ImGuiHoveredFlags_RootWindow =
	    1 << 1, // IsWindowHovered() only: Test from root window (top most
		    // parent of the current hierarchy)
	ImGuiHoveredFlags_AnyWindow = 1 << 2, // IsWindowHovered() only: Return
					      // true if any window is hovered
	ImGuiHoveredFlags_AllowWhenBlockedByPopup =
	    1 << 3, // Return true even if a popup window is normally blocking
		    // access to this item/window
	// ImGuiHoveredFlags_AllowWhenBlockedByModal     = 1 << 4,   // Return
	// true even if a modal popup window is normally blocking access to this
	...
	ImGuiHoveredFlags_AllowWhenOverlapped =
	    1 << 6, // Return true even if the position is obstructed or
...
enum ImGuiDragDropFlags_ {
	...
	ImGuiDragDropFlags_SourceNoPreviewTooltip =
	    1
	    << 0, // By default, a successful call to BeginDragDropSource opens
		  // a tooltip so you can display a preview or description of
		  // the source contents. This flag disable this behavior.
	ImGuiDragDropFlags_SourceNoDisableHover =
	    1
	    << 1, // By default, when dragging we clear data so that
		  // IsItemHovered() will return false, to avoid subsequent user
		  // code submitting tooltips. This flag disable this behavior
	...
	ImGuiDragDropFlags_SourceNoHoldToOpenOthers =
	    1 << 2, // Disable the behavior that allows to open tree nodes and
		    // collapsing header by holding over them while dragging a
		    // source item.
	ImGuiDragDropFlags_SourceAllowNullID =
	    1 << 3, // Allow items such as Text(), Image() that have no unique
		    // identifier to be used as drag source, by manufacturing a
		    // temporary identifier based on their window-relative
		    // position. This is extremely unusual within the dear imgui
		    // ecosystem and so we made it explicit.
	ImGuiDragDropFlags_SourceExtern =
	    1
	    << 4, // External source (from outside of dear imgui), won't attempt
	...
	ImGuiDragDropFlags_SourceAutoExpirePayload =
	    1 << 5, // Automatically expire the payload if the source cease to
		    // be submitted (otherwise payloads are persisting while
		    // being dragged)
	// AcceptDragDropPayload() flags
	ImGuiDragDropFlags_AcceptBeforeDelivery =
	    1
	    << 10, // AcceptDragDropPayload() will returns true even before the
		   // mouse button is released. You can then call IsDelivery()
		   // to test if the payload needs to be delivered.
	ImGuiDragDropFlags_AcceptNoDrawDefaultRect =
	    1 << 11, // Do not draw the default highlight rectangle when
...
#define IMGUI_PAYLOAD_TYPE_COLOR_3F                                            \
	"_COL3F" // float[3]: Standard type for colors, without alpha. User code
		 // may use this type.
#define IMGUI_PAYLOAD_TYPE_COLOR_4F                                            \
	"_COL4F" // float[4]: Standard type for colors. User code may use this
		 // type.
...
// To test io.KeyMods (which is a combination of individual fields io.KeyCtrl,
// io.KeyShift, io.KeyAlt set by user/backend)
...
// ImGuiConfigFlags_NavEnableGamepad to enable. Backend: set
...
enum ImGuiNavInput_ {
	...
	ImGuiNavInput_Activate, // activate / open / toggle / tweak value //
				// e.g. Cross  (PS4), A (Xbox), A (Switch),
				// Space (Keyboard)
	ImGuiNavInput_Cancel, // cancel / close / exit                        //
			      // e.g. Circle (PS4), B (Xbox), B (Switch), Escape
			      // (Keyboard)
	ImGuiNavInput_Input,  // text input / on-screen keyboard              //
			      // e.g. Triang.(PS4), Y (Xbox), X (Switch), Return
			      // (Keyboard)
	ImGuiNavInput_Menu,   // tap: toggle menu / hold: focus, move, resize //
			      // e.g. Square (PS4), X (Xbox), Y (Switch), Alt
			      // (Keyboard)
	ImGuiNavInput_DpadLeft, // move / tweak / resize window (w/ PadMenu) //
				// e.g. D-pad Left/Right/Up/Down (Gamepads),
				// Arrow keys (Keyboard)
	ImGuiNavInput_DpadRight,  //
	ImGuiNavInput_DpadUp,	  //
	ImGuiNavInput_DpadDown,	  //
	ImGuiNavInput_LStickLeft, // scroll / move window (w/ PadMenu) // e.g.
	...
	ImGuiNavInput_TweakSlow, // slower tweaks // e.g. L1 or L2 (PS4), LB or
				 // LT (Xbox), L or ZL (Switch)
	ImGuiNavInput_TweakFast, // faster tweaks // e.g. R1 or R2 (PS4), RB or
				 // RT (Xbox), R or ZL (Switch)

	// [Internal] Don't use directly! This is used internally to
	// differentiate keyboard from gamepad inputs for behaviors that require
	// to differentiate them. Keyboard behavior that have no corresponding
	// gamepad mapping (e.g. CTRL+TAB) will be directly reading from
...
enum ImGuiConfigFlags_ {
	...
	ImGuiConfigFlags_NavEnableGamepad =
	    1 << 1, // Master gamepad navigation enable flag. This is mostly to
		    // instruct your imgui backend to fill io.NavInputs[].
		    // Backend also needs to set ImGuiBackendFlags_HasGamepad.
	ImGuiConfigFlags_NavEnableSetMousePos =
	    1 << 2, // Instruct navigation to move the mouse cursor. May be
		    // useful on TV/console systems where moving a virtual mouse
		    // is awkward. Will update io.MousePos and set
		    // io.WantSetMousePos=true. If enabled you MUST honor
		    // io.WantSetMousePos requests in your backend, otherwise
		    // ImGui will react as if the mouse is jumping around back
		    // and forth.
	ImGuiConfigFlags_NavNoCaptureKeyboard =
	    1 << 3, // Instruct navigation to not set the io.WantCaptureKeyboard
		    // flag when io.NavActive is set.
	ImGuiConfigFlags_NoMouse =
	    1 << 4, // Instruct imgui to clear mouse position/buttons in
		    // NewFrame(). This allows ignoring the mouse information
		    // set by the backend.
	ImGuiConfigFlags_NoMouseCursorChange =
	    1 << 5, // Instruct backend to not alter mouse cursor shape and
		    // visibility. Use if the backend cursor changes are
		    // interfering with yours and you don't want to use
		    // SetMouseCursor() to change mouse cursor. You may want to
		    // honor requests from imgui by reading GetMouseCursor()
		    // yourself instead.

	// User storage (to allow your backend/engine to communicate to code
	// that may be shared between multiple projects. Those flags are not
	// used by core Dear ImGui)
...
enum ImGuiBackendFlags_ {
	...
	ImGuiBackendFlags_HasGamepad =
	    1 << 0, // Backend Platform supports gamepad and currently has one
		    // connected.
	ImGuiBackendFlags_HasMouseCursors =
	...
	ImGuiBackendFlags_RendererHasVtxOffset =
	    1 << 3 // Backend Renderer supports ImDrawCmd::VtxOffset. This
		   // enables output of large meshes (64K+ vertices) while still
...
enum ImGuiCol_ {
	...
	ImGuiCol_FrameBg, // Background of checkbox, radio button, plot, slider,
			  // text input
	...
	ImGuiCol_Header, // Header* colors are used for CollapsingHeader,
			 // TreeNode, Selectable, MenuItem
	...
	ImGuiCol_TableBorderStrong, // Table outer and header borders (prefer
				    // using Alpha=1.0 here)
	ImGuiCol_TableBorderLight,  // Table inner borders (prefer using
				    // Alpha=1.0 here)
	...
	ImGuiCol_ModalWindowDimBg,  // Darken/colorize entire screen behind a
				    // modal window, when one is active
	ImGuiCol_COUNT
...
// Obsolete names (will be removed)
	...
	ImGuiCol_ModalWindowDarkening =
	    ImGuiCol_ModalWindowDimBg // [renamed in 1.63]
#endif
};
...
// offset/type.
enum ImGuiStyleVar_ {
	// Enum name --------------------- // Member in ImGuiStyle structure
...
enum ImGuiButtonFlags_ {
	...
	ImGuiButtonFlags_MouseButtonRight = 1
					    << 1, // React on right mouse button
...
enum ImGuiColorEditFlags_ {
	...
	ImGuiColorEditFlags_NoAlpha =
	    1 << 1, //              // ColorEdit, ColorPicker, ColorButton:
		    //              ignore Alpha component (will only read 3
		    //              components from the input pointer).
	ImGuiColorEditFlags_NoPicker =
	    1 << 2, //              // ColorEdit: disable picker when clicking
		    //              on color square.
	ImGuiColorEditFlags_NoOptions =
	    1 << 3, //              // ColorEdit: disable toggling options menu
		    //              when right-clicking on inputs/small preview.
	ImGuiColorEditFlags_NoSmallPreview =
	    1 << 4, //              // ColorEdit, ColorPicker: disable color
		    //              square preview next to the inputs. (e.g. to
		    //              show only the inputs)
	ImGuiColorEditFlags_NoInputs =
	    1 << 5, //              // ColorEdit, ColorPicker: disable inputs
		    //              sliders/text widgets (e.g. to show only the
		    //              small preview color square).
	ImGuiColorEditFlags_NoTooltip =
	    1 << 6, //              // ColorEdit, ColorPicker, ColorButton:
	...
	ImGuiColorEditFlags_NoSidePreview =
	    1 << 8, //              // ColorPicker: disable bigger color preview
		    //              on right side of the picker, use small color
		    //              square preview instead.
	ImGuiColorEditFlags_NoDragDrop =
	    1 << 9, //              // ColorEdit: disable drag and drop target.
		    //              ColorButton: disable drag and drop source.
	ImGuiColorEditFlags_NoBorder =
	    1 << 10, //              // ColorButton: disable border (which is
		     //              enforced by default)

	// User Options (right-click on widget to change some of them).
	ImGuiColorEditFlags_AlphaBar =
	    1 << 16, //              // ColorEdit, ColorPicker: show vertical
		     //              alpha bar/gradient in picker.
	ImGuiColorEditFlags_AlphaPreview =
	    1 << 17, //              // ColorEdit, ColorPicker, ColorButton:
		     //              display preview as a transparent color over
		     //              a checkerboard, instead of opaque.
	...
	ImGuiColorEditFlags_HDR =
	    1 << 19, //              // (WIP) ColorEdit: Currently only disable
		     //              0.0f..1.0f limits in RGBA edition (note:
		     //              you probably want to use
		     //              ImGuiColorEditFlags_Float flag as well).
	ImGuiColorEditFlags_DisplayRGB =
	    1 << 20, // [Display]    // ColorEdit: override _display_ type among
		     // RGB/HSV/Hex. ColorPicker: select any combination using
		     // one or more of RGB/HSV/Hex.
	...
	ImGuiColorEditFlags_PickerHueBar =
	    1 << 25, // [Picker]     // ColorPicker: bar for Hue, rectangle for
		     // Sat/Value.
	ImGuiColorEditFlags_PickerHueWheel =
	    1 << 26, // [Picker]     // ColorPicker: wheel for Hue, triangle for
		     // Sat/Value.
	ImGuiColorEditFlags_InputRGB =
	    1 << 27, // [Input]      // ColorEdit, ColorPicker: input and output
		     // data in RGB format.
	ImGuiColorEditFlags_InputHSV =
	    1 << 28, // [Input]      // ColorEdit, ColorPicker: input and output
		     // data in HSV format.

	// Defaults Options. You can set application defaults using
	// SetColorEditOptions(). The intent is that you probably don't want to
	// override them in most of your calls. Let the user choose via the
	// option menu and/or call SetColorEditOptions() once during startup.
	...
	ImGuiColorEditFlags__PickerMask = ImGuiColorEditFlags_PickerHueWheel |
					  ImGuiColorEditFlags_PickerHueBar,
...
enum ImGuiSliderFlags_ {
	...
	ImGuiSliderFlags_AlwaysClamp =
	    1 << 4, // Clamp value to min/max bounds when input manually with
		    // CTRL+Click. By default CTRL+Click allows going out of
		    // bounds.
	ImGuiSliderFlags_Logarithmic =
	    1 << 5, // Make the widget logarithmic (linear otherwise). Consider
		    // using ImGuiSliderFlags_NoRoundToFormat with this if using
		    // a format-string with small amount of digits.
	ImGuiSliderFlags_NoRoundToFormat =
	    1 << 6, // Disable rounding underlying value to match precision of
		    // the display format string (e.g. %.3f values are rounded
		    // to those 3 digits)
	ImGuiSliderFlags_NoInput =
	    1 << 7, // Disable CTRL+Click or Enter key allowing to input text
		    // directly into the widget
	ImGuiSliderFlags_InvalidMask_ =
	    0x7000000F // [Internal] We treat using those bits as being
		       // potentially a 'float power' argument from the previous
		       // API that has got miscast to this enum, and will
...
// Obsolete names (will be removed)
	...
	ImGuiSliderFlags_ClampOnInput =
	    ImGuiSliderFlags_AlwaysClamp // [renamed in 1.79]
#endif
};
...
enum ImGuiMouseCursor_ {
	...
	ImGuiMouseCursor_ResizeEW,  // When hovering over a vertical border or a
				    // column
	ImGuiMouseCursor_ResizeNESW, // When hovering over the bottom-left
				     // corner of a window
	ImGuiMouseCursor_ResizeNWSE, // When hovering over the bottom-right
				     // corner of a window
...
enum ImGuiCond_ {
	...
	ImGuiCond_Once = 1 << 1,   // Set the variable once per runtime session
	...
	ImGuiCond_Appearing =
	    1 << 3 // Set the variable if the object/window is appearing after
...
template <typename T> struct ImVector {
	...
	// NB: It is illegal to call push_back/push_front/insert with a
	// reference pointing inside the ImVector data itself! e.g.
	...
	inline T *erase(const T *it, const T *it_last)
	{
		...
		memmove(Data + off, Data + off + count,
			((size_t)Size - (size_t)off - count) * sizeof(T));
		Size -= (int)count;
		return Data + off;
	}
...
struct ImGuiStyle {
	...
	float WindowRounding; // Radius of window corners rounding. Set to 0.0f
			      // to have rectangular windows. Large values tend
			      // to lead to variety of artifacts and are not
			      // recommended.
	...
	ImVec2 WindowMinSize; // Minimum window size. This is a global setting.
			      // If you want to constraint individual windows,
			      // use SetNextWindowSizeConstraints().
	ImVec2 WindowTitleAlign; // Alignment for title bar text. Defaults to
				 // (0.0f,0.5f) for left-aligned,vertically
				 // centered.
	ImGuiDir
	    WindowMenuButtonPosition; // Side of the collapsing/docking button
	...
	float PopupBorderSize; // Thickness of border around popup/tooltip
			       // windows. Generally set to 0.0f or 1.0f. (Other
			       // values are not well tested and more CPU/GPU
			       // costly).
	...
	float FrameBorderSize;	 // Thickness of border around frames. Generally
				 // set to 0.0f or 1.0f. (Other values are not
				 // well tested and more CPU/GPU costly).
	ImVec2 ItemSpacing;	 // Horizontal and vertical spacing between
				 // widgets/lines.
	ImVec2 ItemInnerSpacing; // Horizontal and vertical spacing between
				 // within elements of a composed widget (e.g. a
	...
	ImVec2
	    TouchExtraPadding; // Expand reactive bounding box for touch-based
			       ...
			       // priority on overlap will always be given to
			       // the first widget. So don't grow this too much!
	float
	    IndentSpacing; // Horizontal indentation when e.g. entering a tree
			   // node. Generally == (FontSize + FramePadding.x*2).
	float ColumnsMinSpacing; // Minimum horizontal spacing between two
	...
	float LogSliderDeadzone; // The size in pixels of the dead-zone around
				 // zero on logarithmic sliders that cross zero.
	float TabRounding;   // Radius of upper corners of a tab. Set to 0.0f to
			     // have rectangular tabs.
	float TabBorderSize; // Thickness of border around tabs.
	float
	    TabMinWidthForCloseButton; // Minimum width for close button to
				       // appears on an unselected tab when
				       // hovered. Set to 0.0f to always show
				       // when hovering, set to FLT_MAX to never
				       // show close button unless selected.
	ImGuiDir ColorButtonPosition;  // Side of the color button in the
				       // ColorEdit4 widget (left/right).
				       // Defaults to ImGuiDir_Right.
	ImVec2
	    ButtonTextAlign; // Alignment of button text when button is larger
			     // than text. Defaults to (0.5f, 0.5f) (centered).
	ImVec2 SelectableTextAlign; // Alignment of selectable text. Defaults to
				    // (0.0f, 0.0f) (top-left aligned). It's
				    // generally important to keep this
				    // left-aligned if you want to lay multiple
				    // items on a same line.
	ImVec2 DisplayWindowPadding; // Window position are clamped to be
				     // visible within the display area or
				     // monitors by at least this amount. Only
				     // applies to regular windows.
	ImVec2
	    DisplaySafeAreaPadding; // If you cannot see the edges of your
				    // screen (e.g. on a TV) increase the safe
				    // area padding. Apply to popups/tooltips as
				    // well regular windows. NB: Prefer
				    // configuring your TV sets correctly!
	float MouseCursorScale; // Scale software rendered mouse cursor (when
				// io.MouseDrawCursor is enabled). May be
				// removed later.
	bool
	    AntiAliasedLines; // Enable anti-aliased lines/borders. Disable if
			      // you are really tight on CPU/GPU. Latched at the
			      // beginning of the frame (copied to ImDrawList).
	bool AntiAliasedLinesUseTex; // Enable anti-aliased lines/borders using
				     // textures where possible. Require backend
				     // to render with bilinear filtering.
				     // Latched at the beginning of the frame
				     // (copied to ImDrawList).
	bool AntiAliasedFill; // Enable anti-aliased edges around filled shapes
			      // (rounded rectangles, circles, etc.). Disable if
			      // you are really tight on CPU/GPU. Latched at the
			      // beginning of the frame (copied to ImDrawList).
	...
	float
	    CircleSegmentMaxError; // Maximum error (in pixels) allowed when
...
struct ImGuiIO {
	...
	ImGuiConfigFlags
	    ConfigFlags; // = 0              // See ImGuiConfigFlags_ enum. Set
			 // by user/application. Gamepad/keyboard navigation
			 // options, etc.
	ImGuiBackendFlags
	    BackendFlags;   // = 0              // See ImGuiBackendFlags_ enum.
			    // Set by backend (imgui_impl_xxx files or custom
			    // backend) to communicate features supported by the
			    // backend.
	ImVec2 DisplaySize; // <unset>          // Main display size, in pixels.
	float DeltaTime; // = 1.0f/60.0f     // Time elapsed since last frame,
			 // in seconds.
	float IniSavingRate; // = 5.0f           // Minimum time between saving
			     // positions/sizes to .ini file, in seconds.
	const char
	    *IniFilename; // = "imgui.ini"    // Path to .ini file. Set NULL to
			  // disable automatic .ini loading/saving, if e.g. you
			  // want to manually load/save from memory.
	const char *LogFilename; // = "imgui_log.txt"// Path to .log file
				 // (default parameter to ImGui::LogToFile when
	...
	float MouseDoubleClickMaxDist; // = 6.0f           // Distance threshold
				       // to stay in to validate a double-click,
				       // in pixels.
	...
	float KeyRepeatDelay; // = 0.250f         // When holding a key/button,
			      // time before it starts repeating, in seconds
	...
	ImFontAtlas *Fonts; // <auto>           // Font atlas: load, rasterize
			    // and pack one or more fonts into a single texture.
	float FontGlobalScale;	   // = 1.0f           // Global scale all fonts
	bool FontAllowUserScaling; // = false          // Allow user scaling
				   // text of individual window with CTRL+Wheel.
	...
	ImVec2
	    DisplayFramebufferScale; // = (1, 1)         // For retina display
				     // or other situations where window
				     // coordinates are different from
				     // framebuffer coordinates. This generally
				     // ends up in ImDrawData::FramebufferScale.

	// Miscellaneous options
	bool MouseDrawCursor; // = false          // Request ImGui to draw a
			      // mouse cursor for you (if you are on a platform
			      // without a mouse cursor). Cannot be easily
			      // renamed to 'io.ConfigXXX' because this is
			      // frequently used by backend implementations.
	bool
	    ConfigMacOSXBehaviors; // = defined(__APPLE__) // OS X style: Text
				   // editing cursor movement using Alt instead
				   // of Ctrl, Shortcuts using Cmd/Super instead
				   // of Ctrl, Line/Text Start and End using
				   // Cmd+Arrows instead of Home/End, Double
				   // click selects by word instead of selecting
				   // whole text, Multi-selection in lists uses
				   // Cmd/Super instead of Ctrl (was called
				   // io.OptMacOSXBehaviors prior to 1.63)
	bool ConfigInputTextCursorBlink; // = true           // Set to false to
					 // disable blinking cursor, for users
					 // who consider it distracting. (was
					 // called: io.OptCursorBlink prior
					 // to 1.63)
	bool
	    ConfigWindowsResizeFromEdges; // = true           // Enable resizing
					  // of windows from their edges and
					  // from the lower-left corner. This
					  // requires (io.BackendFlags &
					  // ImGuiBackendFlags_HasMouseCursors)
					  // because it needs mouse cursor
					  // feedback. (This used to be a
					  // per-window
					  // ImGuiWindowFlags_ResizeFromAnySide
					  // flag)
	bool ConfigWindowsMoveFromTitleBarOnly; // = false       // [BETA] Set
						// to true to only allow moving
						// windows when clicked+dragged
						// from the title bar. Windows
						// without a title bar are not
						// affected.
	float ConfigMemoryCompactTimer; // = 60.0f          // [BETA] Free
					// transient windows/tables memory
					// buffers when unused for given amount
					// of time. Set to -1.0f to disable.
	...
	// displayed in About Window) + User data for backend/wrappers to store
	// their own stuff.
	...
	void *BackendRendererUserData;	 // = NULL           // User data for
					 // renderer backend
	void *BackendLanguageUserData; // = NULL           // User data for non
				       // C++ programming language backend

	// Optional: Access OS clipboard
	// (default to use native Win32 clipboard on Windows, otherwise uses a
	// private clipboard. Override to access OS clipboard on other
	// architectures)
	...
	void *ImeWindowHandle; // = NULL           // (Windows) Set this to your
			       // HWND to get automatic IME cursor positioning.

	//------------------------------------------------------------------
	// Input - Fill before calling NewFrame()
	//------------------------------------------------------------------

	ImVec2 MousePos;   // Mouse position, in pixels. Set to ImVec2(-FLT_MAX,
			   // -FLT_MAX) if mouse is unavailable (on another
			   // screen, etc.)
	bool MouseDown[5]; // Mouse buttons: 0=left, 1=right, 2=middle + extras
			   // (ImGuiMouseButton_COUNT == 5). Dear ImGui mostly
			   // uses left and right buttons. Others buttons allows
			   // us to track if the mouse is being used by your
			   // application + available to user as a convenience
			   // via IsMouse** API.
	float MouseWheel;  // Mouse wheel Vertical: 1 unit scrolls about 5 lines
			   // text.
	float MouseWheelH; // Mouse wheel Horizontal. Most users don't have a
			   // mouse with an horizontal wheel, may not be filled
			   // by all backends.
	bool KeyCtrl;	   // Keyboard modifier pressed: Control
	bool KeyShift;	   // Keyboard modifier pressed: Shift
	bool KeyAlt;	   // Keyboard modifier pressed: Alt
	bool KeySuper;	   // Keyboard modifier pressed: Cmd/Super/Windows
	bool KeysDown[512]; // Keyboard keys that are pressed (ideally left in
			    // the "native" order your engine has access to
			    // keyboard keys, so you can use your own
			    // defines/enums for keys).
	float NavInputs[ImGuiNavInput_COUNT]; // Gamepad inputs. Cleared back to
					      // zero by EndFrame(). Keyboard
					      // keys will be auto-mapped and be
					      // written here by NewFrame().

	// Functions
	IMGUI_API void
	AddInputCharacter(unsigned int c); // Queue new character input
	IMGUI_API void AddInputCharacterUTF16(
	    ImWchar16 c); // Queue new character input from an UTF-16 character,
			  // it can be a surrogate
	IMGUI_API void AddInputCharactersUTF8(
	    const char *str); // Queue new characters input from an UTF-8 string
	IMGUI_API void
	ClearInputCharacters(); // Clear the text input buffer manually

	//------------------------------------------------------------------
	// Output - Updated by NewFrame() or EndFrame()/Render()
	// (when reading from the io.WantCaptureMouse, io.WantCaptureKeyboard
	// flags to dispatch your inputs, it is
	//  generally easier and more correct to use their state BEFORE calling
	//  NewFrame(). See FAQ for details!)
	//------------------------------------------------------------------

	bool WantCaptureMouse; // Set when Dear ImGui will use mouse inputs, in
			       // this case do not dispatch them to your main
			       // game/application (either way, always pass on
			       // mouse inputs to imgui). (e.g. unclicked mouse
			       // is hovering over an imgui window, widget is
			       // active, mouse was clicked over an imgui
			       // window, etc.).
	bool
	    WantCaptureKeyboard; // Set when Dear ImGui will use keyboard
				 // inputs, in this case do not dispatch them to
				 // your main game/application (either way,
				 // always pass keyboard inputs to imgui). (e.g.
				 // InputText active, or an imgui window is
				 // focused and navigation is enabled, etc.).
	bool WantTextInput;	 // Mobile/console: when set, you may display an
			    // on-screen keyboard. This is set by Dear ImGui
			    // when it wants textual keyboard input to happen
			    // (e.g. when a InputText widget is active).
	bool
	    WantSetMousePos; // MousePos has been altered, backend should
			     // reposition mouse on next frame. Rarely used! Set
			     // only when ImGuiConfigFlags_NavEnableSetMousePos
			     // flag is enabled.
	bool
	    WantSaveIniSettings; // When manual .ini load/save is active
				 // (io.IniFilename == NULL), this will be set
				 // to notify your application that you can call
				 // SaveIniSettingsToMemory() and save yourself.
				 // Important: clear io.WantSaveIniSettings
				 // yourself after saving!
	bool NavActive;	 // Keyboard/Gamepad navigation is currently allowed
			 // (will handle ImGuiKey_NavXXX events) = a window is
			 // focused and it doesn't use the
			 // ImGuiWindowFlags_NoNavInputs flag.
	bool NavVisible; // Keyboard/Gamepad navigation is visible and allowed
			 // (will handle ImGuiKey_NavXXX events).
	float Framerate; // Application framerate estimate, in frame per second.
			 // Solely for convenience. Rolling average estimation
			 // based on io.DeltaTime over 120 frames.
	int MetricsRenderVertices; // Vertices output during last call to
				   // Render()
	int MetricsRenderIndices; // Indices output during last call to Render()
				  // = number of triangles * 3
	int MetricsRenderWindows; // Number of visible windows
	int MetricsActiveWindows; // Number of active windows
	int MetricsActiveAllocations; // Number of active allocations, updated
				      // by MemAlloc/MemFree based on current
				      // context. May be off if you have
				      // multiple imgui contexts.
	ImVec2 MouseDelta; // Mouse delta. Note that this is zero if either
			   // current or previous position are invalid
			   // (-FLT_MAX,-FLT_MAX), so a disappearing/reappearing
			   // mouse won't have a huge delta.

	//------------------------------------------------------------------
	// [Internal] Dear ImGui will maintain those fields. Forward
	// compatibility not guaranteed!
	//------------------------------------------------------------------

	ImGuiKeyModFlags KeyMods; // Key mods flags (same as
				  // io.KeyCtrl/KeyShift/KeyAlt/KeySuper but
				  // merged into flags), updated by NewFrame()
	ImVec2 MousePosPrev; // Previous mouse position (note that MouseDelta is
			     // not necessary == MousePos-MousePosPrev, in case
	...
	bool MouseDownOwned[5];	    // Track if button was clicked inside a dear
				// imgui window. We don't request mouse capture
				// from the application if click started outside
				// ImGui bounds.
	bool MouseDownWasDoubleClick[5]; // Track if button down was a
					 // double-click
	float MouseDownDuration[5]; // Duration the mouse button has been down
				    // (0.0f == just clicked)
	float MouseDownDurationPrev[5];	   // Previous time the mouse button has
					   // been down
	ImVec2 MouseDragMaxDistanceAbs[5]; // Maximum distance, absolute, on
					   // each axis, of how much mouse has
					   // traveled from the clicking point
	float MouseDragMaxDistanceSqr[5];  // Squared maximum distance of how
					   // much mouse has traveled from the
					   // clicking point
	float KeysDownDuration[512]; // Duration the keyboard key has been down
				     // (0.0f == just pressed)
	float KeysDownDurationPrev[512]; // Previous duration the key has been
					 // down
	...
	ImVector<ImWchar>
	    InputQueueCharacters; // Queue of _characters_ input (obtained by
				  // platform backend). Fill using
...
struct ImGuiInputTextCallbackData {
	...
	// InsertChars() / DeleteChars() function. InsertChars() will take care
	// of calling the resize callback if necessary.
	...
	// You need to update 'BufTextLen' accordingly (0 <= BufTextLen <
	// BufSize) and set 'BufDirty'' to true so InputText can update its
	// internal state.
	ImWchar EventChar; // Character input                      // Read-write
			   // // [CharFilter] Replace character with another
			   // one, or set to zero to drop. return 1 is
	...
	char *Buf; // Text buffer                          // Read-write   //
		   ...
		   // Only write to pointed data, don't replace the actual
		   // pointer!
	...
	int BufSize; // Buffer size (in bytes) = capacity+1  // Read-only    //
		     // [Resize,Completion,History,Always] Include
		     // zero-terminator storage. In C land ==
		     // ARRAYSIZE(my_char_array), in C++ land:
		     // string.capacity()+1
	...
	// Use those function to benefit from the CallbackResize behaviors.
	// Calling those function reset the selection.
...
struct ImGuiSizeCallbackData {
	...
	ImVec2
	    DesiredSize; // Read-write.  Desired size, based on user's mouse
...
struct ImGuiPayload {
	...
	char DataType[32 + 1];	// Data type tag (short user-supplied string, 32
				// characters max)
	bool Preview;  // Set when AcceptDragDropPayload() was called and mouse
		       // has been hovering the target item (nb: handle
...
struct ImGuiTableColumnSortSpecs {
	...
	ImS16 SortOrder;      // Index within parent ImGuiTableSortSpecs (always
			 // stored in order starting from 0, tables sorted on a
			 // single criteria will always have a 0 here)
	ImGuiSortDirection
	    SortDirection : 8; // ImGuiSortDirection_Ascending or
			       // ImGuiSortDirection_Descending (you can use
			       // this or SortSign, whichever is more convenient
			       // for your sort function)
...
struct ImGuiTableSortSpecs {
	...
	int SpecsCount;	 // Sort spec count. Most often 1 unless e.g.
			 // ImGuiTableFlags_MultiSortable is enabled.
	bool SpecsDirty; // Set to true when specs have changed since last time!
			 // Use this to sort again, then clear the flag.
...
// imgui.cpp for details) Please keep your copy of dear imgui up to date!
// Occasionally set '#define IMGUI_DISABLE_OBSOLETE_FUNCTIONS' in imconfig.h to
// stay ahead.
//-----------------------------------------------------------------------------
...
} // Bool return value removed. Use IsWindowAppearing() in BeginPopup() instead.
  // Renamed in 1.77, renamed back in 1.79. Sorry!
// OBSOLETED in 1.78 (from June 2020)
// Old drag/sliders functions that took a 'float power = 1.0' argument instead
// of flags. For shared code, you can version check at compile-time with `#if
// IMGUI_VERSION_NUM >= 17704`.
IMGUI_API bool DragScalar(const char *label, ImGuiDataType data_type,
			  void *p_data, float v_speed, const void *p_min,
			  const void *p_max, const char *format, float power);
IMGUI_API bool DragScalarN(const char *label, ImGuiDataType data_type,
			   void *p_data, int components, float v_speed,
			   const void *p_min, const void *p_max,
			   const char *format, float power);
static inline bool DragFloat(const char *label, float *v, float v_speed,
...
static inline bool DragFloat2(const char *label, float v[2], float v_speed,
			      float v_min, float v_max, const char *format,
			      float power)
{
...
static inline bool DragFloat4(const char *label, float v[4], float v_speed,
			      float v_min, float v_max, const char *format,
			      float power)
{
...
IMGUI_API bool SliderScalarN(const char *label, ImGuiDataType data_type,
			     ...
			     float power);
static inline bool SliderFloat(const char *label, float *v, float v_min,
			       float v_max, const char *format, float power)
{
	return SliderScalar(label, ImGuiDataType_Float, v, &v_min, &v_max,
			    format, power);
}
static inline bool SliderFloat2(const char *label, float v[2], float v_min,
				float v_max, const char *format, float power)
{
	return SliderScalarN(label, ImGuiDataType_Float, v, 2, &v_min, &v_max,
			     format, power);
}
static inline bool SliderFloat3(const char *label, float v[3], float v_min,
				float v_max, const char *format, float power)
{
	return SliderScalarN(label, ImGuiDataType_Float, v, 3, &v_min, &v_max,
			     format, power);
}
static inline bool SliderFloat4(const char *label, float v[4], float v_min,
				float v_max, const char *format, float power)
{
...
static inline bool BeginPopupContextWindow(const char *str_id,
					   ImGuiMouseButton mb, bool over_items)
{
...
typedef ImGuiInputTextCallback
    ImGuiTextEditCallback; // OBSOLETED in 1.63 (from Aug 2018): made the names
			   // consistent
...
struct ImGuiTextFilter {
	...
	IMGUI_API bool
	Draw(const char *label = "Filter (inc,-exc)",
...
struct ImGuiStorage {
	...
	// - Get***() functions find pair, never add/allocate. Pairs are sorted
	// so a query is O(log N)
	// - Set***() functions find pair, insertion on demand if missing.
	// - Sorted insertion is costly, paid once. A typical frame shouldn't
	// need to insert any new pair.
	...
	// - References are only valid until a new value is added to the
	// storage. Calling a Set***() function or a Get***Ref() function
	// invalidates the pointer.
	// - A typical use case where this is convenient for quick hacking (e.g.
	// add storage during a live Edit&Continue session if you can't modify
	// existing struct)
	//      float* pvar = ImGui::GetFloatRef(key); ImGui::SliderFloat("var",
	//      pvar, 0, 100.0f); some_var += *pvar;
	...
	// For quicker full rebuild of a storage (instead of an incremental
...
// non-visible items we have skipped. (Dear ImGui already clip items based on
// their bounds but it needs to measure text size to do so, whereas manual
// coarse clipping before submission makes this cost and your own data
// fetching/submission cost almost null) Usage:
...
struct ImGuiListClipper {
	...
	IMGUI_API void
	Begin(int items_count,
	      float items_height =
		  -1.0f); // Automatically called by constructor if you passed
			  // 'items_count' or by Step() in Step 1.
	IMGUI_API void End(); // Automatically called on the last call of Step()
			      // that returns false.
	IMGUI_API bool
	Step(); // Call until it returns false. The DisplayStart/DisplayEnd
...
//-----------------------------------------------------------------------------
// Draw List API (ImDrawCmd, ImDrawIdx, ImDrawVert, ImDrawChannel,
...
// - VtxOffset/IdxOffset: When 'io.BackendFlags &
// ImGuiBackendFlags_RendererHasVtxOffset' is enabled,
//   those fields allow us to render meshes larger than 64K vertices while
//   keeping 16-bit indices. Pre-1.71 backends will typically ignore the
//   VtxOffset/IdxOffset fields.
// - The ClipRect/TextureId/VtxOffset fields must be contiguous as we memcmp()
// them together (this is asserted for).
struct ImDrawCmd {
	ImVec4 ClipRect; // 4*4  // Clipping rectangle (x1, y1, x2, y2).
			 // Subtract ImDrawData->DisplayPos to get clipping
			 // rectangle in "viewport" coordinates
	ImTextureID TextureId; // 4-8  // User-provided texture ID. Set by user
			       // in ImfontAtlas::SetTexID() for fonts or passed
			       // to Image*() functions. Ignore if never using
			       // images or multiple fonts atlas.
	unsigned int
	    VtxOffset; // 4    // Start offset in vertex buffer.
		       // ImGuiBackendFlags_RendererHasVtxOffset: always 0,
		       // otherwise may be >0 to support meshes larger than 64K
		       // vertices with 16-bit indices.
	unsigned int IdxOffset; // 4    // Start offset in index buffer. Always
				// equal to sum of ElemCount drawn so far.
	unsigned int ElemCount; // 4    // Number of indices (multiple of 3) to
				// be rendered as triangles. Vertices are stored
				// in the callee ImDrawList's vtx_buffer[]
				// array, indices in idx_buffer[].
	ImDrawCallback
	    UserCallback; // 4-8  // If != NULL, call the function instead of
			  // rendering the vertices. clip_rect and texture_id
			  // will be set normally.
	void *
	    UserCallbackData; // 4-8  // The draw callback code can access this.
...
// ImGuiBackendFlags_RendererHasVtxOffset' and handle ImDrawCmd::VtxOffset in
// the renderer backend (recommended). To use 32-bit indices: override with
...
// declared a the time you'd want to set your type up. NOTE: IMGUI DOESN'T CLEAR
...
struct ImDrawListSplitter {
	...
	IMGUI_API void SetCurrentChannel(ImDrawList *draw_list,
					 int channel_idx);
};
...
enum ImDrawCornerFlags_ {
	...
	ImDrawCornerFlags_All =
	    0xF // In your function calls you may use ~0 (= all bits sets)
		// instead of ImDrawCornerFlags_All, as a convenience
};

// Flags for ImDrawList. Those are set automatically by ImGui:: functions from
// ImGuiIO settings, and generally not manipulated directly. It is however
// possible to temporarily alter flags between calls to ImDrawList:: functions.
enum ImDrawListFlags_ {
	ImDrawListFlags_None = 0,
	ImDrawListFlags_AntiAliasedLines =
	    1
	    << 0, // Enable anti-aliased lines/borders (*2 the number of
		  // triangles for 1.0f wide line or lines thin enough to be
		  // drawn using textures, otherwise *3 the number of triangles)
	ImDrawListFlags_AntiAliasedLinesUseTex =
	    1 << 1, // Enable anti-aliased lines/borders using textures when
...
struct ImDrawList {
	...
	unsigned int _VtxCurrentIdx; // [Internal] generally == VtxBuffer.Size
				     // unless we are past 64K vertices, in
				     // which case this gets reset to 0.
	const ImDrawListSharedData
	    *_Data; // Pointer to shared draw data (you can use
		    // ImGui::GetDrawListSharedData() to get the one from
		    // current ImGui context)
	const char *_OwnerName; // Pointer to owner window's name for debugging
	ImDrawVert *_VtxWritePtr; // [Internal] point within VtxBuffer.Data
				  // after each add command (to avoid using the
	...
	ImDrawCmdHeader
	    _CmdHeader; // [Internal] template of active commands. Fields should
			// match those of CmdBuffer.back().
	ImDrawListSplitter
	    _Splitter; // [Internal] for channels api (note: prefer using your
		       // own persistent instance of ImDrawListSplitter!)
	...
	IMGUI_API void
	PushClipRect(ImVec2 clip_rect_min, ImVec2 clip_rect_max,
		     bool intersect_with_current_clip_rect =
			 false); // Render-level scissoring. This is passed down
				 // to your render function but not used for
				 // CPU-side coarse clipping. Prefer using
				 // higher-level ImGui::PushClipRect() to affect
	...
	//   defaulted to num_segments == 12. In future versions we will use
	//   textures to provide cheaper and higher-quality circles. Use
	//   AddNgon() and AddNgonFilled() functions if you need to guaranteed a
	...
	AddRect(const ImVec2 &p_min, const ImVec2 &p_max, ImU32 col,
		...
		float thickness =
		    1.0f); // a: upper-left, b: lower-right (== upper-left +
	...
	IMGUI_API void AddQuad(const ImVec2 &p1, const ImVec2 &p2,
	...
	IMGUI_API void AddCircleFilled(const ImVec2 &center, float radius,
				       ImU32 col, int num_segments = 0);
	IMGUI_API void AddNgon(const ImVec2 &center, float radius, ImU32 col,
			       int num_segments, float thickness = 1.0f);
	IMGUI_API void AddNgonFilled(const ImVec2 &center, float radius,
				     ImU32 col, int num_segments);
	IMGUI_API void AddText(const ImVec2 &pos, ImU32 col,
			       const char *text_begin,
			       const char *text_end = NULL);
	IMGUI_API void AddText(const ImFont *font, float font_size,
			       const ImVec2 &pos, ImU32 col,
			       const char *text_begin,
			       const char *text_end = NULL,
			       float wrap_width = 0.0f,
			       const ImVec4 *cpu_fine_clip_rect = NULL);
	IMGUI_API void AddPolyline(const ImVec2 *points, int num_points,
				   ImU32 col, bool closed, float thickness);
	IMGUI_API void
	AddConvexPolyFilled(const ImVec2 *points, int num_points,
			    ImU32 col); // Note: Anti-aliased filling requires
					// points to be in clockwise order.
	IMGUI_API void AddBezierCurve(const ImVec2 &p1, const ImVec2 &p2,
				      const ImVec2 &p3, const ImVec2 &p4,
	...
	// - "p_min" and "p_max" represent the upper-left and lower-right
	// corners of the rectangle.
	// - "uv_min" and "uv_max" represent the normalized texture coordinates
	// to use for those corners. Using (0,0)->(1,1) texture coordinates will
	// generally display the entire texture.
	...
	IMGUI_API void AddImageQuad(ImTextureID user_texture_id,
				    ...
				    ImU32 col = IM_COL32_WHITE);
	IMGUI_API void AddImageRounded(
	    ImTextureID user_texture_id, const ImVec2 &p_min,
	...
	inline void PathFillConvex(ImU32 col)
	{
		...
		_Path.Size = 0;
	} // Note: Anti-aliased filling requires points to be in clockwise
	  // order.
	...
	IMGUI_API void PathArcTo(const ImVec2 &center, float radius,
	...
	IMGUI_API void PathBezierCurveTo(const ImVec2 &p2, const ImVec2 &p3,
					 const ImVec2 &p4,
					 int num_segments = 0);
	IMGUI_API void
	PathRect(const ImVec2 &rect_min, const ImVec2 &rect_max,
		 float rounding = 0.0f,
		 ImDrawCornerFlags rounding_corners = ImDrawCornerFlags_All);
	...
	IMGUI_API void
	AddDrawCmd(); // This is useful if you need to forcefully create a new
		      // draw call (to allow for dependent rendering /
	...
	// Advanced: Channels
	// - Use to split render into layers. By switching channels to can
	// render out-of-order (e.g. submit FG primitives before BG primitives)
	// - Use to minimize draw calls (e.g. if going back-and-forth between
	// multiple clipping rectangles, prefer to append into separate channels
	// then merge at the end)
	// - FIXME-OBSOLETE: This API shouldn't have been in ImDrawList in the
	// first place!
	//   Prefer using your own persistent instance of ImDrawListSplitter as
	//   you can stack them. Using the ImDrawList::ChannelsXXXX you cannot
	//   stack a split over another.
	...
	IMGUI_API void PrimRectUV(const ImVec2 &a, const ImVec2 &b,
				  ...
				  ImU32 col);
	IMGUI_API void PrimQuadUV(const ImVec2 &a, const ImVec2 &b,
				  const ImVec2 &c, const ImVec2 &d,
				  const ImVec2 &uv_a, const ImVec2 &uv_b,
				  const ImVec2 &uv_c, const ImVec2 &uv_d,
				  ImU32 col);
...
struct ImDrawData {
	bool Valid; // Only valid after Render() is called and before the next
		    // NewFrame() is called.
	ImDrawList *
	    *CmdLists; // Array of ImDrawList* to render. The ImDrawList are
		       // owned by ImGuiContext and only pointed to from here.
	int CmdListsCount;  // Number of ImDrawList* to render
	int TotalIdxCount;  // For convenience, sum of all ImDrawList's
			    // IdxBuffer.Size
	int TotalVtxCount;  // For convenience, sum of all ImDrawList's
			    // VtxBuffer.Size
	ImVec2 DisplayPos;  // Upper-left position of the viewport to render (==
			    // upper-left of the orthogonal projection matrix to
			    // use)
	ImVec2 DisplaySize; // Size of the viewport to render (== io.DisplaySize
			    // for the main viewport) (DisplayPos + DisplaySize
			    // == lower-right of the orthogonal projection
			    // matrix to use)
	ImVec2
	    FramebufferScale; // Amount of pixels for each unit of DisplaySize.
			      // Based on io.DisplayFramebufferScale. Generally
			      // (1,1) on normal display, (2,2) on OSX with
			      // Retina display.
	...
	DeIndexAllBuffers(); // Helper to convert all buffers from indexed to
			     ...
			     // Note: this is slow and most likely a waste of
			     // resources. Always prefer indexed rendering!
	IMGUI_API void ScaleClipRects(
	    const ImVec2
		&fb_scale); // Helper to scale the ClipRect field of each
			    // ImDrawCmd. Use if your final output buffer is at
			    // a different scale than Dear ImGui expects, or if
			    // there is a difference between your window
			    // resolution and framebuffer resolution.
};

//-----------------------------------------------------------------------------
// Font API (ImFontConfig, ImFontGlyph, ImFontAtlasFlags, ImFontAtlas,
...
struct ImFontConfig {
	...
	bool FontDataOwnedByAtlas; // true     // TTF/OTF data ownership taken
				   // by the container ImFontAtlas (will delete
				   // memory itself).
	int FontNo;	  // 0        // Index of font within TTF/OTF file
	float SizePixels; //          // Size in pixels for rasterizer (more or
			  //          less maps to the resulting font height).
	int OversampleH;  // 3        // Rasterize at higher quality for
			  // sub-pixel positioning. Read
			 // https://github.com/nothings/stb/blob/master/tests/oversample/README.md
			 // for details.
	int OversampleV; // 1        // Rasterize at higher quality for
			 // sub-pixel positioning. We don't use sub-pixel
			 // positions on the Y axis.
	bool PixelSnapH; // false    // Align every glyph to pixel boundary.
			 // Useful e.g. if you are merging a non-pixel aligned
			 // font with the default font. If enabled, you can set
			 // OversampleH/V to 1.
	ImVec2
	    GlyphExtraSpacing; // 0, 0     // Extra spacing (in pixels) between
			       // glyphs. Only X axis is supported for now.
	ImVec2
	    GlyphOffset; // 0, 0     // Offset all glyphs from this font input.
	const ImWchar
	    *GlyphRanges; // NULL     // Pointer to a user-provided list of
			  // Unicode range (2 value per range, values are
			  // inclusive, zero-terminated list). THE ARRAY DATA
			  // NEEDS TO PERSIST AS LONG AS THE FONT IS ALIVE.
	float GlyphMinAdvanceX; // 0        // Minimum AdvanceX for glyphs, set
				// Min to align font icons, set both Min/Max to
				// enforce mono-space font
	float GlyphMaxAdvanceX; // FLT_MAX  // Maximum AdvanceX for glyphs
	bool MergeMode; // false    // Merge into previous ImFont, so you can
			// combine multiple inputs font into one ImFont (e.g.
			// ASCII font + icons + Japanese glyphs). You may want
			// to use GlyphOffset.y when merge font of different
			// heights.
	unsigned int RasterizerFlags; // 0x00     // Settings for custom font
				      // rasterizer (e.g. ImGuiFreeType). Leave
				      // as zero if you aren't using one.
	float RasterizerMultiply;     // 1.0f     // Brighten (>1.0f) or darken
				      // (<1.0f) font output. Brightening small
				  // fonts may be a good workaround to make them
				  // more readable.
	ImWchar
	    EllipsisChar; // -1       // Explicitly specify unicode codepoint of
...
struct ImFontGlyphRangesBuilder {
	...
	AddText(const char *text,
		const char *text_end = NULL); // Add string (each character of
					      // the UTF-8 string are added)
	IMGUI_API void
...
struct ImFontAtlasCustomRect {
	...
	float GlyphAdvanceX; // Input    // For custom font glyphs only: glyph
			     // xadvance
...
enum ImFontAtlasFlags_ {
	...
	ImFontAtlasFlags_NoBakedLines =
	    1 << 2 // Don't build thick line textures into the atlas (save a
		   // little texture memory). The AntiAliasedLinesUseTex
		   // features uses them, otherwise they will be rendered using
...
struct ImFontAtlas {
	...
	IMGUI_API ImFont *
	AddFontFromFileTTF(const char *filename, float size_pixels,
	...
	IMGUI_API ImFont *AddFontFromMemoryTTF(
		...
		NULL); // Note: Transfer ownership of 'ttf_data' to ImFontAtlas!
		       // Will be deleted after destruction of the atlas. Set
		       // font_cfg->FontDataOwnedByAtlas=false to keep ownership
		       // of your data and it won't be freed.
	IMGUI_API ImFont *AddFontFromMemoryCompressedTTF(
	    const void *compressed_font_data, int compressed_font_size,
	    float size_pixels, const ImFontConfig *font_cfg = NULL,
	    const ImWchar *glyph_ranges =
		NULL); // 'compressed_font_data' still owned by caller. Compress
	...
	IMGUI_API ImFont *AddFontFromMemoryCompressedBase85TTF(
		...
		NULL); // 'compressed_font_data_base85' still owned by caller.
		       ...
		       // parameter.
	IMGUI_API void
	ClearInputData(); // Clear input data (all ImFontConfig structures
			  // including sizes, TTF data, glyph ranges, etc.) =
			  // all the data used to build the texture and fonts.
	IMGUI_API void
	ClearTexData(); // Clear output texture data (CPU side). Saves RAM once
			// the texture has been copied to graphics memory.
	IMGUI_API void ClearFonts(); // Clear output font data (glyphs storage,
				     // UV coordinates).
	IMGUI_API void Clear();	     // Clear all input and output.

	// Build atlas, retrieve pixel data.
	// User is in charge of copying the pixels into graphics memory (e.g.
	// create a texture with your engine). Then store your texture handle
	// with SetTexID(). The pitch is always = Width * BytesPerPixels (1 or
	// 4) Building in RGBA32 format is provided for convenience and
	// compatibility, but note that unless you manually manipulate or copy
	// color data into the texture (e.g. when using the AddCustomRect***
	// api), then the RGB pixels emitted will always be white (~75% of
	// memory/bandwidth waste.
	IMGUI_API bool
	Build(); // Build pixels data. This is called automatically for you by
	...
	// values are inclusive, zero-terminated list) NB: Make sure that your
	// string are UTF-8 and NOT in your local code page. In C++11, you can
	// create UTF-8 string literal using the u8"Hello world" syntax. See FAQ
	// for details. NB: Consider using ImFontGlyphRangesBuilder to build
	...
	GetGlyphRangesChineseFull(); // Default + Half-Width + Japanese
				     // Hiragana/Katakana + full set of about
				     // 21000 CJK Unified Ideographs
	IMGUI_API const ImWchar *
	GetGlyphRangesChineseSimplifiedCommon(); // Default + Half-Width +
						 // Japanese Hiragana/Katakana +
						 // set of 2500 CJK Unified
						 // Ideographs for common
						 // simplified Chinese
	IMGUI_API const ImWchar *
	...
	// You can request arbitrary rectangles to be packed into the atlas, for
	// your own purposes. After calling Build(), you can query the rectangle
	// position and render your pixels. You can also request your rectangles
	// to be mapped as font glyph (given a font + Unicode point), so you can
	// render e.g. custom colorful icons and use them as regular glyphs.
	// Read docs/FONTS.md for more details about using colorful icons.
	// Note: this API may be redesigned later in order to support
	// multi-monitor varying DPI settings.
	IMGUI_API int AddCustomRectRegular(int width, int height);
	IMGUI_API int
	AddCustomRectFontGlyph(ImFont *font, ImWchar id, int width, int height,
			       float advance_x,
	...
	bool Locked; // Marked as Locked by ImGui::NewFrame() so attempt to
		     // modify the atlas will assert.
	ImFontAtlasFlags Flags; // Build flags (see ImFontAtlasFlags_)
	ImTextureID
	    TexID; // User data to refer to the texture once it has been
		   // uploaded to user's graphic systems. It is passed back to
		   // you during rendering via the ImDrawCmd structure.
	int TexDesiredWidth; // Texture width desired by user before Build().
			     // Must be a power-of-two. If have many glyphs your
			     // graphics API have texture size restrictions you
			     // may want to increase texture width to decrease
			     // height.
	int TexGlyphPadding; // Padding between glyphs within texture in pixels.
			     // Defaults to 1. If your rendering method doesn't
			     // rely on bilinear filtering you may set this to
			     // 0.

	// [Internal]
	// NB: Access texture data via GetTexData*() calls! Which will setup a
	// default font for you.
	unsigned char *TexPixelsAlpha8; // 1 component per pixel, each component
					// is unsigned 8-bit. Total size =
					// TexWidth * TexHeight
	unsigned int *TexPixelsRGBA32;	// 4 component per pixel, each component
					// is unsigned 8-bit. Total size =
	...
	ImVector<ImFontAtlasCustomRect>
	    CustomRects; // Rectangles for packing custom texture data into the
			 // atlas.
	...
	int PackIdMouseCursors; // Custom texture rectangle ID for white pixel
				// and mouse cursors
	int PackIdLines; // Custom texture rectangle ID for baked anti-aliased
			 // lines

#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
	typedef ImFontAtlasCustomRect CustomRect; // OBSOLETED in 1.72+
	typedef ImFontGlyphRangesBuilder
	    GlyphRangesBuilder; // OBSOLETED in 1.67+
#endif
};
...
struct ImFont {
	...
	ImVector<float>
	    IndexAdvanceX; // 12-16 // out //            // Sparse.
			   // Glyphs->AdvanceX in a directly indexable way
			   // (cache-friendly for CalcTextSize functions which
			   // only this this info, and are often bottleneck in
			   // large UI).
	float FallbackAdvanceX; // 4     // out // = FallbackGlyph->AdvanceX
	float FontSize;		// 4     // in  //            // Height of
			// characters/line, set during loading (don't change
			// after loading)

	// Members: Hot ~28/40 bytes (for CalcTextSize + render loop)
	ImVector<ImWchar> IndexLookup; // 12-16 // out //            // Sparse.
				       // Index glyphs by Unicode code-point.
	ImVector<ImFontGlyph>
	    Glyphs; // 12-16 // out //            // All glyphs.
	const ImFontGlyph
	    *FallbackGlyph; // 4-8   // out // = FindGlyph(FontFallbackChar)

	// Members: Cold ~32/40 bytes
	ImFontAtlas *ContainerAtlas;	// 4-8   // out //            // What we
					// has been loaded into
	const ImFontConfig *ConfigData; // 4-8   // in  //            // Pointer
					// within ContainerAtlas->ConfigData
	short ConfigDataCount; // 2     // in  // ~ 1        // Number of
			       // ImFontConfig involved in creating this font.
			       // Bigger than 1 when merging multiple font
			       // sources into one ImFont.
	ImWchar FallbackChar;  // 2     // in  // = '?'      // Replacement
			       // character if a glyph isn't found. Only set via
			       // SetFallbackChar()
	ImWchar EllipsisChar;  // 2     // out // = -1       // Character used
			       // for ellipsis rendering.
	bool DirtyLookupTables; // 1     // out //
	float Scale; // 4     // in  // = 1.f      // Base font scale,
		     // multiplied by the per-window font scale which you can
		     // adjust with SetWindowFontScale()
	float Ascent, Descent; // 4+4   // out //            // Ascent: distance
			       // from top to bottom of e.g. 'A' [0..FontSize]
	int MetricsTotalSurface; // 4     // out //            // Total surface
				 // in pixels to get an idea of the font
				 // rasterization/texture cost (not exact, we
				 // approximate the cost of padding between
				 // glyphs)
	ImU8 Used4kPagesMap[(IM_UNICODE_CODEPOINT_MAX + 1) / 4096 /
			    8]; // 2 bytes if ImWchar=ImWchar16, 34 bytes if
				// ImWchar==ImWchar32. Store 1-bit for each
	...
	// 'max_width' stops rendering after a certain width (could be turned
	// into a 2d size). FLT_MAX to disable. 'wrap_width' enable automatic
	// word-wrapping across multiple lines to fit into given width. 0.0f to
	// disable.
	IMGUI_API ImVec2
	CalcTextSizeA(float size, float max_width, float wrap_width,
	...
	IMGUI_API void AddGlyph(const ImFontConfig *src_cfg, ImWchar c,
				float x0, float y0, float x1, float y1,
				float u0, float v0, float u1, float v1,
				float advance_x);
	IMGUI_API void
	AddRemapChar(ImWchar dst, ImWchar src,
		     bool overwrite_dst =
			 true); // Makes 'dst' character/glyph points to 'src'
				// character/glyph. Currently needs to be called
				// AFTER fonts have been built.
	IMGUI_API void SetGlyphVisible(ImWchar c, bool visible);
	IMGUI_API void SetFallbackChar(ImWchar c);
	IMGUI_API bool IsGlyphRangeUnused(unsigned int c_begin,
					  unsigned int c_last);
};
