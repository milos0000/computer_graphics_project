...
// - TableUpdateLayout() [Internal]             automatically called by the
// FIRST call to TableNextRow() or TableHeadersRow(): lock all widths, columns
// positions, clipping rectangles
//    | TableSetupDrawChannels()                - setup ImDrawList channels
//    | TableUpdateBorders()                    - detect hovering columns for
//    resize, ahead of contents submission | TableDrawContextMenu() - draw
...
// About 'outer_size', its meaning needs to differ slightly depending of if we
// are using ScrollX/ScrollY flags:
//   X:
//   - outer_size.x < 0.0f  ->  right align from window/work-rect maximum x
//   edge.
...
//   - outer_size.y > 0.0f  ->  fixed child height. recommended when using
//   Scrolling on any axis. Y without scrolling, we output table directly in
//   parent window:
//   - outer_size.y < 0.0f  ->  bottom align (will auto extend, unless
//   NoHostExtendV is set)
//   - outer_size.y = 0.0f  ->  zero minimum height (will auto extend, unless
//   NoHostExtendV is set)
//   - outer_size.y > 0.0f  ->  minimum height (will auto extend, unless
//   NoHostExtendV is set)
// About 'inner_width':
//   With ScrollX:
...
// - For large numbers of rows, it is recommended you use ImGuiListClipper to
// only submit visible rows.
...
//  when TableNextColumn() / TableSetColumnIndex() return false, user can skip
...
//  ImDrawList output:    normal      dummy        dummy       -> [internal]
//  when using the dummy channel, ImDrawList submissions (if any) will be wasted
//  (because cliprect is zero-width anyway).
//
// - We need distinguish those cases because non-hidden columns that are clipped
// outside of scrolling bounds should still contribute their height to the row.
...
static const int TABLE_DRAW_CHANNEL_UNCLIPPED =
    2; // When using ImGuiTableFlags_NoClip
				     ...
				     ImGuiWindow *outer_window)
{
	...
	if ((flags & (ImGuiTableFlags_ColumnsWidthStretch |
		      ImGuiTableFlags_ColumnsWidthFixed)) == 0)
	...
	// Adjust flags: disable Resizable when using SameWidths (done above
	// enforcing BordersInnerV)
	...
	if ((flags &
		...
		// Inherit _NoSavedSettings from top-level window (child windows
		// always have _NoSavedSettings set)
...
ImGuiTable::~ImGuiTable() { IM_FREE(RawData); }
...
bool ImGui::BeginTable(const char *str_id, int columns_count,
		       ImGuiTableFlags flags, const ImVec2 &outer_size,
		       float inner_width)
{
...
bool ImGui::BeginTableEx(const char *name, ImGuiID id, int columns_count,
			 ImGuiTableFlags flags, const ImVec2 &outer_size,
			 float inner_width)
{
	ImGuiContext &g = *GImGui;
	...
	if (outer_window
		->SkipItems) // Consistent with other tables + beneficial side
			     // effect that assert on miscalling EndTable() will
			     // be more visible.
		return false;

	// Sanity checks
	IM_ASSERT(columns_count > 0 &&
		  columns_count <= IMGUI_TABLE_MAX_COLUMNS &&
		  "Only 1..64 columns allowed!");
	if (flags & ImGuiTableFlags_ScrollX)
		IM_ASSERT(inner_width >= 0.0f);

	// If an outer size is specified ahead we will be able to early out when
	// not visible. Exact clipping rules may evolve.
	const bool use_child_window =
	    (flags & (ImGuiTableFlags_ScrollX | ImGuiTableFlags_ScrollY)) != 0;
	const ImVec2 avail_size = GetContentRegionAvail();
	ImVec2 actual_outer_size =
	    CalcItemSize(outer_size, ImMax(avail_size.x, 1.0f),
			 use_child_window ? ImMax(avail_size.y, 1.0f) : 0.0f);
	ImRect outer_rect(outer_window->DC.CursorPos,
	...
	const int instance_no = (table->LastFrameActive != g.FrameCount)
				    ? 0
	...
	if (instance_no > 0)
		IM_ASSERT(table->ColumnsCount == columns_count &&
			  "BeginTable(): Cannot change columns count mid-frame "
			  "while preserving same ID");
	...
	// contents.
	if (use_child_window) {
		// Ensure no vertical scrollbar appears if we only want
		// horizontal one, to make flag consistent (we have no other way
		// to disable vertical scrollbar of a window while keeping the
		// horizontal one showing)
		ImVec2 override_content_size(FLT_MAX, FLT_MAX);
		if ((flags & ImGuiTableFlags_ScrollX) &&
		    !(flags & ImGuiTableFlags_ScrollY))
			override_content_size.y = FLT_MIN;

		// Ensure specified width (when not specified, Stretched columns
		// will act as if the width == OuterWidth and never lead to any
		// scrolling). We don't handle inner_width < 0.0f, we could
		// potentially use it to right-align based on the right side of
		// the child window work rect, which would require knowing ahead
		// if we are going to have decoration taking horizontal spaces
		// (typically a vertical scrollbar).
		...
		if (override_content_size.x != FLT_MAX ||
			...
			SetNextWindowContentSize(
			    ImVec2(override_content_size.x != FLT_MAX
		...
		// Create scrolling region (without border and zero window
		// padding)
		ImGuiWindowFlags child_flags =
		    (flags & ImGuiTableFlags_ScrollX)
			? ImGuiWindowFlags_HorizontalScrollbar
			: ImGuiWindowFlags_None;
		BeginChildEx(name, instance_id, table->OuterRect.GetSize(),
			     false, child_flags);
	...
	// Push a standardized ID for both child-using and not-child-using
	// tables
	...
	table->HostBackupItemWidthStackSize =
	...
	const bool pad_outer_x =
	    (flags & ImGuiTableFlags_NoPadOuterX) ? false
	    : (flags & ImGuiTableFlags_PadOuterX)
		? true
		: (flags & ImGuiTableFlags_BordersOuterV) != 0;
	const bool pad_inner_x =
	    (flags & ImGuiTableFlags_NoPadInnerX) ? false : true;
	...
	const float inner_padding_explicit =
		...
		? g.Style.CellPadding.x
		: 0.0f;
	const float inner_spacing =
	    inner_spacing_for_border + inner_spacing_explicit;
	...
	table->OuterPaddingX =
	    (outer_padding_for_border + outer_padding_explicit) -
	    table->CellPaddingX;
	...
	table->InnerClipRect.Max.y = (flags & ImGuiTableFlags_NoHostExtendY)
					 ? ImMin(table->InnerClipRect.Max.y,
						 inner_window->WorkRect.Max.y)
					 : inner_window->ClipRect.Max.y;

	// Initial draw cmd starts with a BgClipRect that matches the one of its
	// host, to facilitate merge draw commands by default. This is because
	// all our cell highlight are manually clipped with BgClipRect Larger at
	// first, if/after unfreezing will become same as tight
	...
	table->BorderX1 =
		...
		.x; // +((table->Flags & ImGuiTableFlags_BordersOuter) ? 0.0f :
		    // -1.0f);
	table->BorderX2 =
	    table->InnerClipRect.Max
		.x; // +((table->Flags & ImGuiTableFlags_BordersOuter) ? 0.0f :
		    // +1.0f);
	...
	if (inner_window !=
	    outer_window) // So EndChild() within the inner window can restore
			  // the table properly.
		...
		for (int n = 0; n < columns_count; n++) {
			...
			*column = ImGuiTableColumn();
			...
			column->IsPreserveWidthAuto =
			    true; // Preserve WidthAuto when reinitializing a
				  // live table: not technically necessary but
				  // remove a visible flicker
	...
	// This is designed to facilitate DPI changes with the assumption that
	// e.g. style.CellPadding has been scaled as well. It will also react to
	// changing fonts with mixed results. It doesn't need to be perfect but
	// merely provide a decent transition.
	// FIXME-DPI: Provide consistent standards for reference size. Perhaps
	// using g.CurrentDpiScale would be more self explanatory. This is will
	// lead us to non-rounded WidthRequest in columns, which should work but
	// is a poorly tested path.
	const float new_ref_scale_unit =
	    g.FontSize; // g.Font->GetCharAdvance('A') ?
	if (table->RefScale != 0.0f && table->RefScale != new_ref_scale_unit) {
		const float scale_factor = new_ref_scale_unit / table->RefScale;
		// IMGUI_DEBUG_LOG("[table] %08X RefScaleUnit %.3f -> %.3f,
	...
	// Disable output until user calls TableNextRow() or TableNextColumn()
	// leading to the TableUpdateLayout() call.. This is not strictly
	// necessary but will reduce cases were "out of table" output will be
	// misleading to the user. Because we cannot safely assert in EndTable()
...
// + 2 * active_channels_count (for ImDrawCmd and ImDrawIdx buffers inside
...
void ImGui::TableBeginInitMemory(ImGuiTable *table, int columns_count)
{
	...
	span_allocator.ReserveBytes(0,
				    columns_count * sizeof(ImGuiTableColumn));
	span_allocator.ReserveBytes(1, columns_count *
					   sizeof(ImGuiTableColumnIdx));
	span_allocator.ReserveBytes(2,
				    columns_count * sizeof(ImGuiTableCellData));
...
void ImGui::TableBeginApplyRequests(ImGuiTable *table)
{
	...
	// FIXME-TABLE: Preserve contents width _while resizing down_ until
	// releasing.
	// FIXME-TABLE: Contains columns if our work area doesn't allow for
	// scrolling.
	if (table->InstanceCurrent == 0) {
		...
		// Process auto-fit for single stretch column, which is a
		// special case
		// FIXME-TABLE: Would be nice to redistribute available stretch
		// space accordingly to other weights, instead of giving it all
		// to siblings.
	...
	if (table->InstanceCurrent == 0) {
		...
		if (table->ReorderColumn != -1 &&
		    table->ReorderColumnDir != 0) {
			// We need to handle reordering across hidden columns.
			// In the configuration below, moving C to the right of
			// E will lead to:
			//    ... C [D] E  --->  ... [D] E  C   (Column
			//    name/index)
			...
			ImGuiTableColumn *src_column =
			    &table->Columns[table->ReorderColumn];
			ImGuiTableColumn *dst_column =
			    &table
				 ->Columns[(reorder_dir == -1)
					       ? src_column->PrevEnabledColumn
					       : src_column->NextEnabledColumn];
			IM_UNUSED(dst_column);
			const int src_order = src_column->DisplayOrder;
			const int dst_order = dst_column->DisplayOrder;
			src_column->DisplayOrder =
			    (ImGuiTableColumnIdx)dst_order;
			...
			IM_ASSERT(dst_column->DisplayOrder ==
				  dst_order - reorder_dir);

			// Display order is stored in both
			// columns->IndexDisplayOrder and table->DisplayOrder[],
			// rebuild the later from the former.
			for (int column_n = 0; column_n < table->ColumnsCount;
			     column_n++)
				table->DisplayOrderToIndex
				    [table->Columns[column_n].DisplayOrder] =
...
// extending
static ImGuiTableColumnFlags TableFixColumnFlags(ImGuiTable *table,
						 ImGuiTableColumnFlags flags)
{
	// Sizing Policy
	if ((flags & ImGuiTableColumnFlags_WidthMask_) == 0) {
		// FIXME-TABLE: Inconsistent to promote columns to
		// WidthAutoResize
	...
	IM_ASSERT(ImIsPowerOfTwo(
	    flags & ImGuiTableColumnFlags_WidthMask_)); // Check that only 1 of
							// each set is used.
	...
	// IM_ASSERT(ImIsPowerOfTwo(flags & ImGuiTableColumnFlags_AlignMask_));
...
// Layout columns for the frame. This is in essence the followup to
// BeginTable(). Runs on the first call to TableNextRow(), to give a chance for
// TableSetupColumn() to be called first.
// FIXME-TABLE: Our width (and therefore our WorkRect) will be minimal in the
// first frame for WidthAutoResize columns, increase feedback side-effect with
// widgets relying on WorkRect.Max.x. Maybe provide a default distribution for
// WidthAutoResize columns?
void ImGui::TableUpdateLayout(ImGuiTable *table)
{
	...
	// Process columns in their visible orders as we are building the
	// Prev/Next indices.
	...
	for (int order_n = 0; order_n < table->ColumnsCount; order_n++) {
		...
		if (column->SortOrder > 0 &&
		    !(table->Flags & ImGuiTableFlags_MultiSortable))
			table->IsSortSpecsDirty = true;
		...
		if (column->IsEnabled) {
			column->PrevEnabledColumn =
			    (ImGuiTableColumnIdx)last_visible_column_idx;
			column->NextEnabledColumn = -1;
			if (last_visible_column_idx != -1)
				table->Columns[last_visible_column_idx]
				    .NextEnabledColumn =
				    (ImGuiTableColumnIdx)column_n;
			column->IndexWithinEnabledSet =
		...
		IM_ASSERT(column->IndexWithinEnabledSet <=
			  column->DisplayOrder);
	}
	table->RightMostEnabledColumn =
	    (ImGuiTableColumnIdx)last_visible_column_idx;

	// [Part 2] Disable child window clipping while fitting columns. This is
	// not strictly necessary but makes it possible to avoid the column
	// fitting to wait until the first visible frame of the child container
	// (may or not be a good thing).
	// FIXME-TABLE: for always auto-resizing columns may not want to do that
	// all the time.
	...
	// [Part 3] Fix column flags. Calculate ideal width for columns. Count
	// how many fixed/stretch columns we have and sum of weights.
	const float min_column_width = TableGetMinColumnWidth();
	const float min_column_distance =
	    min_column_width + table->CellPaddingX * 2.0f +
	    table->CellSpacingX1 + table->CellSpacingX2;
	int count_fixed =
	    0; // Number of columns that have fixed sizing policy (not stretched
	       // sizing policy) (this is NOT the opposite of count_resizable!)
	int count_resizable = 0; // Number of columns the user can resize (this
				 // is NOT the opposite of count_fixed!)
	float sum_weights_stretched =
	    0.0f; // Sum of all weights for weighted columns.
	float sum_width_fixed_requests =
	    0.0f; // Sum of all width for fixed and auto-resize columns,
		  // excluding width contributed by Stretch columns.
	float max_width_auto =
	    0.0f; // Largest auto-width (used for SameWidths feature)
	...
	for (int column_n = 0; column_n < table->ColumnsCount; column_n++) {
		...
		// Adjust flags: default width mode + weighted columns are not
		// allowed when auto extending
		// FIXME-TABLE: Clarify why we need to do this again here and
		// not just in TableSetupColumn()
		column->Flags =
		    TableFixColumnFlags(table, column->FlagsIn) |
		...
		// We have a unusual edge case where if the user doesn't call
		// TableGetSortSpecs() but has sorting enabled or varying
		// sorting flags, we still want the sorting arrows to honor
		// those flags.
		if (table->Flags & ImGuiTableFlags_Sortable)
			TableFixColumnSortDirection(column);

		// Calculate ideal/auto column width (that's the width required
		// for all contents to be visible without clipping) Combine
		// width from regular rows + width from headers unless requested
		// not to.
		if (!column->IsPreserveWidthAuto) {
			const float content_width_body =
			    (float)ImMax(column->ContentMaxXFrozen,
					 column->ContentMaxXUnfrozen) -
			    column->WorkMinX;
			const float content_width_headers =
			    (float)column->ContentMaxXHeadersIdeal -
			    column->WorkMinX;
			float width_auto = content_width_body;
			if (!(table->Flags & ImGuiTableFlags_NoHeadersWidth) &&
			    !(column->Flags &
			      ImGuiTableColumnFlags_NoHeaderWidth))
				width_auto =
				    ImMax(width_auto, content_width_headers);
			width_auto = ImMax(width_auto, min_column_width);

			// Non-resizable columns also submit their requested
			// width
			if ((column->Flags &
			     ImGuiTableColumnFlags_WidthFixed) &&
		...
		if (column->Flags & (ImGuiTableColumnFlags_WidthFixed |
			...
			// Latch initial size for fixed columns and update it
			// constantly for auto-resizing column (unless clipped!)
			if ((column->AutoFitQueue != 0x00) ||
			    ((column->Flags &
			      ImGuiTableColumnFlags_WidthAutoResize) &&
			     column->IsVisibleX))
			...
			// FIXME-TABLE: Increase minimum size during init frame
			// to avoid biasing auto-fitting widgets (e.g.
			// TextWrapped) too much. Otherwise what tends to happen
			// is that TextWrapped would output a very large height
			// (= first frame scrollbar display very off + clipper
			// would skip lots of items). This is merely making the
			// side-effect less extreme, but doesn't properly fixes
			// it.
			// FIXME: Move this to ->WidthGiven to avoid temporary
			// lossyless?
			if (column->AutoFitQueue > 0x01 &&
			    table->IsInitializing)
				column->WidthRequest =
				    ImMax(column->WidthRequest,
					  min_column_width *
					      4.0f); // FIXME-TABLE: Another
						     // constant/scale?
			count_fixed += 1;
			sum_width_fixed_requests += column->WidthRequest;
		} else {
			IM_ASSERT(column->Flags &
	...
	// - When all columns are fixed or columns are of mixed type: use the
	// maximum auto width.
	// - When all columns are stretch: use same weight.
	const bool mixed_same_widths =
	    (table->Flags & ImGuiTableFlags_SameWidths) && count_fixed > 0;
	if (table->Flags & ImGuiTableFlags_SameWidths) {
		for (int column_n = 0; column_n < table->ColumnsCount;
		     column_n++) {
			if (!(table->EnabledMaskByIndex &
			      ((ImU64)1 << column_n)))
				continue;
			ImGuiTableColumn *column = &table->Columns[column_n];
			if (column->Flags &
			    (ImGuiTableColumnFlags_WidthFixed |
			     ImGuiTableColumnFlags_WidthAutoResize)) {
				sum_width_fixed_requests +=
				    max_width_auto -
				    column->WidthRequest; // Update old sum
				column->WidthRequest = max_width_auto;
			} else {
				sum_weights_stretched +=
				    1.0f -
				    column->StretchWeight; // Update old sum
				column->StretchWeight = 1.0f;
				if (count_fixed > 0)
					column->WidthRequest = max_width_auto;
			}
	...
	const float width_avail = ((table->Flags & ImGuiTableFlags_ScrollX) &&
	...
	const float width_avail_for_stretched_columns =
	    mixed_same_widths
		? 0.0f
		: width_avail - width_spacings - sum_width_fixed_requests;
	...
	for (int column_n = 0; column_n < table->ColumnsCount; column_n++) {
		...
		if (column->Flags & ImGuiTableColumnFlags_WidthStretch) {
			...
			if (!mixed_same_widths) {
				float weight_ratio = column->StretchWeight /
						     sum_weights_stretched;
				column->WidthRequest = IM_FLOOR(
				    ImMax(width_avail_for_stretched_columns *
					      weight_ratio,
					  min_column_width) +
				    0.01f);
			...
			// [Resize Rule 2] Resizing from right-side of a stretch
			// column preceding a fixed column needs to forward
			// resizing to left-side of fixed column. We also need
			// to copy the NoResize flag..
			if (column->NextEnabledColumn != -1)
				if (ImGuiTableColumn *next_column =
					&table->Columns
					     [column->NextEnabledColumn])
					if (next_column->Flags &
					    ImGuiTableColumnFlags_WidthFixed)
						column->Flags |=
						    (next_column->Flags &
		...
		// [Resize Rule 1] The right-most Visible column is not
		// resizable if there is at least one Stretch column (see
		// comments in TableResizeColumn())
		if (column->NextEnabledColumn == -1 &&
		    table->LeftMostStretchedColumnDisplayOrder != -1)
			column->Flags |= ImGuiTableColumnFlags_NoDirectResize_;

		// Assign final width, record width in case we will need to
		// shrink
		column->WidthGiven =
		    ImFloor(ImMax(column->WidthRequest, min_column_width));
		table->ColumnsTotalWidth +=
		    column->WidthGiven + table->CellPaddingX * 2.0f;
		table->ColumnsAutoFitWidth +=
		    column->WidthAuto + table->CellPaddingX * 2.0f;
	}

	// [Part 6] Redistribute stretch remainder width due to rounding
	// (remainder width is < 1.0f * number of Stretch column). Using
	// right-to-left distribution (more likely to match resizing cursor).
	if (width_remaining_for_stretched_columns >= 1.0f &&
		...
		for (int order_n = table->ColumnsCount - 1;
		     sum_weights_stretched > 0.0f &&
		     width_remaining_for_stretched_columns >= 1.0f &&
		     order_n >= 0;
		     order_n--) {
			if (!(table->EnabledMaskByDisplayOrder &
			      ((ImU64)1 << order_n)))
				continue;
			ImGuiTableColumn *column =
			    &table
				 ->Columns[table->DisplayOrderToIndex[order_n]];
			if (!(column->Flags &
			      ImGuiTableColumnFlags_WidthStretch))
				continue;
	...
	const ImRect mouse_hit_rect(
	    ...
	    ImMax(table->OuterRect.Max.y,
		  table->OuterRect.Min.y + table->LastOuterHeight));
	const bool is_hovering_table = ItemHoverable(mouse_hit_rect, 0);

	// [Part 7] Setup final position, offset, skip/clip states and clipping
	// rectangles, detect hovered column Process columns in their visible
	// orders as we are comparing the visible order and adjusting
	// host_clip_rect while looping.
	int visible_n = 0;
	float offset_x =
	    ((table->FreezeColumnsCount > 0) ? table->OuterRect.Min.x
					     : work_rect.Min.x) +
	...
	for (int order_n = 0; order_n < table->ColumnsCount; order_n++) {
		...
		column->NavLayerCurrent =
		    (ImS8)((table->FreezeRowsCount > 0 ||
			    column_n < table->FreezeColumnsCount)
			       ? ImGuiNavLayer_Menu
			       : ImGuiNavLayer_Main);

		if (table->FreezeColumnsCount > 0 &&
		    table->FreezeColumnsCount == visible_n)
		...
		if ((table->EnabledMaskByDisplayOrder &
		     ((ImU64)1 << order_n)) == 0) {
			// Hidden column: clear a few fields and we are done
			// with it for the remainder of the function. We set a
			// zero-width clip rect but set Min.y/Max.y properly to
			// not interfere with the clipper.
			...
			column->IsVisibleX = column->IsVisibleY =
		...
		if (is_hovering_table &&
		    g.IO.MousePos.x >= column->ClipRect.Min.x &&
		...
		if (table->Flags & ImGuiTableFlags_ScrollX) {
			// Frozen columns can't reach beyond visible width else
			// scrolling will naturally break.
			if (order_n < table->FreezeColumnsRequest) {
				max_width =
				    (table->InnerClipRect.Max.x -
				     (table->FreezeColumnsRequest - order_n) *
					 min_column_distance) -
				    offset_x;
				max_width = max_width - table->OuterPaddingX -
					    table->CellPaddingX -
					    table->CellSpacingX2;
			}
		} else if ((table->Flags &
			    ImGuiTableFlags_NoKeepColumnsVisible) == 0) {
			// If horizontal scrolling if disabled, we apply a final
			// lossless shrinking of columns in order to make sure
			// they are all visible. Because of this we also know
			// that all of the columns will always fit in
			// table->WorkRect and therefore in table->InnerRect
			// (because ScrollX is off)
			// FIXME-TABLE: This is solved incorrectly but also
			// quite a difficult problem to fix as we also want
			// ClipRect width to match. See "table_width_distrib"
			// and "table_width_keep_visible" tests
		...
		// Minimum width
		column->WidthGiven =
		    ImMax(column->WidthGiven,
			  ImMin(column->WidthRequest, min_column_width));

		// Lock all our positions
		// - ClipRect.Min.x: Because merging draw commands doesn't
		// compare min boundaries, we make ClipRect.Min.x match left
		// bounds to be consistent regardless of merging.
		// - ClipRect.Max.x: using WorkMaxX instead of MaxX (aka
		// including padding) is detrimental to visibility in very-small
		// column.
		// - FIXME-TABLE: We want equal width columns to have equal
		// (ClipRect.Max.x - WorkMinX) width, which means ClipRect.max.x
		// cannot stray off host_clip_rect.Max.x else right-most column
		// may appear shorter.
		...
		// Note that scrolling tables (where inner_window !=
		// outer_window) handle Y clipped earlier in BeginTable() so
		// IsVisibleY really only applies to non-scrolling tables.
		// FIXME-TABLE: Because InnerClipRect.Max.y is conservatively
		// ==outer_window->ClipRect.Max.y, we never can mark columns
		// _Above_ the scroll line as not IsVisibleY. Taking advantage
		// of LastOuterHeight would yield good results there...
		// FIXME-TABLE: IsVisible == false is disabled because it
		// effectively means not submitting will reduces contents width
		// which is fed to outer_window->DC.CursorMaxPos.x, and this may
		// be used (e.g. typically by outer_window using
		// AlwaysAutoResize or outer_window's horizontal scrollbar, but
		// could be something else). Possible solution to preserve last
		// known content width for clipped column. Test
		// 'table_reported_size' fails when enabling Y clipping and
		...
		// non-resizable sets are auto-fitting at all times and
		// therefore always request output.
		column->IsRequestOutput = is_visible ||
					  column->AutoFitQueue != 0 ||
					  column->CannotSkipItemsQueue != 0;
		if (column->IsRequestOutput)
			table->RequestOutputMaskByIndex |=
			    ((ImU64)1 << column_n);

		// Mark column as SkipItems (ignoring all items/layout)
		column->IsSkipItems =
		    !column->IsEnabled || table->HostSkipItems;
		// if (!is_visible && (column->Flags &
		// ImGuiTableColumnFlags_AutoCull))
		//     if ((column->AutoFitQueue & 1) == 0 &&
		...
		// Alignment
		// FIXME-TABLE: This align based on the whole column width, not
		// per-cell, and therefore isn't useful in many cases (to be
		// able to honor this we might be able to store a log of cells
		// width, per row, for visible rows, but nav/programmatic scroll
		// would have visible artifacts.)
		// if (column->Flags & ImGuiTableColumnFlags_AlignRight)
		//    column->WorkMinX = ImMax(column->WorkMinX, column->MaxX -
		//    column->ContentWidthRowsUnfrozen);
		// else if (column->Flags & ImGuiTableColumnFlags_AlignCenter)
		//    column->WorkMinX = ImLerp(column->WorkMinX,
		//    ImMax(column->StartX, column->MaxX -
		...
		// Don't decrement auto-fit counters until container window got
		// a chance to submit its items
	...
	// [Part 8] Detect/store when we are hovering the unused space after the
	// right-most column (so e.g. context menus can react on it) Clear
	// Resizable flag if none of our column are actually resizable (either
	// via an explicit _NoResize flag, either because of using
	// _WidthAutoResize/_WidthStretch). This will hide the resizing option
	...
	if (is_hovering_table && table->HoveredColumnBody == -1) {
		...
		if (g.IO.MousePos.x >= unused_x1)
			table->HoveredColumnBody =
			    (ImGuiTableColumnIdx)table->ColumnsCount;
	}
	...
	if (table->IsContextPopupOpen &&
		...
		const ImGuiID context_menu_id =
		    ImHashStr("##ContextMenu", 0, table->ID);
	...
	// [Part 13] Sanitize and build sort specs before we have a change to
	// use them for display. This path will only be exercised when sort
	// specs are modified before header rows (e.g. init or visibility
	// change)
	if (table->IsSortSpecsDirty &&
	    (table->Flags & ImGuiTableFlags_Sortable))
	...
	if (table->Flags & ImGuiTableFlags_NoClip)
		...
		inner_window->DrawList->PushClipRect(inner_window->ClipRect.Min,
						     inner_window->ClipRect.Max,
						     false);
}

// Process hit-testing on resizing borders. Actual size change will be applied
// in EndTable()
// - Set table->HoveredColumnBorder with a short delay/timer to reduce feedback
// noise
// - Submit ahead of table contents and header, use
// ImGuiButtonFlags_AllowItemOverlap to prioritize widgets
...
void ImGui::TableUpdateBorders(ImGuiTable *table)
{
	...
	// At this point OuterRect height may be zero or under actual final
	// height, so we rely on temporal coherency and use the final height
	// from last frame. Because this is only affecting _interaction_ with
	// columns, it is not really problematic (whereas the actual visual will
	// be displayed in EndTable() and using the current frame height).
	// Actual columns highlight/render will be performed in EndTable() and
	// not be affected.
	const float hit_half_width = TABLE_RESIZE_SEPARATOR_HALF_THICKNESS;
	const float hit_y1 = table->OuterRect.Min.y;
	const float hit_y2_body =
	    ImMax(table->OuterRect.Max.y, hit_y1 + table->LastOuterHeight);
	...
	for (int order_n = 0; order_n < table->ColumnsCount; order_n++) {
		...
		// ImGuiTableFlags_NoBordersInBodyUntilResize will be honored in
		// TableDrawBorders()
		const float border_y2_hit =
		    (table->Flags & ImGuiTableFlags_NoBordersInBody)
			? hit_y2_head
			: hit_y2_body;
		if ((table->Flags & ImGuiTableFlags_NoBordersInBody) &&
		    table->IsUsingHeaders == false)
			continue;

		ImGuiID column_id = TableGetColumnResizeID(
		    table, column_n, table->InstanceCurrent);
		ImRect hit_rect(column->MaxX - hit_half_width, hit_y1,
		...
		bool pressed =
		    ButtonBehavior(hit_rect, column_id, &hovered, &held,
...
void ImGui::EndTable()
{
	...
	// This assert would be very useful to catch a common error...
	// unfortunately it would probably trigger in some cases, and for
	// consistency user may sometimes output empty tables (and still benefit
	// from e.g. outer border)
	// IM_ASSERT(table->IsLayoutLocked && "Table unused: never called
	// TableNextRow(), is that the intent?");

	// If the user never got to call TableNextRow() or TableNextColumn(), we
	// call layout ourselves to ensure all our code paths are consistent
	// (instead of just hoping that TableBegin/TableEnd will work), get
	// borders drawn, etc.
	...
	IM_ASSERT(outer_window == inner_window ||
	...
	if (inner_window != outer_window) {
		table->OuterRect.Max.y =
		    ImMax(table->OuterRect.Max.y,
			  inner_window->Pos.y + inner_window->Size.y);
		inner_window->DC.CursorMaxPos.y = table->RowPosY2;
	} else if (!(flags & ImGuiTableFlags_NoHostExtendY)) {
		table->OuterRect.Max.y =
		    ImMax(table->OuterRect.Max.y, inner_window->DC.CursorPos.y);
		inner_window->DC.CursorMaxPos.y = table->RowPosY2;
	}
	table->WorkRect.Max.y =
	    ImMax(table->WorkRect.Max.y, table->OuterRect.Max.y);
	...
	// Store content width reference for each column (before attempting to
	// merge draw calls)
	...
	if (!(table->Flags & ImGuiTableFlags_ScrollX) &&
		...
		inner_window->Scroll.x = 0.0f;
		   ...
		   table->ResizedColumn == -1 && inner_window->ScrollbarX &&
		   table->InstanceInteracted == table->InstanceCurrent) {
		// When releasing a column being resized, scroll to keep the
		// resulting column in sight
		const float neighbor_width_to_keep_visible =
		    TableGetMinColumnWidth() + table->CellPaddingX * 2.0f;
		ImGuiTableColumn *column =
		    &table->Columns[table->LastResizedColumn];
	...
	if (table->ResizedColumn != -1 &&
	    table->InstanceCurrent == table->InstanceInteracted) {
		ImGuiTableColumn *column =
		    &table->Columns[table->ResizedColumn];
		const float new_x2 =
		    (g.IO.MousePos.x - g.ActiveIdClickOffset.x +
		     TABLE_RESIZE_SEPARATOR_HALF_THICKNESS);
		const float new_width =
		    ImFloor(new_x2 - column->MinX - table->CellSpacingX1 -
			    table->CellPaddingX * 2.0f);
		table->ResizedColumnNextWidth = new_width;
	}

	// Layout in outer window
	IM_ASSERT_USER_ERROR(inner_window->IDStack.back() ==
				 table->ID + table->InstanceCurrent,
	...
	// Override EndChild/ItemSize max extent with our own to enable
	// auto-resize on the X axis when possible
	// FIXME-TABLE: This can be improved (e.g. for Fixed columns we don't
	// want to auto AutoFitWidth? or propagate window auto-fit to table?)
	if (table->Flags & ImGuiTableFlags_ScrollX) {
		inner_window->DC.CursorMaxPos.x =
		    max_pos_x; // Set contents width for scrolling
		outer_window->DC.CursorMaxPos.x =
		    ImMax(backup_outer_max_pos_x,
			  backup_outer_cursor_pos_x + table->ColumnsTotalWidth +
			      inner_window->ScrollbarSizes.x); // For auto-fit
	} else {
		outer_window->DC.CursorMaxPos.x =
		    ImMax(backup_outer_max_pos_x,
			  table->WorkRect.Min.x +
			      table->ColumnsAutoFitWidth); // For auto-fit
	}
	...
	g.CurrentTable =
	    g.CurrentTableStack.Size
		? g.Tables.GetByIndex(g.CurrentTableStack.back().Index)
		: NULL;
...
static void TableUpdateColumnsWeightFromWidth(ImGuiTable *table)
{
	...
	for (int column_n = 0; column_n < table->ColumnsCount; column_n++) {
		...
		column->StretchWeight =
		    ((column->WidthRequest + 0.0f) / visible_width) *
		    visible_weight;
	}
}
...
void ImGui::TableSetColumnWidth(int column_n, float width)
{
	...
	if (!(table->Flags & ImGuiTableFlags_ScrollX))
		max_width_0 = (table->WorkRect.Max.x - column_0->MinX) -
			      (table->ColumnsEnabledCount -
			       (column_0->IndexWithinEnabledSet + 1)) *
				  min_width;
	...
	// In this surprisingly not simple because of how we support mixing
	// Fixed and Stretch columns. When forwarding resize from Wn| to Fn+1|
	// we need to be considerate of the _NoResize flag on Fn+1.
	// FIXME-TABLE: Find a way to rewrite all of this so interactions feel
	// more consistent for the user. Scenarios:
	// - F1 F2 F3  resize from F1| or F2|   --> ok: alter ->WidthRequested
	// of Fixed column. Subsequent columns will be offset.
	// - F1 F2 F3  resize from F3|          --> ok: alter ->WidthRequested
	// of Fixed column. If active, ScrollX extent can be altered.
	// - F1 F2 W3  resize from F1| or F2|   --> ok: alter ->WidthRequested
	// of Fixed column. If active, ScrollX extent can be altered, but it
	// doesn't make much sense as the Stretch column will always be minimal
	// size.
	// - F1 F2 W3  resize from W3|          --> ok: no-op (disabled by
	// Resize Rule 1)
	// - W1 W2 W3  resize from W1| or W2|   --> FIXME
	// - W1 W2 W3  resize from W3|          --> ok: no-op (disabled by
	// Resize Rule 1)
	// - W1 F2 F3  resize from F3|          --> ok: no-op (disabled by
	// Resize Rule 1)
	// - W1 F2     resize from F2|          --> ok: no-op (disabled by
	// Resize Rule 1)
	// - W1 W2 F3  resize from W1| or W2|   --> ok
	// - W1 F2 W3  resize from W1| or F2|   --> FIXME
	// - F1 W2 F3  resize from W2|          --> ok
	// - W1 F2 F3  resize from W1|          --> ok: equivalent to resizing
	// |F2. F3 will not move. (forwarded by Resize Rule 2)
	// - W1 F2 F3  resize from F2|          --> FIXME should resize F2, F3
	// and not have effect on W1 (Stretch columns are _before_ the Fixed
	// column).

	// Rules:
	// - [Resize Rule 1] Can't resize from right of right-most visible
	// column if there is any Stretch column. Implemented in
	// TableUpdateLayout().
	// - [Resize Rule 2] Resizing from right-side of a Stretch column before
	// a fixed column forward sizing to left-side of fixed column.
	// - [Resize Rule 3] If we are are followed by a fixed column and we
	// have a Stretch column before, we need to ensure that our left border
	// won't move.
	table->IsSettingsDirty = true;
	if (column_0->Flags & ImGuiTableColumnFlags_WidthFixed) {
		// [Resize Rule 3] If we are are followed by a fixed column and
		// we have a Stretch column before, we need to ensure that our
		// left border won't move, which we can do by making sure
		// column_a/column_b resizes cancels each others.
		if (column_1 &&
		    (column_1->Flags & ImGuiTableColumnFlags_WidthFixed))
			if (table->LeftMostStretchedColumnDisplayOrder != -1 &&
			    table->LeftMostStretchedColumnDisplayOrder <
				column_0->DisplayOrder) {
				// (old_a + old_b == new_a + new_b) --> (new_a
				// == old_a + old_b - new_b)
				float column_1_width =
				    ImMax(column_1->WidthRequest -
					      (column_0_width -
					       column_0->WidthRequest),
					  min_width);
				column_0_width = column_0->WidthRequest +
						 column_1->WidthRequest -
						 column_1_width;
		...
		if (column_1->Flags & ImGuiTableColumnFlags_WidthFixed) {
			...
			column_1->WidthRequest =
			    ImMax(min_width, column_1_width);
		} else {
			// (old_a + old_b == new_a + new_b) --> (new_a == old_a
			// + old_b - new_b)
			float column_1_width =
			    ImMax(column_1->WidthRequest -
				      (column_0_width - column_0->WidthRequest),
...
// - with ImGuiTableColumnFlags_WidthFixed,    width  <= 0 --> init width ==
// auto
// - with ImGuiTableColumnFlags_WidthFixed,    width  >  0 --> init width ==
// manual
// - with ImGuiTableColumnFlags_WidthStretch,  weight <  0 --> init weight
// == 1.0f
// - with ImGuiTableColumnFlags_WidthStretch,  weight >= 0 --> init weight ==
// custom Widths are specified _without_ CellPadding. So if you specify a width
// of 100.0f the column will be 100.0f+Padding*2.0f and you can fit a 100.0-wide
// item in it.
			     ...
			     float init_width_or_weight, ImGuiID user_id)
{
	...
	// (we write to FlagsIn which is a little misleading, another solution
	// would be to pass init_width_or_weight to TableFixColumnFlags)
	if ((flags & ImGuiTableColumnFlags_WidthMask_) == 0)
		if ((table->Flags & ImGuiTableFlags_ColumnsWidthFixed) &&
	...
	if (flags & ImGuiTableColumnFlags_WidthStretch)
		IM_ASSERT(init_width_or_weight != 0.0f &&
			  "Need to provide a valid weight!");
	column->InitStretchWeightOrWidth = init_width_or_weight;
	if (table->IsInitializing && column->WidthRequest < 0.0f &&
		...
		// Disable auto-fit if an explicit width/weight has been
		// specified
	...
	if (table->IsInitializing) {
		...
		if ((flags & ImGuiTableColumnFlags_DefaultHide) &&
		    (table->SettingsLoadedFlags & ImGuiTableFlags_Hideable) ==
			0)
			column->IsEnabled = column->IsEnabledNextFrame = false;
		if (flags & ImGuiTableColumnFlags_DefaultSort &&
		    (table->SettingsLoadedFlags & ImGuiTableFlags_Sortable) ==
			0) {
			column->SortOrder =
			    0; // Multiple columns using _DefaultSort will be
			       // reordered when building the sort specs.
...
void ImGui::TableSetupScrollFreeze(int columns, int rows)
{
	...
	table->FreezeColumnsRequest = (table->Flags & ImGuiTableFlags_ScrollX)
					  ? (ImGuiTableColumnIdx)columns
					  : 0;
	table->FreezeColumnsCount = (table->InnerWindow->Scroll.x != 0.0f)
					? table->FreezeColumnsRequest
					: 0;
	table->FreezeRowsRequest = (table->Flags & ImGuiTableFlags_ScrollY)
				       ? (ImGuiTableColumnIdx)rows
				       : 0;
...
void ImGui::TableNextRow(ImGuiTableRowFlags row_flags, float row_min_height)
{
	...
	// We honor min_row_height requested by user, but cannot guarantee
	// per-row maximum height, because that would essentially require a
	// unique clipping rectangle per-cell.
	table->RowPosY2 += table->CellPaddingY * 2.0f;
	table->RowPosY2 =
	    ImMax(table->RowPosY2, table->RowPosY1 + row_min_height);
...
void ImGui::TableBeginRow(ImGuiTable *table)
{
	...
	// Making the header BG color non-transparent will allow us to overlay
	// it multiple times when handling smooth dragging.
...
void ImGui::TableEndRow(ImGuiTable *table)
{
	...
	// TableBeginCell() will reposition the cursor to take account of
	// vertical padding.
	...
	if (is_visible) {
		...
		if ((bg_col0 | bg_col1 | border_col) != 0 ||
		    draw_strong_bottom_border || draw_cell_bg_color) {
			// In theory we could call
			// SetWindowClipRectBeforeSetChannel() but since we know
			// TableEndRow() is always followed by a change of
			// clipping rectangle we perform the smallest overwrite
			// possible here.
			if ((table->Flags & ImGuiTableFlags_NoClip) == 0)
				window->DrawList->_CmdHeader.ClipRect =
				    table->BgClipRectForDrawCmd.ToVec4();
			table->DrawSplitter.SetCurrentChannel(
			    window->DrawList, TABLE_DRAW_CHANNEL_BG0);
		}

		// Draw row background
		// We soft/cpu clip this so all backgrounds and borders can
		// share the same clipping rectangle
		if (bg_col0 || bg_col1) {
			ImRect row_rect(table->WorkRect.Min.x, bg_y1,
					table->WorkRect.Max.x, bg_y2);
			row_rect.ClipWith(table->BgClipRect);
			if (bg_col0 != 0 && row_rect.Min.y < row_rect.Max.y)
				window->DrawList->AddRectFilled(
				    row_rect.Min, row_rect.Max, bg_col0);
			if (bg_col1 != 0 && row_rect.Min.y < row_rect.Max.y)
				window->DrawList->AddRectFilled(
				    row_rect.Min, row_rect.Max, bg_col1);
		}
		...
		if (draw_cell_bg_color) {
			...
			for (ImGuiTableCellData *cell_data =
				 &table->RowCellData[0];
			     cell_data <= cell_data_end; cell_data++) {
				const ImGuiTableColumn *column =
				    &table->Columns[cell_data->Column];
				ImRect cell_bg_rect = TableGetCellBgRect(
				    table, cell_data->Column);
				cell_bg_rect.ClipWith(table->BgClipRect);
				cell_bg_rect.Min.x =
				    ImMax(cell_bg_rect.Min.x,
					  column->ClipRect.Min
					      .x); // So that first column after
						   // frozen one gets clipped
				cell_bg_rect.Max.x =
				    ImMin(cell_bg_rect.Max.x, column->MaxX);
				window->DrawList->AddRectFilled(
				    cell_bg_rect.Min, cell_bg_rect.Max,
				    cell_data->BgColor);
			}
		}

		// Draw top border
		if (border_col && bg_y1 >= table->BgClipRect.Min.y &&
		    bg_y1 < table->BgClipRect.Max.y)
			window->DrawList->AddLine(
			    ImVec2(table->BorderX1, bg_y1),
			    ImVec2(table->BorderX2, bg_y1), border_col,
			    border_size);
		...
		if (draw_strong_bottom_border &&
			...
			window->DrawList->AddLine(
			    ImVec2(table->BorderX1, bg_y2),
			    ImVec2(table->BorderX2, bg_y2),
			    table->BorderColorStrong, border_size);
	}

	// End frozen rows (when we are past the last frozen row line, teleport
	// cursor and alter clipping rectangle) We need to do that in
	// TableEndRow() instead of TableBeginRow() so the list clipper can mark
	...
	if (unfreeze_rows_request)
		for (int column_n = 0; column_n < table->ColumnsCount;
			...
			ImGuiTableColumn *column = &table->Columns[column_n];
			column->NavLayerCurrent =
			    (ImS8)((column_n < table->FreezeColumnsCount)
				       ? ImGuiNavLayer_Menu
				       : ImGuiNavLayer_Main);
		}
	if (unfreeze_rows_actual) {
		...
		// BgClipRect starts as table->InnerClipRect, reduce it now and
		// make BgClipRectForDrawCmd == BgClipRect
		float y0 =
		    ImMax(table->RowPosY2 + 1, window->InnerClipRect.Min.y);
		table->BgClipRect.Min.y = table->BgClipRectForDrawCmd.Min.y =
		    ImMin(y0, window->InnerClipRect.Max.y);
		table->BgClipRect.Max.y = table->BgClipRectForDrawCmd.Max.y =
		    window->InnerClipRect.Max.y;
		table->Bg1DrawChannelCurrent = table->Bg1DrawChannelUnfrozen;
		IM_ASSERT(table->BgClipRectForDrawCmd.Min.y <=
			  table->BgClipRectForDrawCmd.Max.y);

		float row_height = table->RowPosY2 - table->RowPosY1;
		table->RowPosY2 = window->DC.CursorPos.y =
		    table->WorkRect.Min.y + table->RowPosY2 -
		...
		for (int column_n = 0; column_n < table->ColumnsCount;
			...
			column->ClipRect.Min.y =
			    table->BgClipRectForDrawCmd.Min.y;
		}

		// Update cliprect ahead of TableBeginCell() so clipper can
		// access to new ClipRect->Min.y
...
void ImGui::TableBeginCell(ImGuiTable *table, int column_n)
{
	...
	if (column->Flags & ImGuiTableColumnFlags_IndentEnable)
		start_x +=
		    table->RowIndentOffsetX; // ~~ += window.DC.Indent.x -
					     // table->HostIndentX, except we
					     // locked it for the row.
	...
	if (!column->IsEnabled)
		window->DC.CursorPos.y =
		    ImMax(window->DC.CursorPos.y, table->RowPosY2);

	window->SkipItems = column->IsSkipItems;
	if (table->Flags & ImGuiTableFlags_NoClip) {
		// FIXME: if we end up drawing all borders/bg in EndTable, could
		// remove this and just assert that channel hasn't changed.
		table->DrawSplitter.SetCurrentChannel(
		    window->DrawList, TABLE_DRAW_CHANNEL_UNCLIPPED);
		// IM_ASSERT(table->DrawSplitter._Current ==
		// TABLE_DRAW_CHANNEL_UNCLIPPED);
	} else {
		// FIXME-TABLE: Could avoid this if draw channel is dummy
		// channel?
		SetWindowClipRectBeforeSetChannel(window, column->ClipRect);
		table->DrawSplitter.SetCurrentChannel(
		    window->DrawList, column->DrawChannelCurrent);
	}
}
...
void ImGui::TableEndCell(ImGuiTable *table)
{
	...
	if (table->RowFlags & ImGuiTableRowFlags_Headers)
		p_max_pos_x =
		    &column->ContentMaxXHeadersUsed; // Useful in case user
						     // submit contents in
						     // header row that is not a
						     // TableHeader() call
	else
		p_max_pos_x = table->IsUnfrozen ? &column->ContentMaxXUnfrozen
	...
	*p_max_pos_x = ImMax(*p_max_pos_x, window->DC.CursorMaxPos.x);
	...
	// FIXME-TABLE: Here we propagate text baseline from the last line of
	// the cell.. instead of the first one.
	table->RowTextBaseline =
	    ImMax(table->RowTextBaseline, window->DC.PrevLineTextBaseOffset);
}
...
bool ImGui::TableNextColumn()
{
	...
	if (table->IsInsideRow &&
	    table->CurrentColumn + 1 < table->ColumnsCount) {
		if (table->CurrentColumn != -1)
			TableEndCell(table);
	...
	// Return whether the column is visible. User may choose to skip
	// submitting items based on this return value, however they shouldn't
	// skip submitting for columns that may have the tallest contribution to
	// row height.
...
// Note: for row coloring we use ->RowBgColorCounter which is the same value
// without counting header rows
...
//   TableNextRow() and don't go below it.
// - Important: if ImGuiTableFlags_PadOuterX is set but
// ImGuiTableFlags_PadInnerX is not set, the outer-most left and right
...
ImGuiID ImGui::TableGetColumnResizeID(const ImGuiTable *table, int column_n,
				      int instance_no)
{
	IM_ASSERT(column_n < table->ColumnsCount);
	ImGuiID id =
	    table->ID + 1 + (instance_no * table->ColumnsCount) + column_n;
	return id;
}
...
// Return -1 when table is not hovered. return columns_count if the unused space
// at the right of visible columns is hovered.
...
void ImGui::TableSetBgColor(ImGuiTableBgTarget bg_target, ImU32 color,
			    int column_n)
{
	ImGuiContext &g = *GImGui;
	ImGuiTable *table = g.CurrentTable;
	...
	// We cannot draw neither the cell or row background immediately as we
	// don't know the row height at this point in time.
	switch (bg_target) {
	case ImGuiTableBgTarget_CellBg: {
		...
		if (table->RowCellDataCurrent < 0 ||
		    table->RowCellData[table->RowCellDataCurrent].Column !=
			column_n)
...
// PushTableBackground() is only used for
//   horizontal spanning. If we allowed vertical spanning we'd need one
//   background draw channel per merge group (1-4).
// Draw channel allocation (before merging):
// - NoClip                       --> 2+D+1 channels: bg0 + bg1 + foreground
// (same clip rect == 1 draw call) (FIXME-TABLE: could merge bg1 and
// foreground?)
// - Clip                         --> 2+D+N channels
...
void ImGui::TableSetupDrawChannels(ImGuiTable *table)
{
	...
	const int channels_for_row = (table->Flags & ImGuiTableFlags_NoClip)
					 ? 1
					 : table->ColumnsEnabledCount;
	...
	table->DummyDrawChannel =
	    (ImGuiTableDrawChannelIdx)((channels_for_dummy > 0)
					   ? channels_total - 1
					   : -1);
	table->Bg1DrawChannelCurrent = TABLE_DRAW_CHANNEL_BG1_FROZEN;
	table->Bg1DrawChannelUnfrozen =
	    (ImGuiTableDrawChannelIdx)((table->FreezeRowsCount > 0)
					   ? 2 + channels_for_row
					   : TABLE_DRAW_CHANNEL_BG1_FROZEN);
...
// groups will each have a ImDrawCmd using the same ClipRect. This function
...
//   Direct ImDrawList calls won't be taken into account by default, if you use
//   them make sure the ImGui:: bounds matches, by e.g. calling
//   SetCursorScreenPos().
...
void ImGui::TableMergeDrawChannels(ImGuiTable *table)
{
	...
	// Track which groups we are going to attempt to merge, and which
	// channels goes into each group.
	struct MergeGroup {
		ImRect ClipRect;
		int ChannelsCount;
	...
	for (int column_n = 0; column_n < table->ColumnsCount; column_n++) {
		...
		for (int merge_group_sub_n = 0;
			...
			// Don't attempt to merge if there are multiple draw
			// calls within the column
			ImDrawChannel *src_channel =
			    &splitter->_Channels[channel_no];
			...
			// Find out the width of this merge group and check if
			// it will fit in our column (note that we assume that
			// rendering didn't stray on the left direction. we
			// should need a CursorMinPos to detect it)
			...
			if (merge_group->ChannelsCount == 0)
				merge_group->ClipRect = ImRect(
				    +FLT_MAX, +FLT_MAX, -FLT_MAX, -FLT_MAX);
			merge_group->ChannelsMask.SetBit(channel_no);
			merge_group->ChannelsCount++;
			merge_group->ClipRect.Add(
			    src_channel->_CmdBuffer[0].ClipRect);
			merge_group_mask |= (1 << merge_group_n);
		}

		// Invalidate current draw channel
		// (we don't clear DrawChannelFrozen/DrawChannelUnfrozen solely
		// to facilitate debugging/later inspection of data)
	...
	if (merge_group_mask != 0) {
		// We skip channel 0 (Bg0) and 1 (Bg1 frozen) from the shuffling
		// since they won't move - see channels allocation in
		// TableSetupDrawChannels().
		const int LEADING_DRAW_CHANNELS = 2;
		g.DrawChannelsTempMergeBuffer.resize(
		    splitter->_Count -
		    LEADING_DRAW_CHANNELS); // Use shared temporary storage so
					    // the allocation gets amortized
		ImDrawChannel *dst_tmp = g.DrawChannelsTempMergeBuffer.Data;
		ImBitArray<IMGUI_TABLE_MAX_DRAW_CHANNELS>
		    remaining_mask; // We need 132-bit of storage
		remaining_mask.ClearBits();
		remaining_mask.SetBitRange(LEADING_DRAW_CHANNELS,
					   splitter->_Count - 1);
		...
		IM_ASSERT(has_freeze_v == false ||
			  table->Bg1DrawChannelUnfrozen !=
			      TABLE_DRAW_CHANNEL_BG1_FROZEN);
		int remaining_count =
		    splitter->_Count - (has_freeze_v ? LEADING_DRAW_CHANNELS + 1
						     : LEADING_DRAW_CHANNELS);
		// ImRect host_rect = (table->InnerWindow == table->OuterWindow)
		// ? table->InnerClipRect : table->HostClipRect;
		ImRect host_rect = table->HostClipRect;
		for (int merge_group_n = 0;
		     merge_group_n < IM_ARRAYSIZE(merge_groups);
		     merge_group_n++) {
			if (int merge_channels_count =
				merge_groups[merge_group_n].ChannelsCount) {
				MergeGroup *merge_group =
				    &merge_groups[merge_group_n];
				ImRect merge_clip_rect = merge_group->ClipRect;

				// Extend outer-most clip limits to match those
				// of host, so draw calls can be merged even if
				// outer-most columns have some outer padding
				// offsetting them from their parent ClipRect.
				// The principal cases this is dealing with are:
				// - On a same-window table (not scrolling =
				// single group), all fitting columns ClipRect
				// -> will extend and match host ClipRect ->
				// will merge
				// - Columns can use padding and have left-most
				// ClipRect.Min.x and right-most ClipRect.Max.x
				// != from host ClipRect -> will extend and
				// match host ClipRect -> will merge
				// FIXME-TABLE FIXME-WORKRECT: We are wasting a
				// merge opportunity on tables without scrolling
				// if column doesn't fit within host clip rect,
				// solely because of the half-padding difference
				// between window->WorkRect and
				// window->InnerClipRect.
				if ((merge_group_n & 1) == 0 || !has_freeze_h)
					merge_clip_rect.Min.x =
					    ImMin(merge_clip_rect.Min.x,
						  host_rect.Min.x);
				if ((merge_group_n & 2) == 0 || !has_freeze_v)
					merge_clip_rect.Min.y =
					    ImMin(merge_clip_rect.Min.y,
						  host_rect.Min.y);
				if ((merge_group_n & 1) != 0)
					merge_clip_rect.Max.x =
					    ImMax(merge_clip_rect.Max.x,
						  host_rect.Max.x);
				if ((merge_group_n & 2) != 0 &&
				    (table->Flags &
				     ImGuiTableFlags_NoHostExtendY) == 0)
					merge_clip_rect.Max.y =
					    ImMax(merge_clip_rect.Max.y,
						  host_rect.Max.y);
#if 0
				...
				for (int n = 0;
				     n < IM_ARRAYSIZE(remaining_mask.Storage);
				     n++)
					remaining_mask.Storage[n] &=
					    ~merge_group->ChannelsMask
						 .Storage[n];
				for (int n = 0; n < splitter->_Count &&
						merge_channels_count != 0;
				     n++) {
					// Copy + overwrite new clip rect
					if (!merge_group->ChannelsMask.TestBit(
						n))
						continue;
					...
					ImDrawChannel *channel =
					    &splitter->_Channels[n];
					IM_ASSERT(
					    channel->_CmdBuffer.Size == 1 &&
					    merge_clip_rect.Contains(
						ImRect(channel->_CmdBuffer[0]
							   .ClipRect)));
					channel->_CmdBuffer[0].ClipRect =
					    merge_clip_rect.ToVec4();
					memcpy(dst_tmp++, channel,
					       sizeof(ImDrawChannel));
				}
			}

			// Make sure Bg1DrawChannelUnfrozen appears in the
			// middle of our groups (whereas Bg0 and Bg1 frozen are
			// fixed to 0 and 1)
		...
		// Append unmergeable channels that we didn't reorder at the end
		// of the list
		for (int n = 0; n < splitter->_Count && remaining_count != 0;
		     n++) {
		...
		IM_ASSERT(dst_tmp == g.DrawChannelsTempMergeBuffer.Data +
...
void ImGui::TableDrawBorders(ImGuiTable *table)
{
	...
	if (inner_window->Hidden ||
	    !table->HostClipRect.Overlaps(table->InnerClipRect))
		return;
	...
	const float draw_y2_head =
		...
		? ((table->FreezeRowsCount >= 1 ? table->OuterRect.Min.y
	...
	if (table->Flags & ImGuiTableFlags_BordersInnerV) {
		for (int order_n = 0; order_n < table->ColumnsCount;
			...
			const bool is_hovered =
			    (table->HoveredColumnBorder == column_n);
			...
			const bool is_resizable =
			    (column->Flags &
			     (ImGuiTableColumnFlags_NoResize |
			      ImGuiTableColumnFlags_NoDirectResize_)) == 0;

			if (column->MaxX > table->InnerClipRect.Max.x &&
			    !is_resized) // && is_hovered)
				continue;
			if (column->NextEnabledColumn == -1 && !is_resizable)
				continue;
			if (column->MaxX <=
			    column->ClipRect.Min
				.x) // FIXME-TABLE FIXME-STYLE: Assume
				    // BorderSize==1, this is problematic if we
				    // want to increase the border size..
				continue;

			// Draw in outer window so right-most column won't be
			// clipped Always draw full height border when being
			// resized/hovered, or on the delimitation of frozen
			// column scrolling.
			ImU32 col;
			float draw_y2;
			if (is_hovered || is_resized ||
				...
				col =
				    ...
				    : is_hovered
					? GetColorU32(ImGuiCol_SeparatorHovered)
					: table->BorderColorStrong;
			} else {
				draw_y2 =
				    (table->Flags &
			...
			if (draw_y2 > draw_y1)
				inner_drawlist->AddLine(
				    ImVec2(column->MaxX, draw_y1),
				    ImVec2(column->MaxX, draw_y2), col,
				    border_size);
		}
	}
	...
	if (table->Flags & ImGuiTableFlags_BordersOuter) {
		// Display outer border offset by 1 which is a simple way to
		// display it without adding an extra draw call (Without the
		// offset, in outer_window it would be rendered behind cells,
		// because child windows are above their parent. In
		// inner_window, it won't reach out over scrollbars. Another
		// weird solution would be to display part of it in inner
		// window, and the part that's over scrollbars in the outer
		// window..) Either solution currently won't allow us to use a
		...
		if ((table->Flags & ImGuiTableFlags_BordersOuter) ==
			...
			outer_drawlist->AddRect(outer_border.Min,
						outer_border.Max, outer_col,
						0.0f, ~0, border_size);
		} else if (table->Flags & ImGuiTableFlags_BordersOuterV) {
			outer_drawlist->AddLine(
			    outer_border.Min,
			    ImVec2(outer_border.Min.x, outer_border.Max.y),
			    outer_col, border_size);
			outer_drawlist->AddLine(
			    ImVec2(outer_border.Max.x, outer_border.Min.y),
	...
	if ((table->Flags & ImGuiTableFlags_BordersInnerH) &&
		...
		if (border_y >= table->BgClipRect.Min.y &&
			...
			inner_drawlist->AddLine(
			    ImVec2(table->BorderX1, border_y),
...
// Return NULL if no sort specs (most often when ImGuiTableFlags_Sortable is not
// set) You can sort your data again when 'SpecsChanged == true'. It will be
// true with sorting specs have changed since last call, or the first time.
...
ImGuiTableSortSpecs *ImGui::TableGetSortSpecs()
{
	...
	// Require layout (in case TableHeadersRow() hasn't been called) as it
	// may alter IsSortSpecsDirty in some paths.
	if (!table->IsLayoutLocked)
		TableUpdateLayout(table);
					...
					bool append_to_sort_specs)
{
	...
	if (append_to_sort_specs)
		for (int other_column_n = 0;
		     other_column_n < table->ColumnsCount; other_column_n++)
			sort_order_max =
			    ImMax(sort_order_max,
...
void ImGui::TableSortSpecsSanitize(ImGuiTable *table)
{
	...
	const bool need_fix_linearize =
	    ((ImU64)1 << sort_order_count) != (sort_order_mask + 1);
	...
	if (need_fix_linearize || need_fix_single_sort_order) {
		...
		for (int sort_n = 0; sort_n < sort_order_count; sort_n++) {
			// Fix: Rewrite sort order fields if needed so they have
			// no gap or duplicate. (e.g. SortOrder 0 disappeared,
			// SortOrder 1..2 exists --> rewrite then as SortOrder
			// 0..1)
			int column_with_smallest_sort_order = -1;
			for (int column_n = 0; column_n < table->ColumnsCount;
			     column_n++)
				if ((fixed_mask &
				     ((ImU64)1 << (ImU64)column_n)) == 0 &&
				    table->Columns[column_n].SortOrder != -1)
					if (column_with_smallest_sort_order ==
						-1 ||
			...
			fixed_mask |=
			    ((ImU64)1 << column_with_smallest_sort_order);
			table->Columns[column_with_smallest_sort_order]
			    .SortOrder = (ImGuiTableColumnIdx)sort_n;

			// Fix: Make sure only one column has a SortOrder if
			// ImGuiTableFlags_MultiSortable is not set.
			if (need_fix_single_sort_order) {
				sort_order_count = 1;
				for (int column_n = 0;
				     column_n < table->ColumnsCount; column_n++)
					if (column_n !=
					    column_with_smallest_sort_order)
						table->Columns[column_n]
						    .SortOrder = -1;
				break;
			}
		}
	...
	// Fallback default sort order (if no column had the
	// ImGuiTableColumnFlags_DefaultSort flag)
...
void ImGui::TableSortSpecsBuild(ImGuiTable *table)
{
	...
	table->SortSpecsMulti.resize(single_sort_specs ? 0
						       : table->SortSpecsCount);
	ImGuiTableColumnSortSpecs *sort_specs =
	    single_sort_specs ? &table->SortSpecsSingle
...
float ImGui::TableGetHeaderRowHeight()
{
	...
	// Calculate row height, for the unlikely case that some labels may be
	// taller than others. If we didn't do that, uneven header height would
	// highlight but smaller one before the tallest wouldn't catch input for
	// all height. In your custom header row you may omit this all together
	// and just call TableNextRow() without a height...
	float row_height = GetTextLineHeight();
	int columns_count = TableGetColumnCount();
	for (int column_n = 0; column_n < columns_count; column_n++)
		if (TableGetColumnFlags(column_n) &
		    ImGuiTableColumnFlags_IsEnabled)
			row_height =
			    ImMax(row_height,
				  CalcTextSize(TableGetColumnName(column_n)).y);
...
void ImGui::TableHeadersRow()
{
	...
	if (table->HostSkipItems) // Merely an optimization, you may skip in
				  // your own code.
		return;
	...
	for (int column_n = 0; column_n < columns_count; column_n++) {
		...
		// Push an id to allow unnamed labels (generally accidental, but
		// let's behave nicely with them)
		// - in your own code you may omit the PushID/PopID
		// all-together, provided you know they won't collide
		// - table->InstanceCurrent is only >0 when we use multiple
		// BeginTable/EndTable calls with same identifier.
	...
	// Allow opening popup from the right-most section after the last
	// column.
	ImVec2 mouse_pos = ImGui::GetMousePos();
	if (IsMouseReleased(1) && TableGetHoveredColumn() == columns_count)
		if (mouse_pos.y >= row_y1 && mouse_pos.y < row_y1 + row_height)
			TableOpenContextMenu(
			    -1); // Will open a non-column-specific popup.
}
...
void ImGui::TableHeader(const char *label)
{
	...
	ImGuiTable *table = g.CurrentTable;
	IM_ASSERT(table != NULL &&
		  "Need to call TableHeader() after BeginTable()!");
	...
	// FIXME-TABLE-PADDING: Problem if the correct outer-padding CellBgRect
	// strays off our ClipRect
	ImRect cell_r = TableGetCellBgRect(table, column_n);
	float label_height = ImMax(
	    label_size.y, table->RowMinHeight - table->CellPaddingY * 2.0f);
	...
	ImRect bb(cell_r.Min.x, cell_r.Min.y, cell_r.Max.x,
		  ImMax(cell_r.Max.y, cell_r.Min.y + label_height +
					  g.Style.CellPadding.y * 2.0f));
	ItemSize(
	    ImVec2(0.0f, label_height)); // Don't declare unclipped width, it'll
					 // be fed ContentMaxPosHeadersIdeal
	if (!ItemAdd(bb, id))
		return;

	// GetForegroundDrawList()->AddRect(cell_r.Min, cell_r.Max,
	// IM_COL32(255, 0, 0, 255)); // [DEBUG]
	// GetForegroundDrawList()->AddRect(bb.Min, bb.Max, IM_COL32(255, 0, 0,
	// 255)); // [DEBUG]
	...
	// FIXME-TABLE: Scroll request while reordering a column and it lands
	// out of the scrolling zone.
	if (held && (table->Flags & ImGuiTableFlags_Reorderable) &&
	    IsMouseDragging(0) && !g.DragDropActive) {
		// While moving a column it will jump on the other side of the
		// mouse, so we also test for MouseDelta.x
		table->ReorderColumn = (ImGuiTableColumnIdx)column_n;
		table->InstanceInteracted = table->InstanceCurrent;

		// We don't reorder: through the frozen<>unfrozen line, or
		// through a column that is marked with
		// ImGuiTableColumnFlags_NoReorder.
		if (g.IO.MouseDelta.x < 0.0f && g.IO.MousePos.x < cell_r.Min.x)
			if (ImGuiTableColumn *prev_column =
	...
	if ((table->Flags & ImGuiTableFlags_Sortable) &&
		...
		w_arrow =
		    ImFloor(g.FontSize * ARROW_SCALE +
			    g.Style.FramePadding.x); // table->CellPadding.x);
		if (column->SortOrder != -1) {
			char sort_order_suf[8];
			w_sort_text = 0.0f;
			if (column->SortOrder > 0) {
				ImFormatString(sort_order_suf,
					       IM_ARRAYSIZE(sort_order_suf),
					       "%d", column->SortOrder + 1);
				w_sort_text = g.Style.ItemInnerSpacing.x +
					      CalcTextSize(sort_order_suf).x;
			}

			float x = ImMax(cell_r.Min.x,
					cell_r.Max.x - w_arrow - w_sort_text);
			...
			if (column->SortOrder > 0) {
				PushStyleColor(
				    ImGuiCol_Text,
				    GetColorU32(ImGuiCol_Text, 0.70f));
				RenderText(
				    ImVec2(x + g.Style.ItemInnerSpacing.x, y),
				    sort_order_suf);
				PopStyleColor();
				x += w_sort_text;
			}
			RenderArrow(window->DrawList, ImVec2(x, y), col,
				    column->SortDirection ==
		...
		if (pressed && table->ReorderColumn != column_n) {
			...
			// - If the PreferSortDescending flag is set, we will
			// default to a Descending direction on the first click.
			// - Note that the PreferSortAscending flag is never
			// checked, it is essentially the default and therefore
			// a no-op.
			ImGuiSortDirection sort_direction;
			if (column->SortOrder == -1)
				sort_direction =
				...
				sort_direction =
					...
					? ImGuiSortDirection_Descending
					: ImGuiSortDirection_Ascending;
			TableSetColumnSortDirection(column_n, sort_direction,
						    g.IO.KeyShift);
		}
	}

	// Render clipped label. Clipping here ensure that in the majority of
	// situations, all our header cells will be merged into a single draw
	// call.
	// window->DrawList->AddCircleFilled(ImVec2(ellipsis_max, label_pos.y),
	// 40, IM_COL32_WHITE);
	RenderTextEllipsis(window->DrawList, label_pos,
			   ImVec2(ellipsis_max, label_pos.y + label_height +
						    g.Style.FramePadding.y),
			   ellipsis_max, ellipsis_max, label, label_end,
			   &label_size);

	const bool text_clipped = label_size.x > (ellipsis_max - label_pos.x);
	if (text_clipped && hovered &&
	    g.HoveredIdNotActiveTimer > g.TooltipSlowDelay)
		SetTooltip("%.*s", (int)(label_end - label), label);

	// We feed our unclipped width to the column without writing on
	// CursorMaxPos, so that column is still considering for merging.
	float max_pos_x = label_pos.x + label_size.x + w_sort_text + w_arrow;
	column->ContentMaxXHeadersUsed =
	    ImMax(column->ContentMaxXHeadersUsed, column->WorkMaxX);
	column->ContentMaxXHeadersIdeal =
	    ImMax(column->ContentMaxXHeadersIdeal, max_pos_x);

	// We don't use BeginPopupContextItem() because we want the popup to
	// stay up even after the column is hidden
...
void ImGui::TableOpenContextMenu(int column_n)
{
	...
	if (column_n == -1 &&
	    table->CurrentColumn !=
		-1) // When called within a column automatically use this one
		    // (for consistency)
		column_n = table->CurrentColumn;
...
void ImGui::TableDrawContextMenu(ImGuiTable *table)
{
	...
	const int column_n = (table->ContextPopupColumn >= 0 &&
				 ...
				 : -1;
	ImGuiTableColumn *column =
	    (column_n != -1) ? &table->Columns[column_n] : NULL;
	...
	if (table->Flags & ImGuiTableFlags_Resizable) {
		if (column != NULL) {
			const bool can_resize =
			    ...
			    column->IsEnabled;
			if (MenuItem("Size column to fit", NULL, false,
				     can_resize))
				TableSetColumnWidthAutoSingle(table, column_n);
		}

		const char *size_all_desc;
		if (table->ColumnsEnabledFixedCount ==
		    table->ColumnsEnabledCount)
			size_all_desc =
			    "Size all columns to fit###SizeAll"; // All fixed
		else if (table->ColumnsEnabledFixedCount == 0)
			size_all_desc =
			    "Size all columns to default###SizeAll"; // All
								     // stretch
		else
			size_all_desc = "Size all columns to "
					"fit/default###SizeAll"; // Mixed
	...
	if (table->Flags & ImGuiTableFlags_Reorderable) {
		if (MenuItem("Reset order", NULL, false,
	...
	// Sorting
	// (modify TableOpenContextMenu() to add _Sortable flag if enabling
	// this)
#if 0
	...
	if (table->Flags & ImGuiTableFlags_Hideable) {
		...
		for (int other_column_n = 0;
			...
			ImGuiTableColumn *other_column =
			    &table->Columns[other_column_n];
			const char *name =
			    TableGetColumnName(table, other_column_n);
			if (name == NULL || name[0] == 0)
				name = "<Unknown>";

			// Make sure we can't hide the last active column
			bool menu_item_active =
			    (other_column->Flags & ImGuiTableColumnFlags_NoHide)
				? false
				: true;
			if (other_column->IsEnabled &&
			    table->ColumnsEnabledCount <= 1)
				menu_item_active = false;
			if (MenuItem(name, NULL, other_column->IsEnabled,
				     menu_item_active))
				other_column->IsEnabledNextFrame =
				    !other_column->IsEnabled;
		}
		PopItemFlag();
	}
}
...
{
	return sizeof(ImGuiTableSettings) +
	       (size_t)columns_count * sizeof(ImGuiTableColumnSettings);
}

ImGuiTableSettings *ImGui::TableSettingsCreate(ImGuiID id, int columns_count)
{
	ImGuiContext &g = *GImGui;
	ImGuiTableSettings *settings = g.SettingsTables.alloc_chunk(
...
ImGuiTableSettings *ImGui::TableGetBoundSettings(ImGuiTable *table)
{
	if (table->SettingsOffset != -1) {
		...
		if (settings->ColumnsCountMax >= table->ColumnsCount)
			return settings; // OK
		settings->ID = 0; // Invalidate storage, we won't fit because of
				  // a count change
	}
	return NULL;
}
...
void ImGui::TableResetSettings(ImGuiTable *table)
{
	...
	table->SettingsLoadedFlags =
	    ImGuiTableFlags_None; // Mark as nothing loaded so our initialized
...
void ImGui::TableSaveSettings(ImGuiTable *table)
{
	...
	// Serialize ImGuiTable/ImGuiTableColumn into
	// ImGuiTableSettings/ImGuiTableColumnSettings
	IM_ASSERT(settings->ID == table->ID);
	...
	for (int n = 0; n < table->ColumnsCount;
	     n++, column++, column_settings++) {
		...
		column_settings->IsStretch =
		    (column->Flags & ImGuiTableColumnFlags_WidthStretch) ? 1
									 : 0;
		if ((column->Flags & ImGuiTableColumnFlags_WidthStretch) == 0)
			save_ref_scale = true;

		// We skip saving some data in the .ini file when they are
		// unnecessary to restore our state. Note that fixed width where
		// initial width was derived from auto-fit will always be saved
		// as InitStretchWeightOrWidth will be 0.0f.
		// FIXME-TABLE: We don't have logic to easily compare SortOrder
		// to DefaultSortOrder yet so it's always saved when present.
		...
		if (column->IsEnabled !=
		    ((column->Flags & ImGuiTableColumnFlags_DefaultHide) == 0))
...
void ImGui::TableLoadSettings(ImGuiTable *table)
{
	...
	if (table->SettingsOffset == -1) {
		...
		if (settings->ColumnsCount !=
		    table->ColumnsCount) // Allow settings if columns count
					 // changed. We could otherwise decide
					 // to return...
			table->IsSettingsDirty = true;
		table->SettingsOffset =
		    g.SettingsTables.offset_from_ptr(settings);
					  ...
					  const char *line)
{
	...
	if (sscanf(line, "Column %d%n", &column_n, &r) == 1) {
		...
		ImGuiTableColumnSettings *column =
		    settings->GetColumnSettings() + column_n;
		...
		if (sscanf(line, "Sort=%d%c%n", &n, &c, &r) == 2) {
			...
			column->SortDirection =
			    (c == '^') ? ImGuiSortDirection_Descending
				       : ImGuiSortDirection_Ascending;
			settings->SaveFlags |= ImGuiTableFlags_Sortable;
		}
	}
					  ...
					  ImGuiTextBuffer *buf)
{
	...
	for (ImGuiTableSettings *settings = g.SettingsTables.begin();
		...
		// TableSaveSettings() may clear some of those flags when we
		// establish that the data can be stripped (e.g. Order was
		// unchanged)
...
void ImGui::DebugNodeTable(ImGuiTable *table)
{
	...
	const bool is_active =
	    (table->LastFrameActive >=
	     ImGui::GetFrameCount() -
		 2); // Note that fully clipped early out scrolling tables will
		     // appear as inactive here.
	ImFormatString(p, buf_end - p, "Table 0x%08X (%d columns, in '%s')%s",
	...
	BulletText("OuterRect: Pos: (%.1f,%.1f) Size: (%.1f,%.1f)",
		   table->OuterRect.Min.x, table->OuterRect.Min.y,
		   table->OuterRect.GetWidth(), table->OuterRect.GetHeight());
	BulletText("ColumnsWidth: %.1f, AutoFitWidth: %.1f, InnerWidth: %.1f%s",
		   table->ColumnsTotalWidth, table->ColumnsAutoFitWidth,
		   table->InnerWidth,
	...
	for (int n = 0; n < table->ColumnsCount; n++) {
		...
		ImFormatString(
		    ...
		    n, column->DisplayOrder, name,
		    column->MinX - table->WorkRect.Min.x,
		    column->MaxX - table->WorkRect.Min.x, column->IsEnabled,
		    column->IsVisibleX, column->IsVisibleY,
		    column->IsRequestOutput, column->IsSkipItems,
		    column->DrawChannelFrozen, column->DrawChannelUnfrozen,
		    column->WidthGiven, column->WidthRequest, column->WidthAuto,
		    column->StretchWeight, column->MinX, column->MaxX,
		    column->MaxX - column->MinX, column->ClipRect.Min.x,
			...
			? "WidthStretch "
			: "",
		    (column->Flags & ImGuiTableColumnFlags_WidthFixed)
			? "WidthFixed "
			: "",
		...
		Selectable(buf);
		if (IsItemHovered()) {
			ImRect r(column->MinX, table->OuterRect.Min.y,
				 column->MaxX, table->OuterRect.Max.y);
			GetForegroundDrawList()->AddRect(
			    r.Min, r.Max, IM_COL32(255, 255, 0, 255));
		}
	}
...
void ImGui::DebugNodeTableSettings(ImGuiTableSettings *settings)
{
	if (!TreeNode((void *)(intptr_t)settings->ID,
		      "Settings 0x%08X (%d columns)", settings->ID,
		      settings->ColumnsCount))
		return;
	...
	for (int n = 0; n < settings->ColumnsCount; n++) {
		...
		BulletText(
		    ...
		    "0x%08X",
		    n, column_settings->DisplayOrder,
...
// many times with the underlying ImDrawCmd. Instead, we do a preemptive
...
int ImGui::GetColumnIndex()
{
	...
	return window->DC.CurrentColumns ? window->DC.CurrentColumns->Current
					 : 0;
}
...
{
	// Active (dragged) column always follow mouse. The reason we need this
	// is that dragging a column to the right edge of an auto-resizing
	...
	x = ImMax(x, ImGui::GetColumnOffset(column_index - 1) +
			 g.Style.ColumnsMinSpacing);
	if ((columns->Flags & ImGuiOldColumnFlags_NoPreserveWidths))
		x = ImMin(x, ImGui::GetColumnOffset(column_index + 1) -
				 g.Style.ColumnsMinSpacing);
			      ...
			      bool before_resize = false)
{
	...
	if (before_resize)
		offset_norm =
		    columns->Columns[column_index + 1].OffsetNormBeforeResize -
...
float ImGui::GetColumnWidth(int column_index)
{
	...
	return GetColumnOffsetFromNorm(
	    columns, columns->Columns[column_index + 1].OffsetNorm -
			 columns->Columns[column_index].OffsetNorm);
}
...
void ImGui::SetColumnOffset(int column_index, float offset)
{
	...
	const bool preserve_width =
	    !(columns->Flags & ImGuiOldColumnFlags_NoPreserveWidths) &&
	    (column_index < columns->Count - 1);
	const float width = preserve_width
				? GetColumnWidthEx(columns, column_index,
	...
	if (preserve_width)
		SetColumnOffset(column_index + 1,
				offset +
				    ImMax(g.Style.ColumnsMinSpacing, width));
}
...
void ImGui::SetColumnWidth(int column_index, float width)
{
	...
	if (column_index < 0)
		column_index = columns->Current;
	SetColumnOffset(column_index + 1,
			GetColumnOffset(column_index) + width);
}
...
void ImGui::PopColumnsBackground()
{
	...
	columns->Splitter.SetCurrentChannel(window->DrawList,
					    columns->Current + 1);
}

ImGuiOldColumns *ImGui::FindOrCreateColumns(ImGuiWindow *window, ImGuiID id)
{
	// We have few columns per window so for now we don't need bother much
	// with turning this into a faster lookup.
...
ImGuiID ImGui::GetColumnsID(const char *str_id, int columns_count)
{
	...
	// users name their columns set the same as another widget. In addition,
	// when an identifier isn't explicitly provided we include the number of
	// columns in the hash to make it uniquer.
...
void ImGui::BeginColumns(const char *str_id, int columns_count,
			 ImGuiOldColumnFlags flags)
{
	...
	IM_ASSERT(window->DC.CurrentColumns ==
	...
	// We aim so that the right-most column will have the same clipping
	// width as other after being clipped by parent ClipRect
	const float column_padding = g.Style.ItemSpacing.x;
	const float half_clip_extend_x = ImFloor(
	    ImMax(window->WindowPadding.x * 0.5f, window->WindowBorderSize));
	const float max_1 =
	    window->WorkRect.Max.x + column_padding -
	...
	columns->OffMinX =
	    ...
	    ImMax(column_padding - window->WindowPadding.x, 0.0f);
	columns->OffMaxX =
	    ImMax(ImMin(max_1, max_2) - window->Pos.x, columns->OffMinX + 1.0f);
	...
	for (int n = 0; n < columns_count; n++) {
		...
		float clip_x2 =
		    IM_ROUND(window->Pos.x + GetColumnOffset(n + 1) - 1.0f);
	...
	// We don't generally store Indent.x inside ColumnsOffset because it may
	...
	window->DC.ColumnsOffset.x =
	    ImMax(column_padding - window->WindowPadding.x, 0.0f);
	window->DC.CursorPos.x = IM_FLOOR(window->Pos.x + window->DC.Indent.x +
					  window->DC.ColumnsOffset.x);
...
void ImGui::NextColumn()
{
	...
	// Optimization: avoid PopClipRect() + SetCurrentChannel() +
	// PushClipRect() (which would needlessly attempt to update commands in
	// the wrong channel, then pop or overwrite them),
	...
	// FIXME-COLUMNS: Share code with BeginColumns() - move code on columns
	// setup.
	float offset_0 = GetColumnOffset(columns->Current);
	float offset_1 = GetColumnOffset(columns->Current + 1);
	float width = offset_1 - offset_0;
...
void ImGui::EndColumns()
{
	...
	if (!(flags & ImGuiOldColumnFlags_GrowParentContentsSize))
		window->DC.CursorMaxPos.x =
		    columns->HostCursorMaxPosX; // Restore cursor max pos, as
						// columns don't grow parent

	// Draw columns borders and handle resize
	// The IsBeingResized flag ensure we preserve pre-resize columns width
	// so back-and-forth are not lossy
	bool is_being_resized = false;
	if (!(flags & ImGuiOldColumnFlags_NoBorder) && !window->SkipItems) {
		// We clip Y boundaries CPU side because very long triangles are
		// mishandled by some GPU drivers.
		const float y1 =
		    ImMax(columns->HostCursorPosY, window->ClipRect.Min.y);
		const float y2 =
		    ImMin(window->DC.CursorPos.y, window->ClipRect.Max.y);
		...
		for (int n = 1; n < columns->Count; n++) {
			...
			const ImRect column_hit_rect(
			    ImVec2(x - column_hit_hw, y1),
			...
			if (!(flags & ImGuiOldColumnFlags_NoResize)) {
				ButtonBehavior(column_hit_rect, column_id,
					       &hovered, &held);
				if (hovered || held)
					g.MouseCursor =
					    ImGuiMouseCursor_ResizeEW;
				if (held && !(column->Flags &
			...
			window->DrawList->AddLine(ImVec2(xi, y1 + 1.0f),
						  ImVec2(xi, y2), col);
		}

		// Apply dragging after drawing the column lines, so our
		// rendered lines are in sync with how items were displayed
		// during the frame.
		if (dragging_column != -1) {
			...
			float x =
			    GetDraggedColumnOffset(columns, dragging_column);
