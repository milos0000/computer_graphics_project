...
//  as void*/ImTextureID. Read the FAQ about ImTextureID! [x] Renderer: Desktop
//  GL only: Support for large meshes (64k+ vertices) with 16-bit indices.
...
bool ImGui_ImplOpenGL3_Init(const char *glsl_version)
{
	...
	if (major == 0 && minor == 0) {
		// Query GL_VERSION in desktop GL 2.x, the string will start
		// with "<major>.<minor>"
	...
	if (g_GlVersion >= 320)
		...
		// Store GLSL version string so we can refer to it later in case
		// we recreate shaders. Note: GLSL version is NOT the same as GL
		// version. Leave this to NULL if unsure.
#if defined(IMGUI_IMPL_OPENGL_ES2)
	...
	IM_ASSERT((int)strlen(glsl_version) + 2 <
		  IM_ARRAYSIZE(g_GlslVersionString));
	strcpy(g_GlslVersionString, glsl_version);
	strcat(g_GlslVersionString, "\n");

	// Debugging construct to make it easily visible in the IDE and debugger
	// which GL loader has been selected. The code actually never uses the
	// 'gl_loader' variable! It is only here so you can read it! If
	// auto-detection fails or doesn't select the same GL loader file as
	// used by your application, you are likely to get a crash below. You
	// can explicitly select a loader by using '#define
	...
	// IF YOU GET A CRASH HERE: it probably means that you haven't
	// initialized the OpenGL function loader used by this code. Desktop
	// OpenGL 3/4 need a function loader. See the
					       ...
					       GLuint vertex_array_object)
{
	...
	if (g_GlVersion >= 330)
		glBindSampler(
		    0, 0); // We use combined texture/sampler state.
...
void ImGui_ImplOpenGL3_RenderDrawData(ImDrawData *draw_data)
{
	...
	GLboolean last_enable_primitive_restart =
	    (g_GlVersion >= 310) ? glIsEnabled(GL_PRIMITIVE_RESTART) : GL_FALSE;
#endif
	...
	for (int n = 0; n < draw_data->CmdListsCount; n++) {
		...
		for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++) {
			...
			if (pcmd->UserCallback != NULL) {
				...
				// (ImDrawCallback_ResetRenderState is a special
				// callback value used by the user to request
...
static bool CheckShader(GLuint handle, const char *desc)
{
	...
	if ((GLboolean)status == GL_FALSE)
		fprintf(stderr,
			"ERROR: ImGui_ImplOpenGL3_CreateDeviceObjects: failed "
			"to compile %s!\n",
			desc);
	if (log_length > 1) {
		ImVector<char> buf;
...
bool ImGui_ImplOpenGL3_CreateDeviceObjects()
{
	...
	g_AttribLocationVtxUV =
	    (GLuint)glGetAttribLocation(g_ShaderHandle, "UV");
