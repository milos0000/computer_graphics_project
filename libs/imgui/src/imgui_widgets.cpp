...
void ImGui::TextEx(const char *text, const char *text_end, ImGuiTextFlags flags)
{
	...
	if (text_end - text > 2000 && !wrap_enabled) {
		// Long text!
		// Perform manual coarse clipping to optimize for long
		// multi-line text
		// - From this point we will only compute the width of lines
		// that are visible. Optimization only available when
		// word-wrapping is disabled.
		// - We also don't vertically center the text within the line
		// full height, which is unlikely to matter because we are
		// likely the biggest and only item on the line.
		// - We use memchr(), pay attention that well optimized versions
		// of those str/mem functions are much faster than a casually
		// written loop.
		...
		if (!g.LogEnabled) {
			int lines_skippable =
			    (int)((window->ClipRect.Min.y - text_pos.y) /
				  line_height);
			if (lines_skippable > 0) {
				int lines_skipped = 0;
				while (line < text_end &&
				       lines_skipped < lines_skippable) {
					const char *line_end =
					    (const char *)memchr(
						line, '\n', text_end - line);
					if (!line_end)
		...
		// Lines to render
		if (line < text_end) {
			ImRect line_rect(pos,
					 pos + ImVec2(FLT_MAX, line_height));
			while (line < text_end) {
		...
		// Render (we don't hide text after ## in this end-user
		// function)
...
void ImGui::TextV(const char *fmt, va_list args)
{
	...
	ImGuiContext &g = *GImGui;
	const char *text_end =
	    g.TempBuffer + ImFormatStringV(g.TempBuffer,
					   IM_ARRAYSIZE(g.TempBuffer), fmt,
					   args);
	TextEx(g.TempBuffer, text_end,
	       ImGuiTextFlags_NoWidthForLargeClippedText);
}
...
void ImGui::TextColoredV(const ImVec4 &col, const char *fmt, va_list args)
{
	...
	if (fmt[0] == '%' && fmt[1] == 's' && fmt[2] == 0)
		TextEx(va_arg(args, const char *), NULL,
		       ImGuiTextFlags_NoWidthForLargeClippedText); // Skip
								   // formatting
	else
		TextV(fmt, args);
	PopStyleColor();
...
void ImGui::LabelTextV(const char *label, const char *fmt, va_list args)
{
	...
	const ImRect total_bb(
	    ...
	    window->DC.CursorPos +
		ImVec2(
		    w + (label_size.x > 0.0f ? style.ItemInnerSpacing.x : 0.0f),
	...
	const char *value_text_end =
	    value_text_begin + ImFormatStringV(g.TempBuffer,
					       IM_ARRAYSIZE(g.TempBuffer), fmt,
					       args);
...
void ImGui::BulletTextV(const char *fmt, va_list args)
{
	...
	const char *text_end =
	    text_begin + ImFormatStringV(g.TempBuffer,
					 IM_ARRAYSIZE(g.TempBuffer), fmt, args);
...
// ImGuiButtonFlags_), this code is a little complex. By far the most common
// path is interacting with the Mouse using the default
// ImGuiButtonFlags_PressedOnClickRelease button behavior. See the series of
			   ...
			   bool *out_held, ImGuiButtonFlags flags)
{
	...
	const bool flatten_hovered_children =
	    (flags & ImGuiButtonFlags_FlattenChildren) &&
	...
	// Special mode for Drag and Drop where holding button pressed for a
	// long time while dragging another item triggers the button
	if (g.DragDropActive &&
	    (flags & ImGuiButtonFlags_PressedOnDragDropHold) &&
		...
		if (IsItemHovered(
			...
			hovered = true;
			SetHoveredID(id);
			if (CalcTypematicRepeatAmount(
				g.HoveredIdTimer + 0.0001f - g.IO.DeltaTime,
	...
	// AllowOverlap mode (rarely used) requires previous frame HoveredId to
	// be null or to match. This allows using patterns where a later
	// submitted widget overlaps a previous one.
	...
	if (hovered) {
		if (!(flags & ImGuiButtonFlags_NoKeyModifiers) ||
			...
			// 'Repeat' mode acts when held regardless of _PressedOn
			// flags (see table above). Relies on repeat logic of
			// IsMouseClicked() but we may as well do it ourselves
			// if we end up exposing finer RepeatDelay/RepeatRate
			// settings.
			if (g.ActiveId == id &&
			    (flags & ImGuiButtonFlags_Repeat))
				if (g.IO.MouseDownDuration
	...
	// We report navigated item as hovered but we don't set g.HoveredId to
	// not interfere with mouse.
	...
	if (g.NavActivateDownId == id) {
		...
		bool nav_activated_by_inputs = IsNavInputTest(
		    ImGuiNavInput_Activate, (flags & ImGuiButtonFlags_Repeat)
		...
		if (nav_activated_by_code || nav_activated_by_inputs ||
		    g.ActiveId == id) {
			// Set active id so it can be queried by user via
			// IsItemActive(), equivalent of holding the mouse
			// button.
			g.NavActivateId =
			    id; // This is so SetActiveId assign a Nav source
			SetActiveID(id, window);
			if ((nav_activated_by_code ||
	...
	if (g.ActiveId == id) {
		if (g.ActiveIdSource == ImGuiInputSource_Mouse) {
			...
			if (g.IO.MouseDown[mouse_button]) {
				...
				if ((release_in || release_anywhere) &&
					...
					bool is_double_click_release =
					    (flags &
					     ImGuiButtonFlags_PressedOnDoubleClick) &&
					    g.IO.MouseDownWasDoubleClick
						[mouse_button];
					bool is_repeating_already =
					    (flags & ImGuiButtonFlags_Repeat) &&
					    g.IO.MouseDownDurationPrev
						    [mouse_button] >=
						g.IO.KeyRepeatDelay; // Repeat
								     // mode
								     // trumps
								     // <on
								     // release>
					if (!is_double_click_release &&
					    !is_repeating_already)
						pressed = true;
				}
				ClearActiveID();
			}
			...
			// When activated using Nav, we hold on the ActiveID
			// until activation button is released
			if (g.NavActivateDownId != id)
				ClearActiveID();
		}
		if (pressed)
		     ...
		     ImGuiButtonFlags flags)
{
	...
	if ((flags & ImGuiButtonFlags_AlignTextBaseLine) &&
		...
		window->DC
		    .CurrLineTextBaseOffset) // Try to vertically align buttons
					     // that are smaller/have no padding
					     // so that text baseline matches
					     // (bit hacky, since it shouldn't
					     // be a flag)
		pos.y +=
	...
	RenderTextClipped(bb.Min + style.FramePadding,
			  bb.Max - style.FramePadding, label, NULL, &label_size,
...
bool ImGui::ArrowButtonEx(const char *str_id, ImGuiDir dir, ImVec2 size,
			  ImGuiButtonFlags flags)
{
	ImGuiWindow *window = GetCurrentWindow();
	if (window->SkipItems)
...
bool ImGui::ArrowButton(const char *str_id, ImGuiDir dir)
{
	...
	return ArrowButtonEx(str_id, dir, ImVec2(sz, sz),
...
bool ImGui::CollapseButton(ImGuiID id, const ImVec2 &pos)
{
	...
	bool hovered, held;
	bool pressed =
	    ButtonBehavior(bb, id, &hovered, &held, ImGuiButtonFlags_None);
...
{
	return window->GetIDNoKeepAlive(axis == ImGuiAxis_X ? "#SCROLLX"
							    : "#SCROLLY");
}
...
ImRect ImGui::GetWindowScrollbarRect(ImGuiWindow *window, ImGuiAxis axis)
{
	...
	const float scrollbar_size =
	    window->ScrollbarSizes[axis ^ 1]; // (ScrollbarSizes.x = width of Y
					      // scrollbar; ScrollbarSizes.y =
					      // height of X scrollbar)
	IM_ASSERT(scrollbar_size > 0.0f);
	if (axis == ImGuiAxis_X)
		return ImRect(
		    inner_rect.Min.x,
		    ImMax(outer_rect.Min.y,
			  outer_rect.Max.y - border_size - scrollbar_size),
		    inner_rect.Max.x, outer_rect.Max.y);
	else
		return ImRect(
		    ImMax(outer_rect.Min.x,
...
void ImGui::Scrollbar(ImGuiAxis axis)
{
	...
	ScrollbarEx(bb, id, axis, &window->Scroll[axis], size_avail,
		    size_contents, rounding_corners);
}
...
bool ImGui::ScrollbarEx(const ImRect &bb_frame, ImGuiID id, ImGuiAxis axis,
			float *p_scroll_v, float size_avail_v,
			...
			ImDrawCornerFlags rounding_corners)
{
	...
	if ((axis == ImGuiAxis_Y) &&
		...
		alpha = ImSaturate((bb_frame_height - g.FontSize) /
	...
	ImRect bb = bb_frame;
	bb.Expand(ImVec2(
	    -ImClamp(IM_FLOOR((bb_frame_width - 2.0f) * 0.5f), 0.0f, 3.0f),
	...
	const float scrollbar_size_v =
	    (axis == ImGuiAxis_X) ? bb.GetWidth() : bb.GetHeight();

	// Calculate the height of our grabbable box. It generally represent the
	// amount visible (vs the total scrollable amount) But we maintain a
	// minimum size in pixel to allow for the user to still aim inside.
	IM_ASSERT(ImMax(size_contents_v, size_avail_v) >
		  0.0f); // Adding this assert to check if the ImMax(XXX,1.0f)
			 // is still needed. PLEASE CONTACT ME if this triggers.
	const float win_size_v =
	    ImMax(ImMax(size_contents_v, size_avail_v), 1.0f);
	const float grab_h_pixels =
	    ImClamp(scrollbar_size_v * (size_avail_v / win_size_v),
	...
	float grab_v_norm =
	    scroll_ratio * (scrollbar_size_v - grab_h_pixels) /
	...
	if (held && allow_interaction && grab_h_norm < 1.0f) {
		...
		const float clicked_v_norm = ImSaturate(
		    (mouse_pos_v - scrollbar_pos_v) / scrollbar_size_v);
		SetHoveredID(id);
		...
		// Apply scroll (p_scroll_v will generally point on one member
		// of window->Scroll) It is ok to modify Scroll here because we
		// are being called in Begin() after the calculation of
		// ContentSize and before setting up our starting position
		const float scroll_v_norm = ImSaturate(
		    (clicked_v_norm - g.ScrollbarClickDeltaToGrabCenter -
		     grab_h_norm * 0.5f) /
		    (1.0f - grab_h_norm));
		*p_scroll_v =
		    IM_ROUND(scroll_v_norm *
			     scroll_max); //(win_size_contents_v - win_size_v));

		// Update values for rendering
		scroll_ratio = ImSaturate(*p_scroll_v / scroll_max);
		grab_v_norm = scroll_ratio *
			      (scrollbar_size_v - grab_h_pixels) /
			      scrollbar_size_v;
	...
	if (axis == ImGuiAxis_X)
		grab_rect = ImRect(
		    ImLerp(bb.Min.x, bb.Max.x, grab_v_norm), bb.Min.y,
		    ImLerp(bb.Min.x, bb.Max.x, grab_v_norm) + grab_h_pixels,
		    bb.Max.y);
	else
		grab_rect = ImRect(
		    bb.Min.x, ImLerp(bb.Min.y, bb.Max.y, grab_v_norm), bb.Max.x,
		  ...
		  const ImVec4 &border_col)
{
	...
	if (border_col.w > 0.0f) {
		...
		window->DrawList->AddImage(
		    user_texture_id, bb.Min + ImVec2(1, 1),
		    bb.Max - ImVec2(1, 1), uv0, uv1, GetColorU32(tint_col));
	} else {
		window->DrawList->AddImage(user_texture_id, bb.Min, bb.Max, uv0,
...
bool ImGui::ImageButtonEx(ImGuiID id, ImTextureID texture_id,
			  const ImVec2 &size, const ImVec2 &uv0,
			  const ImVec2 &uv1, const ImVec2 &padding,
			  const ImVec4 &bg_col, const ImVec4 &tint_col)
{
	...
	RenderFrame(bb.Min, bb.Max, col, true,
		    ImClamp((float)ImMin(padding.x, padding.y), 0.0f,
			    g.Style.FrameRounding));
	if (bg_col.w > 0.0f)
		window->DrawList->AddRectFilled(
		    bb.Min + padding, bb.Max - padding, GetColorU32(bg_col));
	window->DrawList->AddImage(texture_id, bb.Min + padding,
				   bb.Max - padding, uv0, uv1,
			...
			const ImVec4 &bg_col, const ImVec4 &tint_col)
{
	...
	const ImVec2 padding =
	    (frame_padding >= 0)
		? ImVec2((float)frame_padding, (float)frame_padding)
		: g.Style.FramePadding;
	return ImageButtonEx(id, user_texture_id, size, uv0, uv1, padding,
			     bg_col, tint_col);
}
...
bool ImGui::Checkbox(const char *label, bool *v)
{
	...
	bool mixed_value =
	    (window->DC.ItemFlags & ImGuiItemFlags_MixedValue) != 0;
	if (mixed_value) {
		// Undocumented tristate/mixed/indeterminate checkbox (#2644)
		// This may seem awkwardly designed because the aim is to make
		// ImGuiItemFlags_MixedValue supported by all widgets (not just
		// checkbox)
		...
		window->DrawList->AddRectFilled(check_bb.Min + pad,
						check_bb.Max - pad, check_col,
						style.FrameRounding);
	} else if (*v) {
		const float pad = ImMax(1.0f, IM_FLOOR(square_sz / 6.0f));
		RenderCheckMark(window->DrawList,
				check_bb.Min + ImVec2(pad, pad), check_col,
				square_sz - pad * 2.0f);
	}

	if (g.LogEnabled)
		LogRenderedText(&total_bb.Min, mixed_value ? "[~]"
					       : *v	   ? "[x]"
							   : "[ ]");
	if (label_size.x > 0.0f)
...
bool ImGui::RadioButton(const char *label, bool active)
{
	...
	if (active) {
		const float pad = ImMax(1.0f, IM_FLOOR(square_sz / 6.0f));
		window->DrawList->AddCircleFilled(
		    center, radius - pad, GetColorU32(ImGuiCol_CheckMark), 16);
	}
	...
	if (style.FrameBorderSize > 0.0f) {
		...
		window->DrawList->AddCircle(center, radius,
					    GetColorU32(ImGuiCol_Border), 16,
			...
			const char *overlay)
{
	...
	const ImVec2 fill_br =
	    ImVec2(ImLerp(bb.Min.x, bb.Max.x, fraction), bb.Max.y);
	RenderRectFilledRangeH(window->DrawList, bb,
			       GetColorU32(ImGuiCol_PlotHistogram), 0.0f,
			       fraction, style.FrameRounding);

	// Default displaying the fraction as percentage string, but user can
	// override it
	char overlay_buf[32];
	if (!overlay) {
	...
	if (overlay_size.x > 0.0f)
		RenderTextClipped(
		    ImVec2(ImClamp(fill_br.x + style.ItemSpacing.x, bb.Min.x,
			   ...
			   bb.Min.y),
		    bb.Max, overlay, NULL, &overlay_size, ImVec2(0.0f, 0.5f),
		    &bb);
}
...
void ImGui::NewLine()
{
	...
	if (window->DC.CurrLineSize.y >
	    0.0f) // In the event that we are on a line with items that is
		  // smaller that FontSize high, we will preserve its height.
		ItemSize(ImVec2(0, 0));
	else
...
void ImGui::AlignTextToFramePadding()
{
	...
	window->DC.CurrLineSize.y = ImMax(
	    window->DC.CurrLineSize.y, g.FontSize + g.Style.FramePadding.y * 2);
	window->DC.CurrLineTextBaseOffset =
...
void ImGui::SeparatorEx(ImGuiSeparatorFlags flags)
{
	...
	IM_ASSERT(ImIsPowerOfTwo(
	    flags & (ImGuiSeparatorFlags_Horizontal |
		     ImGuiSeparatorFlags_Vertical))); // Check that only 1
						      // option is selected
	...
	if (flags & ImGuiSeparatorFlags_Vertical) {
		...
		// Not exposed because it is misleading and it doesn't have an
		// effect on regular layout.
		float y1 = window->DC.CursorPos.y;
		float y2 = window->DC.CursorPos.y + window->DC.CurrLineSize.y;
		const ImRect bb(
		    ImVec2(window->DC.CursorPos.x, y1),
		...
		// Draw
		window->DrawList->AddLine(ImVec2(bb.Min.x, bb.Min.y),
		...
		// FIXME-WORKRECT: old hack (#205) until we decide of consistent
		// behavior with WorkRect/Indent and Separator
		if (g.GroupStack.Size > 0 &&
		    g.GroupStack.back().WindowID == window->ID)
		...
		// We don't provide our width to the layout so that it doesn't
		// get feed back into AutoFit
		const ImRect bb(
		    ImVec2(x1, window->DC.CursorPos.y),
		...
		if (item_visible) {
			...
			window->DrawList->AddLine(
			    bb.Min, ImVec2(bb.Max.x, bb.Min.y),
			    GetColorU32(ImGuiCol_Separator));
			if (g.LogEnabled)
				LogRenderedText(
				    &bb.Min,
			     ...
			     float hover_visibility_delay)
{
	...
	if (held || (g.HoveredId == id && g.HoveredIdPreviousFrame == id &&
		...
		SetMouseCursor(axis == ImGuiAxis_Y ? ImGuiMouseCursor_ResizeNS
						   : ImGuiMouseCursor_ResizeEW);

	ImRect bb_render = bb;
	if (held) {
		ImVec2 mouse_delta_2d =
		    g.IO.MousePos - g.ActiveIdClickOffset - bb_interact.Min;
		float mouse_delta =
		    (axis == ImGuiAxis_Y) ? mouse_delta_2d.y : mouse_delta_2d.x;
		...
		if (mouse_delta != 0.0f) {
			...
			*size1 += mouse_delta;
			*size2 -= mouse_delta;
			bb_render.Translate((axis == ImGuiAxis_X)
						? ImVec2(mouse_delta, 0.0f)
...
void ImGui::ShrinkWidths(ImGuiShrinkWidthItem *items, int count,
			 float width_excess)
{
	if (count == 1) {
		if (items[0].Width >= 0.0f)
			items[0].Width =
			    ImMax(items[0].Width - width_excess, 1.0f);
		return;
	}
	ImQsort(items, (size_t)count, sizeof(ImGuiShrinkWidthItem),
	...
	while (width_excess > 0.0f && count_same_width < count) {
		...
		float width_to_remove_per_item =
		    ImMin(width_excess / count_same_width,
	...
	// Round width and redistribute remainder left-to-right (could make it
	// an option of the function?) Ensure that e.g. the right-most tab of a
	// shrunk tab-bar always reaches exactly at the same distance from the
...
bool ImGui::BeginCombo(const char *label, const char *preview_value,
		       ImGuiComboFlags flags)
{
	// Always consume the SetNextWindowSizeConstraint() call in our early
	// return paths
	ImGuiContext &g = *GImGui;
	bool has_window_size_constraint =
	    (g.NextWindowData.Flags &
	     ImGuiNextWindowDataFlags_HasSizeConstraint) != 0;
	...
	const float w =
	    (flags & ImGuiComboFlags_NoPreview) ? arrow_size : expected_w;
	...
	const float value_x2 =
	    ImMax(frame_bb.Min.x, frame_bb.Max.x - arrow_size);
	RenderNavHighlight(frame_bb, id);
	if (!(flags & ImGuiComboFlags_NoPreview))
		window->DrawList->AddRectFilled(
		    frame_bb.Min, ImVec2(value_x2, frame_bb.Max.y), frame_col,
		    style.FrameRounding,
	...
	if (!(flags & ImGuiComboFlags_NoArrowButton)) {
		...
		ImU32 text_col = GetColorU32(ImGuiCol_Text);
		window->DrawList->AddRectFilled(
		    ImVec2(value_x2, frame_bb.Min.y), frame_bb.Max, bg_col,
		    style.FrameRounding,
		    (w <= arrow_size) ? ImDrawCornerFlags_All
				      : ImDrawCornerFlags_Right);
		if (value_x2 + arrow_size - style.FramePadding.x <=
		    frame_bb.Max.x)
			RenderArrow(
			    window->DrawList,
	...
	if (has_window_size_constraint) {
		...
		IM_ASSERT(ImIsPowerOfTwo(
		    flags & ImGuiComboFlags_HeightMask_)); // Only one
		...
		SetNextWindowSizeConstraints(
		    ImVec2(w, 0.0f),
		    ImVec2(FLT_MAX, CalcMaxPopupHeightFromItemCount(
					popup_max_height_in_items)));
	}

	char name[16];
	ImFormatString(
	    name, IM_ARRAYSIZE(name), "##Combo_%02d",
	    g.BeginPopupStack.Size); // Recycle windows based on depth

	// Position the window given a custom constraint (peak into expected
	// window size so we can position it) This might be easier to express
	// with an hypothetical SetNextWindowPosConstraints() function.
	if (ImGuiWindow *popup_window = FindWindowByName(name))
		if (popup_window->WasActive) {
			// Always override 'AutoPosLastDirection' to not leave a
			// chance for a past value to affect us.
			ImVec2 size_expected =
			    CalcWindowExpectedSize(popup_window);
			if (flags & ImGuiComboFlags_PopupAlignLeft)
				popup_window->AutoPosLastDirection =
				    ImGuiDir_Left; // "Below, Toward Left"
			else
				popup_window->AutoPosLastDirection =
				    ImGuiDir_Down; // "Below, Toward Right
						   // (default)"
			ImRect r_outer =
			    GetWindowAllowedExtentRect(popup_window);
			ImVec2 pos = FindBestWindowPosForPopupEx(
			    frame_bb.GetBL(), size_expected,
			    &popup_window->AutoPosLastDirection, r_outer,
			    frame_bb, ImGuiPopupPositionPolicy_ComboBox);
			SetNextWindowPos(pos);
		}

	// We don't use BeginPopupEx() solely because we have a custom name
	// string, which we could make an argument to BeginPopupEx()
	ImGuiWindowFlags window_flags =
	    ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_Popup |
	    ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize |
	    ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoMove;

	// Horizontally align ourselves with the framed text
	PushStyleVar(ImGuiStyleVar_WindowPadding,
		     ImVec2(style.FramePadding.x, style.WindowPadding.y));
	bool ret = Begin(name, NULL, window_flags);
	PopStyleVar();
	if (!ret) {
		EndPopup();
		IM_ASSERT(0); // This should never happen as we tested for
			      // IsPopupOpen() above
		return false;
	}
	return true;
...
{
	// FIXME-OPT: we could pre-compute the indices to fasten this. But only
	// 1 active combo means the waste is limited.
		  ...
		  int items_count, int popup_max_height_in_items)
{
	...
	// general BeginCombo() API doesn't have/need it, but we emulate it
	// here.
		  ...
		  int height_in_items)
{
	const bool value_changed =
	    Combo(label, current_item, Items_ArrayGetter, (void *)items,
...
// FIXME-LEGACY: Prior to 1.61 our DragInt() function internally used floats and
// because of this the compile-time default value for format was "%.0f". Even
// though we changed the compile-time default, we expect users to have carried
// %f around, which would break the display of DragInt() calls. To honor
// backward compatibility we are rewriting the format string, unless
// IMGUI_DISABLE_OBSOLETE_FUNCTIONS is enabled. What could possibly go wrong?!
static const char *PatchFormatStringFloatToInt(const char *fmt)
{
	if (fmt[0] == '%' && fmt[1] == '.' && fmt[2] == '0' && fmt[3] == 'f' &&
	    fmt[4] == 0) // Fast legacy path for "%.0f" which is expected to be
			 // the most common case.
		return "%d";
	const char *fmt_start =
	    ImParseFormatFindStart(fmt); // Find % (if any, and ignore %%)
	const char *fmt_end = ImParseFormatFindEnd(
	    fmt_start); // Find end of format specifier, which itself is an
			// exercise of confidence/recklessness (because snprintf
			// is dependent on libc or user).
	if (fmt_end > fmt_start && fmt_end[-1] == 'f') {
		...
		ImGuiContext &g = *GImGui;
		ImFormatString(
		    g.TempBuffer, IM_ARRAYSIZE(g.TempBuffer), "%.*s%%d%s",
		    (int)(fmt_start - fmt), fmt,
		    fmt_end); // Honor leading and trailing decorations, but
			      // lose alignment/precision.
		return g.TempBuffer;
#else
		IM_ASSERT(
		    0 &&
		    "DragInt(): Invalid format string!"); // Old versions used a
							  // default parameter
							  // of "%.0f", please
							  // replace with e.g.
							  // "%d"
...
int ImGui::DataTypeFormatString(char *buf, int buf_size,
				ImGuiDataType data_type, const void *p_data,
				const char *format)
{
	// Signedness doesn't matter when pushing integer arguments
	if (data_type == ImGuiDataType_S32 || data_type == ImGuiDataType_U32)
		return ImFormatString(buf, buf_size, format,
				      *(const ImU32 *)p_data);
...
void ImGui::DataTypeApplyOp(ImGuiDataType data_type, int op, void *output,
	...
	case ImGuiDataType_S8:
		if (op == '+') {
			*(ImS8 *)output = ImAddClampOverflow(
			    *(const ImS8 *)arg1, *(const ImS8 *)arg2, IM_S8_MIN,
			    IM_S8_MAX);
		}
		if (op == '-') {
	...
	case ImGuiDataType_U8:
		if (op == '+') {
			*(ImU8 *)output = ImAddClampOverflow(
			    *(const ImU8 *)arg1, *(const ImU8 *)arg2, IM_U8_MIN,
			    IM_U8_MAX);
		}
		if (op == '-') {
	...
	case ImGuiDataType_S16:
		if (op == '+') {
			*(ImS16 *)output = ImAddClampOverflow(
			    *(const ImS16 *)arg1, *(const ImS16 *)arg2,
	...
	case ImGuiDataType_U16:
		if (op == '+') {
			*(ImU16 *)output = ImAddClampOverflow(
			    *(const ImU16 *)arg1, *(const ImU16 *)arg2,
	...
	case ImGuiDataType_S32:
		if (op == '+') {
			*(ImS32 *)output = ImAddClampOverflow(
			    *(const ImS32 *)arg1, *(const ImS32 *)arg2,
	...
	case ImGuiDataType_U32:
		if (op == '+') {
			*(ImU32 *)output = ImAddClampOverflow(
			    *(const ImU32 *)arg1, *(const ImU32 *)arg2,
	...
	case ImGuiDataType_S64:
		if (op == '+') {
			*(ImS64 *)output = ImAddClampOverflow(
			    *(const ImS64 *)arg1, *(const ImS64 *)arg2,
	...
	case ImGuiDataType_U64:
		if (op == '+') {
			*(ImU64 *)output = ImAddClampOverflow(
			    *(const ImU64 *)arg1, *(const ImU64 *)arg2,
				    ...
				    const char *format)
{
	...
	// We don't support '-' op because it would conflict with inputing
	// negative value. Instead you can use +-100 to subtract from an
	// existing value
	char op = buf[0];
	if (op == '+' || op == '*' || op == '/') {
		buf++;
	...
	// Copy the value in an opaque buffer so we can compare at the end of
	// the function if it changed at all.
	...
	// FIXME-LEGACY: The aim is to remove those operators and write a proper
	// expression evaluator at some point..
	int arg1i = 0;
	if (data_type == ImGuiDataType_S32) {
		...
		if (op && sscanf(initial_value_buf, format, &arg0i) < 1)
			return false;
		// Store operand in a float so we can use fractional value for
		// multipliers (*1.1), but constant always parsed as integer so
		// we can fit big integers (e.g. 2000000003) past float
		// precision
		if (op == '+') {
		...
		// For floats we have to ignore format with precision (e.g.
		// "%.2f") because sscanf doesn't take them in
		format = "%f";
		...
		format = "%lf"; // scanf differentiate float/double unlike
				// printf which forces everything to double
				// because of ellipsis
		...
		// We don't bother handling support for legacy operators since
		// they are a little too crappy. Instead we will later implement
		// a proper expression evaluator in the future.
		sscanf(buf, format, p_data);
	} else {
		// Small types need a 32-bit buffer to receive the result from
		// scanf()
		int v32;
		sscanf(buf, format, &v32);
		if (data_type == ImGuiDataType_S8)
			*(ImS8 *)p_data =
			    (ImS8)ImClamp(v32, (int)IM_S8_MIN, (int)IM_S8_MAX);
		else if (data_type == ImGuiDataType_U8)
			*(ImU8 *)p_data =
			    (ImU8)ImClamp(v32, (int)IM_U8_MIN, (int)IM_U8_MAX);
		else if (data_type == ImGuiDataType_S16)
			*(ImS16 *)p_data = (ImS16)ImClamp(v32, (int)IM_S16_MIN,
							  (int)IM_S16_MAX);
		else if (data_type == ImGuiDataType_U16)
			*(ImU16 *)p_data = (ImU16)ImClamp(v32, (int)IM_U16_MIN,
							  (int)IM_U16_MAX);
		else
			IM_ASSERT(0);
	}
...
int ImGui::DataTypeCompare(ImGuiDataType data_type, const void *arg_1,
			   const void *arg_2)
{
	switch (data_type) {
	case ImGuiDataType_S8:
		return DataTypeCompareT<ImS8>((const ImS8 *)arg_1,
					      (const ImS8 *)arg_2);
	case ImGuiDataType_U8:
		return DataTypeCompareT<ImU8>((const ImU8 *)arg_1,
					      (const ImU8 *)arg_2);
	case ImGuiDataType_S16:
		return DataTypeCompareT<ImS16>((const ImS16 *)arg_1,
					       (const ImS16 *)arg_2);
	case ImGuiDataType_U16:
		return DataTypeCompareT<ImU16>((const ImU16 *)arg_1,
					       (const ImU16 *)arg_2);
	case ImGuiDataType_S32:
		return DataTypeCompareT<ImS32>((const ImS32 *)arg_1,
					       (const ImS32 *)arg_2);
	case ImGuiDataType_U32:
		return DataTypeCompareT<ImU32>((const ImU32 *)arg_1,
					       (const ImU32 *)arg_2);
	case ImGuiDataType_S64:
		return DataTypeCompareT<ImS64>((const ImS64 *)arg_1,
					       (const ImS64 *)arg_2);
	case ImGuiDataType_U64:
		return DataTypeCompareT<ImU64>((const ImU64 *)arg_1,
					       (const ImU64 *)arg_2);
	case ImGuiDataType_Float:
...
bool ImGui::DataTypeClamp(ImGuiDataType data_type, void *p_data,
			  const void *p_min, const void *p_max)
{
	switch (data_type) {
	case ImGuiDataType_S8:
		return DataTypeClampT<ImS8>((ImS8 *)p_data, (const ImS8 *)p_min,
					    (const ImS8 *)p_max);
	case ImGuiDataType_U8:
		return DataTypeClampT<ImU8>((ImU8 *)p_data, (const ImU8 *)p_min,
					    (const ImU8 *)p_max);
	case ImGuiDataType_S16:
...
static float GetMinimumStepAtDecimalPrecision(int decimal_precision)
{
	...
	if (decimal_precision < 0)
		return FLT_MIN;
	return (decimal_precision < IM_ARRAYSIZE(min_steps))
...
TYPE ImGui::RoundScalarWithFormatT(const char *format, ImGuiDataType data_type,
				   TYPE v)
{
	const char *fmt_start = ImParseFormatFindStart(format);
	if (fmt_start[0] != '%' ||
	    fmt_start[1] == '%') // Don't apply if the value is not visible in
				 // the format string
		return v;
	char v_str[64];
...
bool ImGui::DragBehaviorT(ImGuiDataType data_type, TYPE *v, float v_speed,
			  const TYPE v_min, const TYPE v_max,
			  const char *format, ImGuiSliderFlags flags)
{
	ImGuiContext &g = *GImGui;
	const ImGuiAxis axis =
	    (flags & ImGuiSliderFlags_Vertical) ? ImGuiAxis_Y : ImGuiAxis_X;
	const bool is_decimal = (data_type == ImGuiDataType_Float) ||
				(data_type == ImGuiDataType_Double);
	const bool is_clamped = (v_min < v_max);
	const bool is_logarithmic =
	    (flags & ImGuiSliderFlags_Logarithmic) && is_decimal;
	...
	// current value as soon as it makes a difference with our precision
	// settings
	...
	if (g.ActiveIdSource == ImGuiInputSource_Mouse && IsMousePosValid() &&
		...
		int decimal_precision =
		    is_decimal ? ImParseFormatPrecision(format, 3) : 0;
		adjust_delta = GetNavInputAmount2d(
	...
	// For logarithmic use our range is effectively 0..1 so scale the delta
	// into that range
	...
	// current value is 300 and we are pushing to the right side, keep the
	// 300.
	...
	if (is_logarithmic) {
		// When using logarithmic sliders, we need to clamp to avoid
		// hitting zero, but our choice of clamp value greatly affects
		// slider precision. We attempt to use the specified precision
		// to estimate a good lower bound.
		const int decimal_precision =
		    is_decimal ? ImParseFormatPrecision(format, 3) : 1;
		logarithmic_zero_epsilon =
		    ImPow(0.1f, (float)decimal_precision);
	...
	if (!(flags & ImGuiSliderFlags_NoRoundToFormat))
		v_cur = RoundScalarWithFormatT<TYPE, SIGNEDTYPE>(
		    format, data_type, v_cur);

	// Preserve remainder after rounding has been applied. This also allow
	// slow tweaking of values.
	g.DragCurrentAccumDirty = false;
	if (is_logarithmic) {
		// Convert to parametric space, apply delta, convert back
		...
		g.DragCurrentAccum -=
		    (float)((SIGNEDTYPE)v_cur - (SIGNEDTYPE)*v);
	}
	...
	if (*v != v_cur && is_clamped) {
		if (v_cur < v_min ||
		    (v_cur > *v && adjust_delta < 0.0f && !is_decimal))
			v_cur = v_min;
		if (v_cur > v_max ||
		    (v_cur < *v && adjust_delta > 0.0f && !is_decimal))
			v_cur = v_max;
...
bool ImGui::DragBehavior(ImGuiID id, ImGuiDataType data_type, void *p_v,
			 float v_speed, const void *p_min, const void *p_max,
			 const char *format, ImGuiSliderFlags flags)
{
	// Read imgui.cpp "API BREAKING CHANGES" section for 1.78 if you hit
	// this assert.
	IM_ASSERT(
	    (flags == 1 || (flags & ImGuiSliderFlags_InvalidMask_) == 0) &&
	    "Invalid ImGuiSliderFlags flags! Has the 'float power' argument "
...
bool ImGui::DragScalar(const char *label, ImGuiDataType data_type, void *p_data,
		       float v_speed, const void *p_min, const void *p_max,
		       const char *format, ImGuiSliderFlags flags)
{
	ImGuiWindow *window = GetCurrentWindow();
	...
	if (format == NULL)
		format = DataTypeGetInfo(data_type)->PrintFmt;
		 ...
		 strcmp(format, "%d") !=
		     0) // (FIXME-LEGACY: Patch old "%.0f" format string to use
	...
	if (!temp_input_is_active) {
		const bool focus_requested =
		    temp_input_allowed && FocusableItemRegister(window, id);
		const bool clicked = (hovered && g.IO.MouseClicked[0]);
		const bool double_clicked =
		    (hovered && g.IO.MouseDoubleClicked[0]);
	...
	if (temp_input_is_active) {
		// Only clamp CTRL+Click input when ImGuiSliderFlags_AlwaysClamp
		// is set
	...
	// Drag behavior
	const bool value_changed = DragBehavior(id, data_type, p_data, v_speed,
...
bool ImGui::DragScalarN(const char *label, ImGuiDataType data_type,
...
{
	return DragScalar(label, ImGuiDataType_Float, v, v_speed, &v_min,
			  &v_max, format, flags);
}
...
{
	return DragScalarN(label, ImGuiDataType_Float, v, 2, v_speed, &v_min,
			   &v_max, format, flags);
}
...
bool ImGui::DragFloat3(const char *label, float v[3], float v_speed,
		       ...
		       ImGuiSliderFlags flags)
{
	return DragScalarN(label, ImGuiDataType_Float, v, 3, v_speed, &v_min,
			   &v_max, format, flags);
}
...
bool ImGui::DragFloat4(const char *label, float v[4], float v_speed,
		       ...
		       ImGuiSliderFlags flags)
{
	return DragScalarN(label, ImGuiDataType_Float, v, 4, v_speed, &v_min,
			   &v_max, format, flags);
}

// NB: You likely want to specify the ImGuiSliderFlags_AlwaysClamp when using
// this.
			    ...
			    const char *format_max, ImGuiSliderFlags flags)
{
	...
	ImGuiSliderFlags min_flags =
	    flags | ((min_min == min_max) ? ImGuiSliderFlags_ReadOnly : 0);
	bool value_changed =
	    DragScalar("##min", ImGuiDataType_Float, v_current_min, v_speed,
		       &min_min, &min_max, format, min_flags);
	PopItemWidth();
	SameLine(0, g.Style.ItemInnerSpacing.x);
	...
	ImGuiSliderFlags max_flags =
	    flags | ((max_min == max_max) ? ImGuiSliderFlags_ReadOnly : 0);
	value_changed |= DragScalar(
	    "##max", ImGuiDataType_Float, v_current_max, v_speed, &max_min,
...
{
	return DragScalar(label, ImGuiDataType_S32, v, v_speed, &v_min, &v_max,
			  format, flags);
}
...
bool ImGui::DragInt2(const char *label, int v[2], float v_speed, int v_min,
		     int v_max, const char *format, ImGuiSliderFlags flags)
{
	return DragScalarN(label, ImGuiDataType_S32, v, 2, v_speed, &v_min,
			   &v_max, format, flags);
}
...
bool ImGui::DragInt3(const char *label, int v[3], float v_speed, int v_min,
		     int v_max, const char *format, ImGuiSliderFlags flags)
{
	return DragScalarN(label, ImGuiDataType_S32, v, 3, v_speed, &v_min,
			   &v_max, format, flags);
}
...
bool ImGui::DragInt4(const char *label, int v[4], float v_speed, int v_min,
		     int v_max, const char *format, ImGuiSliderFlags flags)
{
	return DragScalarN(label, ImGuiDataType_S32, v, 4, v_speed, &v_min,
			   &v_max, format, flags);
}
			  ...
			  ImGuiSliderFlags flags)
{
	...
	ImGuiSliderFlags max_flags =
	    flags | ((max_min == max_max) ? ImGuiSliderFlags_ReadOnly : 0);
	value_changed |=
	    DragInt("##max", v_current_max, v_speed, max_min, max_max,
...
// Obsolete versions with power parameter. See
// https://github.com/ocornut/imgui/issues/3361 for details.
bool ImGui::DragScalar(const char *label, ImGuiDataType data_type, void *p_data,
		       float v_speed, const void *p_min, const void *p_max,
		       const char *format, float power)
{
	...
	if (power != 1.0f) {
		IM_ASSERT(
		    power == 1.0f &&
		    "Call function with ImGuiSliderFlags_Logarithmic flags "
		    "instead of using the old 'float power' function!");
		IM_ASSERT(p_min != NULL &&
			  p_max != NULL); // When using a power curve the drag
					  // needs to have known bounds
		drag_flags |=
		    ImGuiSliderFlags_Logarithmic; // Fallback for non-asserting
						  // paths
	}
	return DragScalar(label, data_type, p_data, v_speed, p_min, p_max,
			  format, drag_flags);
}
			...
			const char *format, float power)
{
	...
	return DragScalarN(label, data_type, p_data, components, v_speed, p_min,
			   p_max, format, drag_flags);
}
...
template <typename TYPE, typename SIGNEDTYPE, typename FLOATTYPE>
float ImGui::ScaleRatioFromValueT(ImGuiDataType data_type, TYPE v, TYPE v_min,
				  ...
				  float zero_deadzone_halfsize)
{
	...
	const TYPE v_clamped = (v_min < v_max) ? ImClamp(v, v_min, v_max)
	...
	if (is_logarithmic) {
		...
		// Awkward special cases - we need ranges of the form (-100 ..
		// 0) to convert to (-100 .. -epsilon), not (-100 .. epsilon)
		...
		if (v_clamped <= v_min_fudged)
			result = 0.0f; // Workaround for values that are
				       // in-range but below our fudge
		else if (v_clamped >= v_max_fudged)
			result = 1.0f; // Workaround for values that are
				       // in-range but above our fudge
		else if ((v_min * v_max) <
			 0.0f) // Range crosses zero, so split into two portions
		{
			float zero_point_center =
			    ...
			    ((float)v_max -
			     (float)
				 v_min); // The zero point in parametric space.
					 // There's an argument we should take
					 // the logarithmic nature into account
					 // when calculating this, but for now
					 // this should do (and the most common
					 // case of a symmetrical range works
					 // fine)
			...
			float zero_point_snap_R =
			    zero_point_center + zero_deadzone_halfsize;
			if (v == 0.0f)
				result = zero_point_center; // Special case for
							    // exactly zero
			else if (v < 0.0f)
				result =
				    (1.0f -
				     (float)(ImLog(-(FLOATTYPE)v_clamped /
						   logarithmic_zero_epsilon) /
					     ImLog(-v_min_fudged /
						   logarithmic_zero_epsilon))) *
				    zero_point_snap_L;
			   ...
			   (v_max < 0.0f)) // Entirely negative slider
			result = 1.0f -
				 (float)(ImLog(-(FLOATTYPE)v_clamped /
					       -v_max_fudged) /
			...
			result =
			    (float)(ImLog((FLOATTYPE)v_clamped / v_min_fudged) /
...
template <typename TYPE, typename SIGNEDTYPE, typename FLOATTYPE>
TYPE ImGui::ScaleValueFromRatioT(ImGuiDataType data_type, float t, TYPE v_min,
				 TYPE v_max, bool is_logarithmic,
				 float logarithmic_zero_epsilon,
				 float zero_deadzone_halfsize)
{
	...
	TYPE result;
	if (is_logarithmic) {
		// We special-case the extents because otherwise our fudging can
		// lead to "mathematically correct" but non-intuitive behaviors
		// like a fully-left slider not actually reaching the minimum
		// value
		if (t <= 0.0f)
			result = v_min;
		else if (t >= 1.0f)
			...
			bool flipped =
			    v_max < v_min; // Check if range is "backwards"

			// Fudge min/max to avoid getting silly results close to
			// zero
			FLOATTYPE v_min_fudged =
			    (ImAbs((FLOATTYPE)v_min) < logarithmic_zero_epsilon)
				? ((v_min < 0.0f) ? -logarithmic_zero_epsilon
			...
			// Awkward special case - we need ranges of the form
			// (-100 .. 0) to convert to (-100 .. -epsilon), not
			// (-100 .. epsilon)
			...
			float t_with_flip =
			    flipped ? (1.0f - t)
				    : t; // t, but flipped if necessary to
					 // account for us flipping the range

			if ((v_min * v_max) <
			    0.0f) // Range crosses zero, so we have to do this
				  // in two parts
			{
				float zero_point_center =
				    (-(float)ImMin(v_min, v_max)) /
				    ImAbs((float)v_max -
					  (float)v_min); // The zero point in
							 // parametric space
				float zero_point_snap_L =
				    zero_point_center - zero_deadzone_halfsize;
				float zero_point_snap_R =
				...
				if (t_with_flip >= zero_point_snap_L &&
					...
					result =
					    (TYPE)0.0f; // Special case to make
							// getting exactly zero
							// possible (the epsilon
							// prevents it
							// otherwise)
				...
				result =
				    (TYPE) -
				    (-v_max_fudged *
				     ImPow(-v_min_fudged / -v_max_fudged,
					   (FLOATTYPE)(1.0f - t_with_flip)));
			else
				result =
				    (TYPE)(v_min_fudged *
					   ImPow(v_max_fudged / v_min_fudged,
						 (FLOATTYPE)t_with_flip));
		}
	} else {
		// Linear slider
		if (is_decimal) {
			...
			// match the grab box so we round above
			//   This code is carefully tuned to work with large
			//   values (e.g. high ranges of U64) while preserving
			//   this property..
			// - Not doing a *1.0 multiply at the end of a range as
			// it tends to be lossy. While absolute aiming at a
			// large s64/u64
			//   range is going to be imprecise anyway, with this
			//   check we at least make the edge values matches
			//   expected limits.
			if (t < 1.0) {
				FLOATTYPE v_new_off_f =
				    (SIGNEDTYPE)(v_max - v_min) * t;
				result =
				    (TYPE)((SIGNEDTYPE)v_min +
					   (SIGNEDTYPE)(v_new_off_f +
							(FLOATTYPE)(v_min > v_max
									? -0.5
									: 0.5)));
			} else {
				result = v_max;
...
template <typename TYPE, typename SIGNEDTYPE, typename FLOATTYPE>
bool ImGui::SliderBehaviorT(const ImRect &bb, ImGuiID id,
			    ImGuiDataType data_type, TYPE *v, const TYPE v_min,
			    const TYPE v_max, const char *format,
			    ImGuiSliderFlags flags, ImRect *out_grab_bb)
{
	...
	const float slider_sz =
	    (bb.Max[axis] - bb.Min[axis]) - grab_padding * 2.0f;
	...
	if (g.ActiveId == id) {
		...
		if (g.ActiveIdSource == ImGuiInputSource_Mouse) {
			...
			if (g.ActiveIdIsJustActivated) {
				g.SliderCurrentAccum =
				    0.0f; // Reset any stored nav delta upon
					  // activation
			...
			float input_delta = (axis == ImGuiAxis_X)
						? input_delta2.x
						: -input_delta2.y;
			if (input_delta != 0.0f) {
				const int decimal_precision =
			...
			if (g.NavActivatePressedId == id &&
			    !g.ActiveIdIsJustActivated) {
				ClearActiveID();
			} else if (g.SliderCurrentAccumDirty) {
				clicked_t =
				    ScaleRatioFromValueT<TYPE, SIGNEDTYPE,
							 FLOATTYPE>(
					data_type, *v, v_min, v_max,
					is_logarithmic,
					logarithmic_zero_epsilon,
				...
				if ((clicked_t >= 1.0f && delta > 0.0f) ||
				    (clicked_t <= 0.0f &&
				     delta < 0.0f)) // This is to avoid applying
						    // the saturation when
						    // already past the limits
				{
					set_new_value = false;
					g.SliderCurrentAccum =
					    0.0f; // If pushing up against the
						  // limits, don't continue to
						  // accumulate
				} else {
					set_new_value = true;
					float old_clicked_t = clicked_t;
					clicked_t =
					    ImSaturate(clicked_t + delta);

					// Calculate what our "new" clicked_t
					// will be, and thus how far we actually
					// moved the slider, and subtract this
					// from the accumulator
					TYPE v_new = ScaleValueFromRatioT<
					    TYPE, SIGNEDTYPE, FLOATTYPE>(
					    data_type, clicked_t, v_min, v_max,
					    is_logarithmic,
					    logarithmic_zero_epsilon,
					    zero_deadzone_halfsize);
					if (!(flags &
					      ImGuiSliderFlags_NoRoundToFormat))
						v_new = RoundScalarWithFormatT<
						    TYPE, SIGNEDTYPE>(
						    format, data_type, v_new);
					float new_clicked_t =
					    ScaleRatioFromValueT<
						TYPE, SIGNEDTYPE, FLOATTYPE>(
						data_type, v_new, v_min, v_max,
						is_logarithmic,
						logarithmic_zero_epsilon,
						zero_deadzone_halfsize);
		...
		if (set_new_value) {
			...
			// Round to user desired precision based on format
			// string
			if (!(flags & ImGuiSliderFlags_NoRoundToFormat))
				v_new =
				    RoundScalarWithFormatT<TYPE, SIGNEDTYPE>(
					format, data_type, v_new);
	...
	if (slider_sz < 1.0f) {
		*out_grab_bb = ImRect(bb.Min, bb.Min);
		...
		if (axis == ImGuiAxis_Y)
			grab_t = 1.0f - grab_t;
		const float grab_pos = ImLerp(slider_usable_pos_min,
					      slider_usable_pos_max, grab_t);
		if (axis == ImGuiAxis_X)
			*out_grab_bb = ImRect(
			    grab_pos - grab_sz * 0.5f, bb.Min.y + grab_padding,
			    grab_pos + grab_sz * 0.5f, bb.Max.y - grab_padding);
		else
			*out_grab_bb = ImRect(
			    bb.Min.x + grab_padding, grab_pos - grab_sz * 0.5f,
...
bool ImGui::SliderBehavior(const ImRect &bb, ImGuiID id,
			   ImGuiDataType data_type, void *p_v,
			   const void *p_min, const void *p_max,
			   const char *format, ImGuiSliderFlags flags,
			   ImRect *out_grab_bb)
{
	...
	IM_ASSERT(
	    (flags == 1 || (flags & ImGuiSliderFlags_InvalidMask_) == 0) &&
	    "Invalid ImGuiSliderFlags flag!  Has the 'float power' argument "
	    "been mistakenly cast to flags? Call function with "
	    "ImGuiSliderFlags_Logarithmic flags instead.");
	...
	case ImGuiDataType_S8: {
		...
		bool r = SliderBehaviorT<ImS32, ImS32, float>(
		    bb, id, ImGuiDataType_S32, &v32, *(const ImS8 *)p_min,
	...
	case ImGuiDataType_U8: {
		...
		bool r = SliderBehaviorT<ImU32, ImS32, float>(
		    bb, id, ImGuiDataType_U32, &v32, *(const ImU8 *)p_min,
	...
	case ImGuiDataType_S16: {
		ImS32 v32 = (ImS32) * (ImS16 *)p_v;
		bool r = SliderBehaviorT<ImS32, ImS32, float>(
		    bb, id, ImGuiDataType_S32, &v32, *(const ImS16 *)p_min,
	...
	case ImGuiDataType_U16: {
		ImU32 v32 = (ImU32) * (ImU16 *)p_v;
		bool r = SliderBehaviorT<ImU32, ImS32, float>(
		    bb, id, ImGuiDataType_U32, &v32, *(const ImU16 *)p_min,
			 ...
			 const char *format, ImGuiSliderFlags flags)
{
	...
	ImRect grab_bb;
	const bool value_changed =
	    SliderBehavior(frame_bb, id, data_type, p_data, p_min, p_max,
			   format, flags, &grab_bb);
	if (value_changed)
		MarkItemEdited(id);
...
bool ImGui::SliderFloat(const char *label, float *v, float v_min, float v_max,
			const char *format, ImGuiSliderFlags flags)
{
	return SliderScalar(label, ImGuiDataType_Float, v, &v_min, &v_max,
			    format, flags);
}
...
bool ImGui::SliderFloat2(const char *label, float v[2], float v_min,
			 ...
			 ImGuiSliderFlags flags)
{
	return SliderScalarN(label, ImGuiDataType_Float, v, 2, &v_min, &v_max,
			     format, flags);
}
...
bool ImGui::SliderFloat3(const char *label, float v[3], float v_min,
			 ...
			 ImGuiSliderFlags flags)
{
	return SliderScalarN(label, ImGuiDataType_Float, v, 3, &v_min, &v_max,
			     format, flags);
}
...
bool ImGui::SliderFloat4(const char *label, float v[4], float v_min,
			 ...
			 ImGuiSliderFlags flags)
{
	return SliderScalarN(label, ImGuiDataType_Float, v, 4, &v_min, &v_max,
			     format, flags);
}
...
bool ImGui::SliderInt2(const char *label, int v[2], int v_min, int v_max,
		       const char *format, ImGuiSliderFlags flags)
{
	return SliderScalarN(label, ImGuiDataType_S32, v, 2, &v_min, &v_max,
			     format, flags);
}
...
bool ImGui::SliderInt3(const char *label, int v[3], int v_min, int v_max,
		       const char *format, ImGuiSliderFlags flags)
{
	return SliderScalarN(label, ImGuiDataType_S32, v, 3, &v_min, &v_max,
			     format, flags);
}
...
bool ImGui::SliderInt4(const char *label, int v[4], int v_min, int v_max,
		       const char *format, ImGuiSliderFlags flags)
{
	return SliderScalarN(label, ImGuiDataType_S32, v, 4, &v_min, &v_max,
			     format, flags);
}

bool ImGui::VSliderScalar(const char *label, const ImVec2 &size,
			  ImGuiDataType data_type, void *p_data,
			  const void *p_min, const void *p_max,
			  const char *format, ImGuiSliderFlags flags)
{
	...
	const bool value_changed =
	    SliderBehavior(frame_bb, id, data_type, p_data, p_min, p_max,
			   format, flags | ImGuiSliderFlags_Vertical, &grab_bb);
	if (value_changed)
		MarkItemEdited(id);
...
bool ImGui::VSliderFloat(const char *label, const ImVec2 &size, float *v,
			 ...
			 ImGuiSliderFlags flags)
{
	return VSliderScalar(label, size, ImGuiDataType_Float, v, &v_min,
			     &v_max, format, flags);
}
...
bool ImGui::VSliderInt(const char *label, const ImVec2 &size, int *v, int v_min,
		       int v_max, const char *format, ImGuiSliderFlags flags)
{
	return VSliderScalar(label, size, ImGuiDataType_S32, v, &v_min, &v_max,
			     format, flags);
}
			 ...
			 const char *format, float power)
{
	...
	return SliderScalar(label, data_type, p_data, p_min, p_max, format,
			    slider_flags);
}
			  ...
			  const char *format, float power)
{
	...
	return SliderScalarN(label, data_type, v, components, v_min, v_max,
			     format, slider_flags);
}
...
{
	// Printf/scanf types modifiers: I/L/h/j/l/t/w/z. Other uppercase
	// letters qualify as types aka end of the format.
	if (fmt[0] != '%')
		return fmt;
...
const char *ImParseFormatTrimDecorations(const char *fmt, char *buf,
					 size_t buf_size)
{
	...
	if (fmt_end[0] == 0) // If we only have leading decoration, we don't
			     // need to copy the data.
		return fmt_start;
	ImStrncpy(buf, fmt_start,
		  ImMin((size_t)(fmt_end - fmt_start) + 1, buf_size));
	return buf;
}
...
int ImParseFormatPrecision(const char *fmt, int default_precision)
{
	...
	if (*fmt == 'e' ||
	    *fmt == 'E') // Maximum precision with scientific notation
		precision = -1;
...
bool ImGui::TempInputText(const ImRect &bb, ImGuiID id, const char *label,
			  char *buf, int buf_size, ImGuiInputTextFlags flags)
{
	// On the first frame, g.TempInputTextId == 0, then on subsequent frames
	// it becomes == id. We clear ActiveID on the first frame to allow the
	...
	bool value_changed =
	    InputTextEx(label, NULL, buf, buf_size, bb.GetSize(), flags);
	if (init) {
		// First frame we started displaying the InputText widget, we
		// expect it to take the active id.
...
// clamping values if the ImGuiSliderFlags_AlwaysClamp flag is set! This is
			    ...
			    const void *p_clamp_max)
{
	...
	char fmt_buf[32];
	char data_buf[32];
	format = ImParseFormatTrimDecorations(format, fmt_buf,
					      IM_ARRAYSIZE(fmt_buf));
	DataTypeFormatString(data_buf, IM_ARRAYSIZE(data_buf), data_type,
			     p_data, format);
	ImStrTrimBlanks(data_buf);

	ImGuiInputTextFlags flags = ImGuiInputTextFlags_AutoSelectAll |
				    ImGuiInputTextFlags_NoMarkEdited;
	flags |= ((data_type == ImGuiDataType_Float ||
		   data_type == ImGuiDataType_Double)
		      ? ImGuiInputTextFlags_CharsScientific
		      : ImGuiInputTextFlags_CharsDecimal);
	bool value_changed = false;
	if (TempInputText(bb, id, label, data_buf, IM_ARRAYSIZE(data_buf),
			  flags)) {
		// Backup old value
		...
		DataTypeApplyOpFromText(data_buf,
					g.InputTextState.InitialTextA.Data,
					data_type, p_data, NULL);
		if (p_clamp_min || p_clamp_max) {
			if (DataTypeCompare(data_type, p_clamp_min,
					    p_clamp_max) > 0)
				ImSwap(p_clamp_min, p_clamp_max);
			DataTypeClamp(data_type, p_data, p_clamp_min,
				      p_clamp_max);
		}

		// Only mark as edited if new value is different
		value_changed =
		    memcmp(&data_backup, p_data, data_type_size) != 0;
		if (value_changed)
			MarkItemEdited(id);
	}
	return value_changed;
...
bool ImGui::InputScalar(const char *label, ImGuiDataType data_type,
			...
			ImGuiInputTextFlags flags)
{
	...
	flags |= ImGuiInputTextFlags_NoMarkEdited; // We call MarkItemEdited()
						   // ourselves by comparing the
						   // actual data rather than
						   // the string.

	if (p_step != NULL) {
		const float button_size = GetFrameHeight();

		BeginGroup(); // The only purpose of the group here is to allow
			      // the caller to query item data e.g.
			      // IsItemActive()
		PushID(label);
		...
		if (InputText("", buf, IM_ARRAYSIZE(buf),
			      flags)) // PushId(label) + "" gives us the
				      // expected ID from outside point of view
			value_changed = DataTypeApplyOpFromText(
			    buf, g.InputTextState.InitialTextA.Data, data_type,
			    p_data, format);
...
bool ImGui::InputScalarN(const char *label, ImGuiDataType data_type,
			 ...
			 ImGuiInputTextFlags flags)
{
	...
	for (int i = 0; i < components; i++) {
		...
		if (i > 0)
			SameLine(0, g.Style.ItemInnerSpacing.x);
		value_changed |= InputScalar("", data_type, p_data, p_step,
...
bool ImGui::InputFloat2(const char *label, float v[2], const char *format,
			ImGuiInputTextFlags flags)
{
	return InputScalarN(label, ImGuiDataType_Float, v, 2, NULL, NULL,
			    format, flags);
}
...
bool ImGui::InputFloat3(const char *label, float v[3], const char *format,
			ImGuiInputTextFlags flags)
{
	return InputScalarN(label, ImGuiDataType_Float, v, 3, NULL, NULL,
			    format, flags);
}
...
bool ImGui::InputFloat4(const char *label, float v[4], const char *format,
			ImGuiInputTextFlags flags)
{
	return InputScalarN(label, ImGuiDataType_Float, v, 4, NULL, NULL,
			    format, flags);
}
		     ...
		     ImGuiInputTextFlags flags)
{
	...
	const char *format =
	    (flags & ImGuiInputTextFlags_CharsHexadecimal) ? "%08X" : "%d";
	return InputScalar(label, ImGuiDataType_S32, (void *)v,
			   (void *)(step > 0 ? &step : NULL),
			   (void *)(step_fast > 0 ? &step_fast : NULL), format,
			   flags);
}

bool ImGui::InputInt2(const char *label, int v[2], ImGuiInputTextFlags flags)
{
	return InputScalarN(label, ImGuiDataType_S32, v, 2, NULL, NULL, "%d",
			    flags);
}

bool ImGui::InputInt3(const char *label, int v[3], ImGuiInputTextFlags flags)
{
	return InputScalarN(label, ImGuiDataType_S32, v, 3, NULL, NULL, "%d",
			    flags);
}

bool ImGui::InputInt4(const char *label, int v[4], ImGuiInputTextFlags flags)
{
	return InputScalarN(label, ImGuiDataType_S32, v, 4, NULL, NULL, "%d",
			    flags);
}

bool ImGui::InputDouble(const char *label, double *v, double step,
		      ...
		      ImGuiInputTextCallback callback, void *user_data)
{
	IM_ASSERT(
	    !(flags &
	      ImGuiInputTextFlags_Multiline)); // call InputTextMultiline()
	return InputTextEx(label, NULL, buf, (int)buf_size, ImVec2(0, 0), flags,
			   callback, user_data);
}
...
bool ImGui::InputTextWithHint(const char *label, const char *hint, char *buf,
			      ...
			      ImGuiInputTextCallback callback, void *user_data)
{
	IM_ASSERT(
	    !(flags &
	      ImGuiInputTextFlags_Multiline)); // call InputTextMultiline()
	return InputTextEx(label, hint, buf, (int)buf_size, ImVec2(0, 0), flags,
			   callback, user_data);
}
				     ...
				     ImVec2 *out_offset, bool stop_on_new_line)
{
	...
	while (s < text_end) {
		...
		const float char_width =
		    font->GetCharAdvance((ImWchar)c) * scale;
	...
	if (out_offset)
		*out_offset =
		    ImVec2(line_width,
			   text_size.y +
			       line_height); // offset allow for the possibility
				   ...
				   int line_start_idx)
{
	...
	const ImVec2 size =
	    InputTextCalcTextSizeW(text + line_start_idx, text + obj->CurLenW,
...
#define STB_TEXTEDIT_MOVEWORDLEFT                                              \
	STB_TEXTEDIT_MOVEWORDLEFT_IMPL // They need to be #define for
				       // stb_textedit.h
...
static bool STB_TEXTEDIT_INSERTCHARS(STB_TEXTEDIT_STRING *obj, int pos,
				     const ImWchar *new_text, int new_text_len)
{
	...
	if (new_text_len + text_len + 1 > obj->TextW.Size) {
		...
		obj->TextW.resize(
		    text_len +
		    ImClamp(new_text_len * 4, 32, ImMax(256, new_text_len)) +
		    1);
	}
...
#define STB_TEXTEDIT_K_BACKSPACE                                               \
	0x200009 // keyboard input to delete selection or character left of
		 // cursor
...
// deletion, but somehow, calling the stb_textedit_paste() function creates two
					     ...
					     const char *new_text_end)
{
	...
	if (new_text_len + BufTextLen >= BufSize) {
		...
		// Contrary to STB_TEXTEDIT_INSERTCHARS() this is working in the
		// UTF8 buffer, hence the mildly similar code (until we remove
		// the U16 buffer altogether!)
		ImGuiContext &g = *GImGui;
		...
		int new_buf_size =
		    BufTextLen +
		    ImClamp(new_text_len * 4, 32, ImMax(256, new_text_len)) + 1;
	...
	if (BufTextLen != pos)
		memmove(Buf + pos + new_text_len, Buf + pos,
				     ...
				     void *user_data)
{
	...
	if (c < 0x20) {
		...
		pass |=
		    (c == '\t' && (flags & ImGuiInputTextFlags_AllowTabInput));
		if (!pass)
			return false;
	}

	// We ignore Ascii representation of delete (emitted from Backspace on
	// OSX, see #2578, #2817)
	if (c == 127)
		return false;
	...
	if (flags & (ImGuiInputTextFlags_CharsDecimal |
		     ...
		     ImGuiInputTextFlags_CharsScientific)) {
		// The libc allows overriding locale, with e.g.
		// 'setlocale(LC_NUMERIC, "de_DE.UTF-8");' which affect the
		// output/input of printf/scanf. The standard mandate that
		// programs starts in the "C" locale where the decimal point is
		// '.'. We don't really intend to provide widespread support for
		// it, but out of empathy for people stuck with using odd API,
		// we support the bare minimum aka overriding the decimal point.
...
// misc/cpp/imgui_stdlib.h (FIXME: Rather confusing and messy function, among
...
bool ImGui::InputTextEx(const char *label, const char *hint, char *buf,
			int buf_size, const ImVec2 &size_arg,
			ImGuiInputTextFlags flags,
			ImGuiInputTextCallback callback,
			void *callback_user_data)
{
	...
	if (is_resizable)
		IM_ASSERT(callback !=
			  NULL); // Must provide a callback if you set the
				 // ImGuiInputTextFlags_CallbackResize flag!

	if (is_multiline) // Open group before calling GetID() because groups
			  // tracks id created within their scope,
		BeginGroup();
	const ImGuiID id = window->GetID(label);
	const ImVec2 label_size = CalcTextSize(label, NULL, true);
	...
	const ImVec2 total_size =
	    ImVec2(frame_size.x + (label_size.x > 0.0f
				       ? style.ItemInnerSpacing.x + label_size.x
				       : 0.0f),
		   frame_size.y);
	...
	if (is_multiline) {
		...
		// We reproduce the contents of BeginChildFrame() in order to
		// provide 'label' so our window internal data are easier to
		// read/debug.
		...
		bool child_visible =
		    BeginChildEx(label, id, frame_bb.GetSize(), true,
		...
		draw_window->DC.NavLayerActiveMaskNext |=
		    (1 << draw_window->DC
			      .NavLayerCurrent); // This is to ensure that
						 // EndChild() will display a
						 // navigation highlight so we
						 // can "enter" into it.
	...
	bool select_all = (g.ActiveId != id) &&
			  ((flags & ImGuiInputTextFlags_AutoSelectAll) != 0 ||
	...
	const bool init_changed_specs =
	    (state != NULL && state->Stb.single_line != !is_multiline);
	const bool init_make_active =
	    (focus_requested || user_clicked || user_scroll_finish ||
	     user_nav_input_start);
	...
	if ((init_state && g.ActiveId != id) || init_changed_specs) {
		...
		// UTF-8 format and converted to wchar) From the moment we
		// focused we are ignoring the content of 'buf' (unless we are
		// in read-only mode)
		const int buf_len = (int)strlen(buf);
		state->InitialTextA.resize(
		    buf_len +
		    1); // UTF-8. we use +1 to make sure that .Data is always
		...
		state->TextW.resize(buf_size +
				    1); // wchar count <= UTF-8 count. we use +1
					// to make sure that .Data is always
					// pointing to at least an empty string.
		state->TextA.resize(0);
		state->TextAIsValid = false; // TextA is not valid yet (we will
					     // display buf until then)
		state->CurLenW = ImTextStrFromUtf8(state->TextW.Data, buf_size,
						   buf, NULL, &buf_end);
		state->CurLenA =
		    (int)(buf_end -
			  buf); // We can't get the result from ImStrncpy()
				// above because it is not UTF-8 aware. Here
				// we'll cut off malformed UTF-8.

		// Preserve cursor position and undo/redo stack if we come back
		// to same widget
		// FIXME: For non-readonly widgets we might be able to require
		// that TextAIsValid && TextA == buf ? (untested) and discard
		// undo stack if user buffer has changed.
		const bool recycle_state =
		    (state->ID == id && !init_changed_specs);
		if (recycle_state) {
			// Recycle existing cursor/selection/undo stack but
			// clamp position Note a single mouse click will
			// override the cursor/position immediately by calling
			// stb_textedit_click handler.
			state->CursorClamp();
		} else {
			state->ID = id;
			state->ScrollX = 0.0f;
			stb_textedit_initialize_state(&state->Stb,
						      !is_multiline);
	...
	if (g.ActiveId != id && init_make_active) {
		...
		g.ActiveIdUsingKeyInputMask |=
		    ((ImU64)1 << ImGuiKey_Home) | ((ImU64)1 << ImGuiKey_End);
		if (is_multiline)
			g.ActiveIdUsingKeyInputMask |=
			    ((ImU64)1 << ImGuiKey_PageUp) |
			    ((ImU64)1 << ImGuiKey_PageDown);
		if (flags &
		    (ImGuiInputTextFlags_CallbackCompletion |
		     ImGuiInputTextFlags_AllowTabInput)) // Disable keyboard
							 // tabbing out as we
							 // will use the \t
							 // character.
			g.ActiveIdUsingKeyInputMask |=
			    ((ImU64)1 << ImGuiKey_Tab);
	}
	...
	const bool render_cursor =
	    (g.ActiveId == id) || (state && user_scroll_active);
	...
	// FIXME-OPT: Because our selection/cursor code currently needs the wide
	// text we need to convert it when active, which is not ideal :(
	if (is_readonly && state != NULL &&
	    (render_cursor || render_selection)) {
		const char *buf_end = NULL;
		state->TextW.resize(buf_size + 1);
		state->CurLenW = ImTextStrFromUtf8(
		    state->TextW.Data, state->TextW.Size, buf, NULL, &buf_end);
	...
	const bool buf_display_from_state =
	    (render_cursor || render_selection || g.ActiveId == id) &&
	...
	if (g.ActiveId == id) {
		...
		// Although we are active we don't prevent mouse from hovering
		// other elements unless we are interacting right now with the
		// widget. Down the line we should have a cleaner library-wide
		...
		const float mouse_y =
		    (is_multiline
			 ? (io.MousePos.y - draw_window->DC.CursorPos.y -
			    style.FramePadding.y)
			 : (g.FontSize * 0.5f));

		const bool is_osx = io.ConfigMacOSXBehaviors;
		if (select_all ||
		    (hovered && !is_osx && io.MouseDoubleClicked[0])) {
			state->SelectAll();
			state->SelectedAllMouseLock = true;
		} else if (hovered && is_osx && io.MouseDoubleClicked[0]) {
			// Double-click select a word only, OS X style (by
			...
			if (hovered) {
				stb_textedit_click(state, &state->Stb, mouse_x,
						   mouse_y);
				state->CursorAnimReset();
			}
		...
		// It is ill-defined whether the backend needs to send a \t
		// character when pressing the TAB keys. Win32 and GLFW
		// naturally do it but not SDL.
		const bool ignore_char_inputs =
		    (io.KeyCtrl && !io.KeyAlt) || (is_osx && io.KeySuper);
		if ((flags & ImGuiInputTextFlags_AllowTabInput) &&
		    IsKeyPressedMap(ImGuiKey_Tab) && !ignore_char_inputs &&
		...
		// Process regular text input (before we check for Return
		// because using some IME will effectively send a Return?) We
		// ignore CTRL inputs, but need to allow ALT+CTRL as some
		// keyboards (e.g. German) use AltGR (which _is_ Alt+Ctrl) to
		// input certain characters.
		if (io.InputQueueCharacters.Size > 0) {
			if (!ignore_char_inputs && !is_readonly &&
			    !user_nav_input_start)
				for (int n = 0;
				     n < io.InputQueueCharacters.Size; n++) {
					// Insert character if they pass
					// filtering
					unsigned int c =
					    (unsigned int)
						io.InputQueueCharacters[n];
					if (c == '\t' && io.KeyShift)
						continue;
					if (InputTextFilterCharacter(
						&c, flags, callback,
	...
	if (g.ActiveId == id && !g.ActiveIdIsJustActivated &&
		...
		IM_ASSERT(
		    ...
		    "Mismatching io.KeyCtrl/io.KeyShift/io.KeyAlt/io.KeySuper "
		    "vs io.KeyMods"); // We rarely do this check, but if
		...
		const bool is_osx_shift_shortcut =
		    is_osx && (io.KeyMods == (ImGuiKeyModFlags_Super |
		...
		if (IsKeyPressedMap(ImGuiKey_LeftArrow)) {
			...
			state->OnKeyPressed(
			    (is_startend_key_down   ? STB_TEXTEDIT_K_LINEEND
			...
			state->OnKeyPressed(
			    io.KeyCtrl ? STB_TEXTEDIT_K_TEXTSTART | k_mask
				       : STB_TEXTEDIT_K_LINESTART | k_mask);
		} else if (IsKeyPressedMap(ImGuiKey_End)) {
			state->OnKeyPressed(
			    io.KeyCtrl ? STB_TEXTEDIT_K_TEXTEND | k_mask
			...
			if (!state->HasSelection()) {
				if (is_wordmove_key_down)
					state->OnKeyPressed(
					    ...
					    STB_TEXTEDIT_K_SHIFT);
				else if (is_osx && io.KeySuper && !io.KeyAlt &&
					 !io.KeyCtrl)
					state->OnKeyPressed(
			...
			state->OnKeyPressed(STB_TEXTEDIT_K_BACKSPACE | k_mask);
			   ...
			   IsKeyPressedMap(ImGuiKey_KeyPadEnter)) {
			...
			if (io.SetClipboardTextFn) {
				const int ib =
				    state->HasSelection()
					? ImMin(state->Stb.select_start,
						state->Stb.select_end)
					: 0;
				const int ie =
				    state->HasSelection()
					? ImMax(state->Stb.select_start,
						state->Stb.select_end)
					: state->CurLenW;
				const int clipboard_data_len =
					...
					state->TextW.Data + ie) +
				    1;
				char *clipboard_data = (char *)IM_ALLOC(
				    clipboard_data_len * sizeof(char));
				ImTextStrToUtf8(clipboard_data,
						clipboard_data_len,
						state->TextW.Data + ib,
						state->TextW.Data + ie);
			...
			if (const char *clipboard = GetClipboardText()) {
				...
				ImWchar *clipboard_filtered =
				    (ImWchar *)IM_ALLOC((clipboard_len + 1) *
							sizeof(ImWchar));
				...
				if (clipboard_filtered_len >
				    0) // If everything was filtered, ignore the
				       // pasting operation
				{
					stb_textedit_paste(
					    state, &state->Stb,
		...
		// Update render selection flag after events have been handled,
		// so selection highlight can be displayed during the same
		// frame.
		render_selection |=
		    state->HasSelection() &&
		    (RENDER_SELECTION_WHEN_INACTIVE || render_cursor);
	...
	if (g.ActiveId == id) {
		...
		if (cancel_edit) {
			// Restore initial value. Only return true if restoring
			// to the initial value changes the current buffer
			// contents.
			if (!is_readonly &&
			    strcmp(buf, state->InitialTextA.Data) != 0) {
				// Push records into the undo stack so we can
				// CTRL+Z the revert operation itself
				apply_new_text = state->InitialTextA.Data;
				apply_new_text_length =
				    state->InitialTextA.Size - 1;
				...
				if (apply_new_text_length > 0) {
					w_text.resize(
					    ...
					    1);
					ImTextStrFromUtf8(
					    w_text.Data, w_text.Size,
					    apply_new_text,
					    apply_new_text +
						apply_new_text_length);
				}
				stb_textedit_replace(state, &state->Stb,
						     w_text.Data,
		...
		// When using 'ImGuiInputTextFlags_EnterReturnsTrue' as a
		// special case we reapply the live buffer back to the input
		// buffer before clearing ActiveId, even though strictly
		// speaking it wasn't modified on this frame. If we didn't do
		// that, code like InputInt() with
		// ImGuiInputTextFlags_EnterReturnsTrue would fail. This also
		// allows the user to use InputText() with
		// ImGuiInputTextFlags_EnterReturnsTrue without maintaining any
		// user-side storage (please note that if you use this property
		// along ImGuiInputTextFlags_CallbackResize you can end up with
		// your temporary string object unnecessarily allocating once a
		// frame, either store your string data, either if you don't
		// then don't use ImGuiInputTextFlags_CallbackResize).
		bool apply_edit_back_to_user_buffer =
		    !cancel_edit ||
		    (enter_pressed &&
		     (flags & ImGuiInputTextFlags_EnterReturnsTrue) != 0);
		if (apply_edit_back_to_user_buffer) {
			// Apply new value immediately - copy modified buffer
			// back Note that as soon as the input box is active,
			// the in-widget value gets priority over any underlying
			// modification of the input buffer
			// FIXME: We actually always render 'buf' when calling
			// DrawList->AddText, making the comment above
			// incorrect.
			// FIXME-OPT: CPU waste to do this every time the widget
			// is active, should mark dirty state from the
			// stb_textedit callbacks.
			if (!is_readonly) {
				state->TextAIsValid = true;
				state->TextA.resize(state->TextW.Size * 4 + 1);
				ImTextStrToUtf8(state->TextA.Data,
						state->TextA.Size,
						state->TextW.Data, NULL);
			}
			...
			if ((flags & (ImGuiInputTextFlags_CallbackCompletion |
				...
				// (Completion/History) is that Completion needs
				...
				if (event_flag) {
					...
					callback_data.BufTextLen =
					    state->CurLenA;
					callback_data.BufSize =
					    state->BufCapacityA;
					callback_data.BufDirty = false;

					// We have to convert from
					// wchar-positions to UTF-8-positions,
					// which can be pretty slow (an
					// incentive to ditch the ImWchar
					// buffer, see
					// https://github.com/nothings/stb/issues/188)
					...
					IM_ASSERT(
					    callback_data.Buf ==
					    state->TextA
						.Data); // Invalid to modify
							// those fields
					IM_ASSERT(callback_data.BufSize ==
						  state->BufCapacityA);
					IM_ASSERT(callback_data.Flags == flags);
					const bool buf_dirty =
					    callback_data.BufDirty;
					if (callback_data.CursorPos !=
						utf8_cursor_pos ||
					    buf_dirty) {
						state->Stb.cursor =
						    ImTextCountCharsFromUtf8(
					...
					if (callback_data.SelectionStart !=
		...
		if (apply_new_text) {
			...
			// apply_new_text_length' here because we have no
			// guarantee that the size of our owned buffer matches
			// the size of the string object held by the user, and
			// by design we allow InputText() to be used without any
			...
			if (is_resizable) {
				...
				apply_new_text_length = ImMin(
				    callback_data.BufTextLen, buf_size - 1);
				IM_ASSERT(apply_new_text_length <= buf_size);
			}
			// IMGUI_DEBUG_LOG("InputText(\"%s\"): apply_new_text
			// length %d\n", label, apply_new_text_length);

			// If the underlying buffer resize was denied or not
			// carried to the next frame, apply_new_text_length+1
			// may be >= buf_size.
			ImStrncpy(buf, apply_new_text,
				  ImMin(apply_new_text_length + 1, buf_size));
			value_changed = true;
		}
	...
	// Release active ID at the end of the function (so e.g. pressing Return
	// still does a final application of the value)
	...
	if (!is_multiline) {
		...
		RenderFrame(frame_bb.Min, frame_bb.Max,
			    GetColorU32(ImGuiCol_FrameBg), true,
			    style.FrameRounding);
	}
	...
	const ImVec4 clip_rect(
	    ...
	    frame_bb.Min.y +
		inner_size
		    .y); // Not using frame_bb.Max because we have adjusted size
	...
	// strings. The current pathological worst case is a long line without
	// any carriage return, which would makes ImFont::RenderText() reserve
	// too many vertices and probably crash. Avoid it altogether. Note that
	// we only use this limit on single-line InputText(), so a
	// pathologically large line on a InputTextMultiline() would still
	// crash.
	...
	// active or while scrolling.
	// FIXME: We could remove the '&& render_cursor' to keep rendering
	// selection when inactive.
	if (render_cursor || render_selection) {
		...
		// - Handle scrolling, highlight selection, display cursor
		// (those all requires some form of 1d->2d cursor position
		// calculation)
		// - Measure text height (for scrollbar)
		// We are attempting to do most of that in **one main pass** to
		// minimize the computation cost (non-negligible for large
		// amount of text) + 2nd pass for selection rendering (we could
		// merge them by an extra refactoring effort)
		// FIXME: This should occur on buf_display but we'd need to
		// maintain cursor/select_start/select_end for UTF-8.
		const ImWchar *text_begin = state->TextW.Data;
		ImVec2 cursor_offset, select_start_offset;

		{
			// Find lines numbers straddling 'cursor' (slot 0) and
			// 'select_start' (slot 1) positions.
			...
			if (render_selection) {
				searches_input_ptr[1] =
				    text_begin + ImMin(state->Stb.select_start,
						       state->Stb.select_end);
			...
			// In multi-line mode, we never exit the loop until all
			// lines are counted, so add one extra to the
			// searches_remaining counter.
			searches_remaining += is_multiline ? 1 : 0;
			int line_count = 0;
			// for (const ImWchar* s = text_begin; (s = (const
			// ImWchar*)wcschr((const wchar_t*)s, (wchar_t)'\n')) !=
			// NULL; s++)  // FIXME-OPT: Could use this when wchar_t
			// are 16-bit
			...
			// Calculate 2d position by finding the beginning of the
			// line and measuring distance
			cursor_offset.x =
			    InputTextCalcTextSizeW(
				ImStrbolW(searches_input_ptr[0], text_begin),
				searches_input_ptr[0])
				.x;
			cursor_offset.y =
			    searches_result_line_no[0] * g.FontSize;
			if (searches_result_line_no[1] >= 0) {
				select_start_offset.x =
				    InputTextCalcTextSizeW(
					ImStrbolW(searches_input_ptr[1],
						  text_begin),
					searches_input_ptr[1])
					.x;
				select_start_offset.y =
				    searches_result_line_no[1] * g.FontSize;
			}

			// Store text height (note that we haven't calculated
			// text width at all, see GitHub issues #383, #1224)
			if (is_multiline)
				text_size = ImVec2(inner_size.x,
						   line_count * g.FontSize);
		}
		...
		if (render_cursor && state->CursorFollow) {
			...
			if (!(flags & ImGuiInputTextFlags_NoHorizontalScroll)) {
				...
				if (cursor_offset.x < state->ScrollX)
					state->ScrollX = IM_FLOOR(ImMax(
					    0.0f, cursor_offset.x -
						      scroll_increment_x));
				else if (cursor_offset.x - inner_size.x >=
					 state->ScrollX)
					state->ScrollX = IM_FLOOR(
					    cursor_offset.x - inner_size.x +
					    scroll_increment_x);
			} else {
				state->ScrollX = 0.0f;
			}
			...
			if (is_multiline) {
				...
				if (cursor_offset.y - g.FontSize < scroll_y)
					scroll_y = ImMax(0.0f, cursor_offset.y -
								   g.FontSize);
				else if (cursor_offset.y - inner_size.y >=
					 scroll_y)
					scroll_y = cursor_offset.y -
						   inner_size.y +
				...
				const float scroll_max_y =
				    ImMax((text_size.y +
					  ...
					  0.0f);
				scroll_y =
				    ImClamp(scroll_y, 0.0f, scroll_max_y);
				draw_pos.y +=
				    (draw_window->Scroll.y -
				     scroll_y); // Manipulate cursor pos
						// immediately avoid a frame of
						// lag
		...
		if (render_selection) {
			...
			const ImWchar *text_selected_end =
			    text_begin + ImMax(state->Stb.select_start,
					       state->Stb.select_end);
			...
			ImU32 bg_color = GetColorU32(
			    ...
			    render_cursor
				? 1.0f
				: 0.6f); // FIXME: current code flow mandate
					 // that render_cursor is always true
					 // here, we are leaving the transparent
					 // one for tests.
			float bg_offy_up =
			    is_multiline
				? 0.0f
				: -1.0f; // FIXME: those offsets should be part
					 // of the style? they don't play so
					 // well with multi-line selection.
			float bg_offy_dn = is_multiline ? 0.0f : 2.0f;
			ImVec2 rect_pos =
			    draw_pos + select_start_offset - draw_scroll;
			for (const ImWchar *p = text_selected_begin;
			     p < text_selected_end;) {
				if (rect_pos.y > clip_rect.w + g.FontSize)
					break;
				if (rect_pos.y < clip_rect.y) {
					// p = (const ImWchar*)wmemchr((const
					// wchar_t*)p, '\n', text_selected_end -
					// p);  // FIXME-OPT: Could use this
					// when wchar_t are 16-bit p = p ? p + 1
					// : text_selected_end;
					...
					ImRect rect(
					    rect_pos +
						ImVec2(0.0f,
						       bg_offy_up - g.FontSize),
					    rect_pos + ImVec2(rect_size.x,
							      bg_offy_dn));
					rect.ClipWith(clip_rect);
					if (rect.Overlaps(clip_rect))
						draw_window->DrawList
						    ->AddRectFilled(rect.Min,
								    rect.Max,
								    bg_color);
				}
		...
		// We test for 'buf_display_max_length' as a way to avoid some
		...
		if (is_multiline ||
			...
			ImU32 col = GetColorU32(is_displaying_hint
						    ? ImGuiCol_TextDisabled
						    : ImGuiCol_Text);
			draw_window->DrawList->AddText(
			    g.Font, g.FontSize, draw_pos - draw_scroll, col,
		...
		if (render_cursor) {
			...
			if (cursor_is_visible &&
			    cursor_screen_rect.Overlaps(clip_rect))
				draw_window->DrawList->AddLine(
				    cursor_screen_rect.Min,
				    cursor_screen_rect.GetBL(),
				    GetColorU32(ImGuiCol_Text));

			// Notify OS of text input position for advanced IME (-1
			// x offset so that Windows IME can cover our cursor.
			// Bit of an extra nicety.)
			if (!is_readonly)
				g.PlatformImePos =
				    ImVec2(cursor_screen_pos.x - 1.0f,
		...
		if (is_multiline ||
			...
			ImU32 col = GetColorU32(is_displaying_hint
						    ...
						    : ImGuiCol_Text);
			draw_window->DrawList->AddText(
			    g.Font, g.FontSize, draw_pos, col, buf_display,
			    buf_display_end, 0.0f,
			    is_multiline ? NULL : &clip_rect);
		}
	}
...
// See enum ImGuiColorEditFlags_ for available options. e.g. Only access 3
// floats if ImGuiColorEditFlags_NoAlpha flag is set. With typical options:
...
bool ImGui::ColorEdit4(const char *label, float col[4],
		       ImGuiColorEditFlags flags)
{
	ImGuiWindow *window = GetCurrentWindow();
	if (window->SkipItems)
	...
	if (flags & ImGuiColorEditFlags_NoInputs)
		flags = (flags & (~ImGuiColorEditFlags__DisplayMask)) |
			ImGuiColorEditFlags_DisplayRGB |
			ImGuiColorEditFlags_NoOptions;

	// Context menu: display and modify options (before defaults are
	// applied)
	...
	if (!(flags & ImGuiColorEditFlags__DisplayMask))
		flags |=
		    (g.ColorEditOptions & ImGuiColorEditFlags__DisplayMask);
	...
	IM_ASSERT(ImIsPowerOfTwo(
	    flags &
	    ImGuiColorEditFlags__DisplayMask)); // Check that only 1 is selected
	IM_ASSERT(ImIsPowerOfTwo(
	    flags &
	    ImGuiColorEditFlags__InputMask)); // Check that only 1 is selected
	...
	if ((flags & ImGuiColorEditFlags_InputHSV) &&
		...
		// Hue is lost when converting from greyscale rgb
		// (saturation=0). Restore it.
	...
	if ((flags & (ImGuiColorEditFlags_DisplayRGB |
		...
		const float w_item_one = ImMax(
		    1.0f, IM_FLOOR((w_inputs - (style.ItemInnerSpacing.x) *
						   (components - 1)) /
				   (float)components));
		const float w_item_last =
		    ImMax(1.0f, IM_FLOOR(w_inputs - (w_item_one +
						     style.ItemInnerSpacing.x) *
							(components - 1)));
		...
		static const char *fmt_table_int[3][4] = {
		    ...
		    {"R:%3d", "G:%3d", "B:%3d",
		     "A:%3d"}, // Long display for RGBA
		    {"H:%3d", "S:%3d", "V:%3d",
		     "A:%3d"} // Long display for HSVA
		};
		static const char *fmt_table_float[3][4] = {
		    {"%0.3f", "%0.3f", "%0.3f", "%0.3f"}, // Short display
		    {"R:%0.3f", "G:%0.3f", "B:%0.3f",
		     "A:%0.3f"}, // Long display for RGBA
		    {"H:%0.3f", "S:%0.3f", "V:%0.3f",
		...
		for (int n = 0; n < components; n++) {
			if (n > 0)
				SameLine(0, style.ItemInnerSpacing.x);
			SetNextItemWidth((n + 1 < components) ? w_item_one
							      : w_item_last);

			// FIXME: When ImGuiColorEditFlags_HDR flag is passed HS
			// values snap in weird ways when SV values go below 0.
			if (flags & ImGuiColorEditFlags_Float) {
				value_changed |=
				    DragFloat(ids[n], &f[n], 1.0f / 255.0f,
					      0.0f, hdr ? 0.0f : 1.0f,
					      fmt_table_float[fmt_idx][n]);
				value_changed_as_float |= value_changed;
			} else {
				value_changed |= DragInt(
				    ids[n], &i[n], 1.0f, 0, hdr ? 0 : 255,
		...
		if (alpha)
			ImFormatString(
			    buf, IM_ARRAYSIZE(buf), "#%02X%02X%02X%02X",
			    ImClamp(i[0], 0, 255), ImClamp(i[1], 0, 255),
		...
		if (InputText("##Text", buf, IM_ARRAYSIZE(buf),
			...
			if (alpha)
				sscanf(
				    ...
				    (unsigned int *)&i[1],
				    (unsigned int *)&i[2],
				    (unsigned int *)&i[3]); // Treat at unsigned
							    // (%X is unsigned)
			else
				sscanf(p, "%02X%02X%02X", (unsigned int *)&i[0],
				       (unsigned int *)&i[1],
				       (unsigned int *)&i[2]);
		}
		if (!(flags & ImGuiColorEditFlags_NoOptions))
	...
	if (!(flags & ImGuiColorEditFlags_NoSmallPreview)) {
		...
		const ImVec4 col_v4(col[0], col[1], col[2],
				    alpha ? col[3] : 1.0f);
		if (ColorButton("##ColorButton", col_v4, flags)) {
			if (!(flags & ImGuiColorEditFlags_NoPicker)) {
				...
				OpenPopup("picker");
				SetNextWindowPos(
				    window->DC.LastItemRect.GetBL() +
				    ImVec2(-1, style.ItemSpacing.y));
			}
		}
		if (!(flags & ImGuiColorEditFlags_NoOptions))
			OpenPopupOnItemClick("context");
		...
		if (BeginPopup("picker")) {
			...
			SetNextItemWidth(square_sz *
					 12.0f); // Use 256 + bar sizes?
			value_changed |= ColorPicker4(
			    "##picker", col, picker_flags, &g.ColorPickerRef.x);
			EndPopup();
		}
	}
	...
	if (label != label_display_end &&
		...
		window->DC.CursorPos =
		    ImVec2(pos.x + text_offset_x, pos.y + style.FramePadding.y);
		TextEx(label, label_display_end);
	}
	...
	if (value_changed && picker_active_window == NULL) {
		...
		if ((flags & ImGuiColorEditFlags_DisplayHSV) &&
			...
			ColorConvertHSVtoRGB(f[0], f[1], f[2], f[0], f[1],
					     f[2]);
			memcpy(g.ColorEditLastColor, f, sizeof(float) * 3);
		}
		if ((flags & ImGuiColorEditFlags_DisplayRGB) &&
		    (flags & ImGuiColorEditFlags_InputHSV))
			ColorConvertRGBtoHSV(f[0], f[1], f[2], f[0], f[1],
					     f[2]);
	...
	if ((window->DC.LastItemStatusFlags &
		...
		if (accepted_drag_drop &&
		    (flags & ImGuiColorEditFlags_InputHSV))
			ColorConvertRGBtoHSV(col[0], col[1], col[2], col[0],
					     col[1], col[2]);
		EndDragDropTarget();
	}

	// When picker is being actively used, use its active id so
	// IsItemActive() will function on ColorEdit4().
...
bool ImGui::ColorPicker3(const char *label, float col[3],
			 ImGuiColorEditFlags flags)
{
...
static void RenderArrowsForVerticalBar(ImDrawList *draw_list, ImVec2 pos,
				       ImVec2 half_sz, float bar_w, float alpha)
{
	...
	ImGui::RenderArrowPointingAt(
	    draw_list, ImVec2(pos.x + half_sz.x, pos.y), half_sz,
	    ImGuiDir_Right, IM_COL32(255, 255, 255, alpha8));
	ImGui::RenderArrowPointingAt(
	    draw_list, ImVec2(pos.x + bar_w - half_sz.x - 1, pos.y),
	    ImVec2(half_sz.x + 2, half_sz.y + 1), ImGuiDir_Left,
	    IM_COL32(0, 0, 0, alpha8));
	ImGui::RenderArrowPointingAt(
	    draw_list, ImVec2(pos.x + bar_w - half_sz.x, pos.y), half_sz,
	    ImGuiDir_Left, IM_COL32(255, 255, 255, alpha8));
}

// Note: ColorPicker4() only accesses 3 floats if ImGuiColorEditFlags_NoAlpha
...
bool ImGui::ColorPicker4(const char *label, float col[4],
			 ImGuiColorEditFlags flags, const float *ref_col)
{
	...
	IM_ASSERT(ImIsPowerOfTwo(
	    flags &
	    ImGuiColorEditFlags__PickerMask)); // Check that only 1 is selected
	IM_ASSERT(ImIsPowerOfTwo(
	    flags &
	    ImGuiColorEditFlags__InputMask)); // Check that only 1 is selected
	if (!(flags & ImGuiColorEditFlags_NoOptions))
	...
	float bar0_pos_x =
	    picker_pos.x + sv_picker_size + style.ItemInnerSpacing.x;
	...
	// Note: the triangle is displayed rotated with triangle_pa pointing to
	// Hue, but most coordinates stays unrotated for logic.
	float triangle_r = wheel_r_inner - (int)(sv_picker_size * 0.027f);
	ImVec2 triangle_pa = ImVec2(triangle_r, 0.0f); // Hue point.
	ImVec2 triangle_pb =
	    ImVec2(triangle_r * -0.5f, triangle_r * -0.866025f); // Black point.
	ImVec2 triangle_pc =
	    ImVec2(triangle_r * -0.5f, triangle_r * +0.866025f); // White point.
	...
	if (flags & ImGuiColorEditFlags_PickerHueWheel) {
		...
		InvisibleButton("hsv", ImVec2(sv_picker_size +
						  style.ItemInnerSpacing.x +
						  bars_width,
					      sv_picker_size));
		if (IsItemActive()) {
			...
			if (ImTriangleContainsPoint(
				...
				ImRotate(initial_off, cos_hue_angle,
					 sin_hue_angle))) {
				// Interacting with SV triangle
				ImVec2 current_off_unrotated = ImRotate(
				    current_off, cos_hue_angle, sin_hue_angle);
				if (!ImTriangleContainsPoint(
					triangle_pa, triangle_pb, triangle_pc,
					current_off_unrotated))
					current_off_unrotated =
					    ImTriangleClosestPoint(
						triangle_pa, triangle_pb,
						triangle_pc,
						current_off_unrotated);
				float uu, vv, ww;
		...
		if (IsItemActive()) {
			S = ImSaturate((io.MousePos.x - picker_pos.x) /
				       (sv_picker_size - 1));
			V = 1.0f - ImSaturate((io.MousePos.y - picker_pos.y) /
					      (sv_picker_size - 1));
			value_changed = value_changed_sv = true;
		}
		if (!(flags & ImGuiColorEditFlags_NoOptions))
		...
		if (IsItemActive()) {
			H = ImSaturate((io.MousePos.y - picker_pos.y) /
				       (sv_picker_size - 1));
			value_changed = value_changed_h = true;
		}
	}
	...
	if (alpha_bar) {
		...
		if (IsItemActive()) {
			col[3] =
			    1.0f - ImSaturate((io.MousePos.y - picker_pos.y) /
					      (sv_picker_size - 1));
			value_changed = true;
		}
	}
	...
	if (!(flags & ImGuiColorEditFlags_NoSidePreview)) {
		...
		ImVec4 col_v4(col[0], col[1], col[2],
			      (flags & ImGuiColorEditFlags_NoAlpha) ? 1.0f
								    : col[3]);
		if ((flags & ImGuiColorEditFlags_NoLabel))
			Text("Current");
	...
	if ((flags & ImGuiColorEditFlags_NoInputs) == 0) {
		PushItemWidth((alpha_bar ? bar1_pos_x : bar0_pos_x) +
		...
		ImGuiColorEditFlags sub_flags_to_forward =
		    ...
		    ImGuiColorEditFlags_AlphaPreview |
		    ImGuiColorEditFlags_AlphaPreviewHalf;
		ImGuiColorEditFlags sub_flags = (flags & sub_flags_to_forward) |
						ImGuiColorEditFlags_NoPicker;
		if (flags & ImGuiColorEditFlags_DisplayRGB ||
		    (flags & ImGuiColorEditFlags__DisplayMask) == 0)
			if (ColorEdit4("##rgb", col,
				       sub_flags |
					   ImGuiColorEditFlags_DisplayRGB)) {
				// FIXME: Hackily differentiating using the
				// DragInt (ActiveId != 0 &&
				// !ActiveIdAllowOverlap) vs. using the
				// InputText or DropTarget. For the later we
				// don't want to run the hue-wrap canceling
				// code. If you are well versed in HSV picker
				// please provide your input! (See #2050)
		...
		if (flags & ImGuiColorEditFlags_DisplayHSV ||
		    (flags & ImGuiColorEditFlags__DisplayMask) == 0)
			value_changed |= ColorEdit4(
			    "##hsv", col,
			    sub_flags | ImGuiColorEditFlags_DisplayHSV);
		if (flags & ImGuiColorEditFlags_DisplayHex ||
		    (flags & ImGuiColorEditFlags__DisplayMask) == 0)
			value_changed |= ColorEdit4(
			    "##hex", col,
			    sub_flags | ImGuiColorEditFlags_DisplayHex);
		PopItemWidth();
	}
	...
	if (value_changed_fix_hue_wrap &&
		...
		ColorConvertRGBtoHSV(col[0], col[1], col[2], new_H, new_S,
				     new_V);
		if (new_H <= 0 && H > 0) {
			if (new_V <= 0 && V != new_V)
				ColorConvertHSVtoRGB(
				    H, S, new_V <= 0 ? V * 0.5f : new_V, col[0],
				    col[1], col[2]);
			else if (new_S <= 0)
				ColorConvertHSVtoRGB(
	...
	if (value_changed) {
		if (flags & ImGuiColorEditFlags_InputRGB) {
			...
			B = col[2];
			ColorConvertRGBtoHSV(R, G, B, H, S, V);
			if (memcmp(g.ColorEditLastColor, col,
				   sizeof(float) * 3) ==
			    0) // Fix local Hue as display below will use it
			       // immediately.
			{
				if (S == 0)
					H = g.ColorEditLastHue;
	...
	ColorConvertHSVtoRGB(H, 1, 1, hue_color_f.x, hue_color_f.y,
			     hue_color_f.z);
	ImU32 hue_color32 = ColorConvertFloat4ToU32(hue_color_f);
	ImU32 user_col32_striped_of_alpha = ColorConvertFloat4ToU32(
	    ImVec4(R, G, B, style.Alpha)); // Important: this is still including
					   // the main rendering/style alpha!!
	...
	if (flags & ImGuiColorEditFlags_PickerHueWheel) {
		// Render Hue Wheel
		const float aeps =
		    0.5f / wheel_r_outer; // Half a pixel arc length in radians
					  // (2pi cancels out).
		const int segment_per_arc = ImMax(4, (int)wheel_r_outer / 12);
		for (int n = 0; n < 6; n++) {
			const float a0 = (n) / 6.0f * 2.0f * IM_PI - aeps;
			const float a1 =
			    (n + 1.0f) / 6.0f * 2.0f * IM_PI + aeps;
			const int vert_start_idx = draw_list->VtxBuffer.Size;
			draw_list->PathArcTo(wheel_center,
					     (wheel_r_inner + wheel_r_outer) *
						 0.5f,
					     a0, a1, segment_per_arc);
			draw_list->PathStroke(col_white, false,
					      wheel_thickness);
		...
		ImVec2 hue_cursor_pos(
		    wheel_center.x +
			cos_hue_angle * (wheel_r_inner + wheel_r_outer) * 0.5f,
		    wheel_center.y +
			sin_hue_angle * (wheel_r_inner + wheel_r_outer) * 0.5f);
		float hue_cursor_rad = value_changed_h
					   ? wheel_thickness * 0.65f
					   : wheel_thickness * 0.55f;
		int hue_cursor_segments =
		    ImClamp((int)(hue_cursor_rad / 1.4f), 9, 32);
		draw_list->AddCircleFilled(hue_cursor_pos, hue_cursor_rad,
					   hue_color32, hue_cursor_segments);
		draw_list->AddCircle(hue_cursor_pos, hue_cursor_rad + 1,
				     col_midgrey, hue_cursor_segments);
		draw_list->AddCircle(hue_cursor_pos, hue_cursor_rad, col_white,
				     hue_cursor_segments);

		// Render SV triangle (rotated according to hue)
		ImVec2 tra = wheel_center + ImRotate(triangle_pa, cos_hue_angle,
						     sin_hue_angle);
		ImVec2 trb = wheel_center + ImRotate(triangle_pb, cos_hue_angle,
						     sin_hue_angle);
		ImVec2 trc = wheel_center + ImRotate(triangle_pc, cos_hue_angle,
						     sin_hue_angle);
		...
		sv_cursor_pos = ImLerp(ImLerp(trc, tra, ImSaturate(S)), trb,
				       ImSaturate(1 - V));
	} else if (flags & ImGuiColorEditFlags_PickerHueBar) {
		// Render SV Square
		...
		draw_list->AddRectFilledMultiColor(
		    picker_pos,
		    picker_pos + ImVec2(sv_picker_size, sv_picker_size), 0, 0,
		    col_black, col_black);
		RenderFrameBorder(
		    picker_pos,
		...
		for (int i = 0; i < 6; ++i)
			draw_list->AddRectFilledMultiColor(
			    ImVec2(bar0_pos_x,
				   picker_pos.y + i * (sv_picker_size / 6)),
			    ImVec2(bar0_pos_x + bars_width,
				   picker_pos.y +
				       (i + 1) * (sv_picker_size / 6)),
			    col_hues[i], col_hues[i], col_hues[i + 1],
			    col_hues[i + 1]);
		...
		RenderArrowsForVerticalBar(
		    draw_list, ImVec2(bar0_pos_x - 1, bar0_line_y),
	...
	draw_list->AddCircleFilled(sv_cursor_pos, sv_cursor_rad,
				   user_col32_striped_of_alpha, 12);
	...
	if (alpha_bar) {
		...
		RenderColorRectWithAlphaCheckerboard(
		    draw_list, bar1_bb.Min, bar1_bb.Max, 0,
		...
		RenderArrowsForVerticalBar(
		    draw_list, ImVec2(bar1_pos_x - 1, bar1_line_y),
		    ImVec2(bars_triangles_half_sz + 1, bars_triangles_half_sz),
		    bars_width + 2.0f, style.Alpha);
	}
...
bool ImGui::ColorButton(const char *desc_id, const ImVec4 &col,
			ImGuiColorEditFlags flags, ImVec2 size)
{
	...
	ImVec4 col_rgb = col;
	if (flags & ImGuiColorEditFlags_InputHSV)
		ColorConvertHSVtoRGB(col_rgb.x, col_rgb.y, col_rgb.z, col_rgb.x,
				     col_rgb.y, col_rgb.z);
	...
	if ((flags & ImGuiColorEditFlags_NoBorder) == 0) {
		off = -0.75f; // The border (using Col_FrameBg) tends to look
			      // off when color is near-opaque and rounding is
			      // enabled. This offset seemed like a good middle
			      // ground to reduce those artifacts.
		bb_inner.Expand(off);
	}
	if ((flags & ImGuiColorEditFlags_AlphaPreviewHalf) &&
	    col_rgb.w < 1.0f) {
		float mid_x =
		    IM_ROUND((bb_inner.Min.x + bb_inner.Max.x) * 0.5f);
		RenderColorRectWithAlphaCheckerboard(
		    window->DrawList,
		...
		window->DrawList->AddRectFilled(
		    bb_inner.Min, ImVec2(mid_x, bb_inner.Max.y),
		    GetColorU32(col_rgb_without_alpha), rounding,
		    ImDrawCornerFlags_TopLeft | ImDrawCornerFlags_BotLeft);
	} else {
		// Because GetColorU32() multiplies by the global style Alpha
		// and we don't want to display a checkerboard if the source
		// code had no alpha
		...
		if (col_source.w < 1.0f)
			RenderColorRectWithAlphaCheckerboard(
			    window->DrawList, bb_inner.Min, bb_inner.Max,
			    GetColorU32(col_source), grid_step,
			    ImVec2(off, off), rounding);
		else
			window->DrawList->AddRectFilled(
			    bb_inner.Min, bb_inner.Max, GetColorU32(col_source),
	...
	if ((flags & ImGuiColorEditFlags_NoBorder) == 0) {
		if (g.Style.FrameBorderSize > 0.0f)
			...
			window->DrawList->AddRect(
			    ...
			    rounding); // Color button are often in need of some
				       // sort of border
	}
	...
	if (g.ActiveId == id && !(flags & ImGuiColorEditFlags_NoDragDrop) &&
	    BeginDragDropSource()) {
		if (flags & ImGuiColorEditFlags_NoAlpha)
			SetDragDropPayload(IMGUI_PAYLOAD_TYPE_COLOR_3F,
					   &col_rgb, sizeof(float) * 3,
					   ImGuiCond_Once);
		else
			SetDragDropPayload(IMGUI_PAYLOAD_TYPE_COLOR_4F,
					   &col_rgb, sizeof(float) * 4,
					   ImGuiCond_Once);
	...
	if (!(flags & ImGuiColorEditFlags_NoTooltip) && hovered)
		ColorTooltip(desc_id, &col.x,
			     flags & (ImGuiColorEditFlags__InputMask |
				      ImGuiColorEditFlags_NoAlpha |
				      ImGuiColorEditFlags_AlphaPreview |
				      ImGuiColorEditFlags_AlphaPreviewHalf));
...
void ImGui::SetColorEditOptions(ImGuiColorEditFlags flags)
{
	...
	if ((flags & ImGuiColorEditFlags__InputMask) == 0)
		flags |= ImGuiColorEditFlags__OptionsDefault &
			 ImGuiColorEditFlags__InputMask;
	IM_ASSERT(ImIsPowerOfTwo(
	    flags & ImGuiColorEditFlags__DisplayMask)); // Check only 1 option
							// is selected
	IM_ASSERT(ImIsPowerOfTwo(
	    flags & ImGuiColorEditFlags__DataTypeMask)); // Check only 1 option
							 // is selected
	IM_ASSERT(ImIsPowerOfTwo(
	    flags & ImGuiColorEditFlags__PickerMask)); // Check only 1 option is
						       // selected
	IM_ASSERT(ImIsPowerOfTwo(
	    flags &
	    ImGuiColorEditFlags__InputMask)); // Check only 1 option is selected
	g.ColorEditOptions = flags;
}

// Note: only access 3 floats if ImGuiColorEditFlags_NoAlpha flag is set.
void ImGui::ColorTooltip(const char *text, const float *col,
			 ImGuiColorEditFlags flags)
{
	ImGuiContext &g = *GImGui;
	...
	int cr = IM_F32_TO_INT8_SAT(col[0]), cg = IM_F32_TO_INT8_SAT(col[1]),
	    ...
	    ca = (flags & ImGuiColorEditFlags_NoAlpha)
		     ? 255
	...
	if ((flags & ImGuiColorEditFlags_InputRGB) ||
		...
		if (flags & ImGuiColorEditFlags_NoAlpha)
			...
			Text("#%02X%02X%02X%02X\nR:%d, G:%d, B:%d, "
			     "A:%d\n(%.3f, %.3f, %.3f, %.3f)",
			     cr, cg, cb, ca, cr, cg, cb, ca, col[0], col[1],
			     col[2], col[3]);
	} else if (flags & ImGuiColorEditFlags_InputHSV) {
		if (flags & ImGuiColorEditFlags_NoAlpha)
			Text("H: %.3f, S: %.3f, V: %.3f", col[0], col[1],
			     col[2]);
		else
			Text("H: %.3f, S: %.3f, V: %.3f, A: %.3f", col[0],
			     col[1], col[2], col[3]);
	}
	EndTooltip();
}
...
void ImGui::ColorEditOptionsPopup(const float *col, ImGuiColorEditFlags flags)
{
	...
	if (allow_opt_inputs) {
		if (RadioButton("RGB",
				(opts & ImGuiColorEditFlags_DisplayRGB) != 0))
			opts = (opts & ~ImGuiColorEditFlags__DisplayMask) |
			       ImGuiColorEditFlags_DisplayRGB;
		if (RadioButton("HSV",
				(opts & ImGuiColorEditFlags_DisplayHSV) != 0))
			opts = (opts & ~ImGuiColorEditFlags__DisplayMask) |
			       ImGuiColorEditFlags_DisplayHSV;
		if (RadioButton("Hex",
				(opts & ImGuiColorEditFlags_DisplayHex) != 0))
			opts = (opts & ~ImGuiColorEditFlags__DisplayMask) |
	...
	if (allow_opt_datatype) {
		if (allow_opt_inputs)
			Separator();
		if (RadioButton("0..255",
				(opts & ImGuiColorEditFlags_Uint8) != 0))
			opts = (opts & ~ImGuiColorEditFlags__DataTypeMask) |
			       ImGuiColorEditFlags_Uint8;
		if (RadioButton("0.00..1.00",
				(opts & ImGuiColorEditFlags_Float) != 0))
			opts = (opts & ~ImGuiColorEditFlags__DataTypeMask) |
	...
	if (BeginPopup("Copy")) {
		...
		char buf[64];
		ImFormatString(
		    buf, IM_ARRAYSIZE(buf), "(%.3ff, %.3ff, %.3ff, %.3ff)",
		    col[0], col[1], col[2],
		    (flags & ImGuiColorEditFlags_NoAlpha) ? 1.0f : col[3]);
		if (Selectable(buf))
			SetClipboardText(buf);
		ImFormatString(buf, IM_ARRAYSIZE(buf), "(%d,%d,%d,%d)", cr, cg,
			       cb, ca);
		if (Selectable(buf))
			SetClipboardText(buf);
		ImFormatString(buf, IM_ARRAYSIZE(buf), "#%02X%02X%02X", cr, cg,
			       cb);
		if (Selectable(buf))
			SetClipboardText(buf);
		if (!(flags & ImGuiColorEditFlags_NoAlpha)) {
				    ...
				    ImGuiColorEditFlags flags)
{
	...
	bool allow_opt_alpha_bar = !(flags & ImGuiColorEditFlags_NoAlpha) &&
				   !(flags & ImGuiColorEditFlags_AlphaBar);
	if ((!allow_opt_picker && !allow_opt_alpha_bar) ||
	    !BeginPopup("context"))
		return;
	ImGuiContext &g = *GImGui;
	if (allow_opt_picker) {
		ImVec2 picker_size(
		    g.FontSize * 8,
		    ImMax(g.FontSize * 8 -
			      (GetFrameHeight() + g.Style.ItemInnerSpacing.x),
			  1.0f)); // FIXME: Picker size copied from main picker
				  // function
		PushItemWidth(picker_size.x);
		for (int picker_type = 0; picker_type < 2; picker_type++) {
			// Draw small/thumbnail version of each picker type
			// (over an invisible button for selection)
			if (picker_type > 0)
				Separator();
			PushID(picker_type);
			ImGuiColorEditFlags picker_flags =
			...
			if (Selectable("##selectable", false, 0,
				       picker_size)) // By default, Selectable()
						     // is closing popup
				g.ColorEditOptions =
				    (g.ColorEditOptions &
				     ~ImGuiColorEditFlags__PickerMask) |
			...
			ColorPicker4("##previewing_picker",
				     &previewing_ref_col.x, picker_flags);
			PopID();
		}
		PopItemWidth();
	}
	if (allow_opt_alpha_bar) {
		if (allow_opt_picker)
			Separator();
		CheckboxFlags("Alpha Bar", &g.ColorEditOptions,
			      ImGuiColorEditFlags_AlphaBar);
	}
	EndPopup();
}
...
bool ImGui::TreeNodeBehaviorIsOpen(ImGuiID id, ImGuiTreeNodeFlags flags)
{
	...
	// use the SetNextItemOpen function)
	ImGuiContext &g = *GImGui;
	ImGuiWindow *window = g.CurrentWindow;
	...
	if (g.NextItemData.Flags & ImGuiNextItemDataFlags_HasOpen) {
		if (g.NextItemData.OpenCond & ImGuiCond_Always) {
			...
			// the same because tree node state are not saved
			// persistently.
	...
	// When logging is enabled, we automatically expand tree nodes (but
	// *NOT* collapsing headers.. seems like sensible behavior). NB- If we
	// are above max depth we still allow manually opened nodes to be
	// logged.
			     ...
			     const char *label, const char *label_end)
{
	...
	// We vertically grow up to current line height up the typical widget
	// height.
	...
	ImRect frame_bb;
	frame_bb.Min.x = (flags & ImGuiTreeNodeFlags_SpanFullWidth)
	...
	if (display_frame) {
		// Framed header expand a little outside the default padding, to
		// the edge of InnerClipRect (FIXME: May remove this at some
		// point and make InnerClipRect align with WindowPadding.x
		// instead of WindowPadding.x*0.5f)
		frame_bb.Min.x -=
		    IM_FLOOR(window->WindowPadding.x * 0.5f - 1.0f);
	...
	const float text_offset_y = ImMax(
	    padding.y,
	    window->DC
		.CurrLineTextBaseOffset); // Latch before ItemSize changes it
	...
	if (!display_frame && (flags & (ImGuiTreeNodeFlags_SpanAvailWidth |
	...
	// TreeNode() and TreePop(). This is currently only support 32 level
	...
	// We allow clicking on the arrow section with keyboard modifiers held,
	// in order to easily allow browsing a tree while preserving selection
	// with code implementing multi-selection patterns. When clicking on the
	// rest of the tree node we always disallow keyboard modifiers.
	...
	// _OnOnDoubleClick flags. Some alteration have subtle effects (e.g.
	// toggle on MouseUp vs MouseDown events) due to requirements for
	...
	// _OpenOnDoubleClick=1 and _OpenOnArrow=0) It is rather standard that
	// arrow click react on Down rather than Up. We set
	// ImGuiButtonFlags_PressedOnClickRelease on OpenOnDoubleClick because
	// we want the item to be active on the initial MouseDown in order for
	...
	if (!is_leaf) {
		if (pressed && g.DragDropHoldJustPressedId != id) {
			...
			if (flags & ImGuiTreeNodeFlags_OpenOnArrow)
				toggled |=
				    ...
				    !g.NavDisableMouseHover; // Lightweight
							     // equivalent of
							     // IsMouseHoveringRect()
							     // since
							     // ButtonBehavior()
		...
		if (g.NavId == id && g.NavMoveRequest &&
		    ...
		    !is_open) // If there's something upcoming on the line we
			      // may want to give it the priority?
		{
			toggled = true;
			NavMoveRequestCancel();
		}
	...
	// In this branch, TreeNodeBehavior() cannot toggle the selection so
	...
	if (display_frame) {
		...
		const ImU32 bg_col =
		    GetColorU32((held && hovered) ? ImGuiCol_HeaderActive
				: hovered	  ? ImGuiCol_HeaderHovered
						  : ImGuiCol_Header);
		RenderFrame(frame_bb.Min, frame_bb.Max, bg_col, true,
			    style.FrameRounding);
		...
		if (flags & ImGuiTreeNodeFlags_Bullet)
			...
			RenderArrow(
			    ...
			    ImVec2(text_pos.x - text_offset_x + padding.x,
				   text_pos.y),
			    text_col, is_open ? ImGuiDir_Down : ImGuiDir_Right,
			    1.0f);
		...
		if (g.LogEnabled) {
			// NB: '##' is normally used to hide text (as a
			// library-wide feature), so we need to specify the text
			// range to make sure the ## aren't stripped out here.
			const char log_prefix[] = "\n##";
			const char log_suffix[] = "##";
			LogRenderedText(&text_pos, log_prefix, log_prefix + 3);
			RenderTextClipped(text_pos, frame_bb.Max, label,
...
void ImGui::TreePop()
{
	...
	// ImGuiTreeNodeFlags_NavLeftJumpsBackHere is enabled)
	if (g.NavMoveDir == ImGuiDir_Left && g.NavWindow == window &&
	    NavMoveRequestButNoResultYet())
		if (g.NavIdIsAlive &&
		    (window->DC.TreeJumpToParentOnPopMask & tree_depth_mask)) {
...
// ID stack (because of the ImGuiTreeNodeFlags_NoTreePushOnOpen flag). This is
// basically the same as calling TreeNodeEx(label,
// ImGuiTreeNodeFlags_CollapsingHeader). You can remove the _NoTreePushOnOpen
			     ...
			     ImGuiTreeNodeFlags flags)
{
	...
	if (p_open != NULL) {
		...
		// FIXME: CloseButton can overlap into text, need find a way to
		// clip the text somehow.
		ImGuiContext &g = *GImGui;
...
// ImGuiSelectableFlags_SpanAllColumns and ImGuiSelectableFlags_AllowItemOverlap
// are also frequently used flags.
		       ...
		       ImGuiSelectableFlags flags, const ImVec2 &size_arg)
{
	...
	// Submit label or explicit size to ItemSize(), whereas ItemAdd() will
	// submit a larger/spanning rectangle.
	ImGuiID id = window->GetID(label);
	...
	// We don't support (size < 0.0f) in Selectable() because the
	// ItemSpacing extension would make explicitely right-aligned sizes not
	// visibly match other widgets.
	const bool span_all_columns =
	    (flags & ImGuiSelectableFlags_SpanAllColumns) != 0;
	const float min_x =
	    span_all_columns ? window->ParentWorkRect.Min.x : pos.x;
	const float max_x = span_all_columns ? window->ParentWorkRect.Max.x
					     : window->WorkRect.Max.x;
	if (size_arg.x == 0.0f || (flags & ImGuiSelectableFlags_SpanAvailWidth))
		size.x = ImMax(label_size.x, max_x - min_x);

	// Text stays at the submission position, but bounding box may be
	// extended on both sides
	const ImVec2 text_min = pos;
	const ImVec2 text_max(min_x + size.x, pos.y + size.y);

	// Selectables are meant to be tightly packed together with no
	// click-gap, so we extend their box to cover spacing between
	// selectable.
	ImRect bb(min_x, pos.y, text_max.x, text_max.y);
	if ((flags & ImGuiSelectableFlags_NoPadWithHalfSpacing) == 0) {
		const float spacing_x =
		    span_all_columns ? 0.0f : style.ItemSpacing.x;
	...
	if (flags & ImGuiSelectableFlags_Disabled) {
		ImGuiItemFlags backup_item_flags = window->DC.ItemFlags;
		window->DC.ItemFlags |=
		    ImGuiItemFlags_Disabled | ImGuiItemFlags_NoNavDefaultFocus;
	...
	// FIXME: We can standardize the behavior of those two, we could also
	// keep the fast path of override ClipRect + full push on render only,
	...
	// Update NavId when clicking or when Hovering (this doesn't happen on
	// most widgets), so navigation can be resumed with gamepad/keyboard
	if (pressed ||
	    (hovered && (flags & ImGuiSelectableFlags_SetNavIdOnHover))) {
...
// ListBoxHeader->BeginListBox, ListBoxFooter->EndListBox and promote using them
// over existing ListBox() functions, similarly to change with combo boxes.
//-------------------------------------------------------------------------

// FIXME: In principle this function should be called BeginListBox(). We should
// rename it after re-evaluating if we want to keep the same signature. Helper
// to calculate the size of a listbox and display a label on the right. Tip: To
// have a list filling the entire window width, PushItemWidth(-1) and pass an
// non-visible label e.g. "##empty"
bool ImGui::ListBoxHeader(const char *label, const ImVec2 &size_arg)
{
	...
	ImVec2 size = CalcItemSize(size_arg, CalcItemWidth(),
				   GetTextLineHeightWithSpacing() * 7.4f +
				       style.ItemSpacing.y);
	...
	window->DC.LastItemRect =
	    bb; // Forward storage for ListBoxFooter.. dodgy.
	g.NextItemData.ClearFlags();
...
// FIXME: In principle this function should be called EndListBox(). We should
// rename it after re-evaluating if we want to keep the same signature.
			  ...
			  int height_in_items)
{
	...
	// if there are more items up/down, without looking at the scrollbar. We
	// don't add this extra bit if items_count <= height_in_items. It is
	// slightly dodgy, because it means a dynamic list of items will make
	// the widget resize occasionally when it crosses that size. I am
	// expecting that someone will come and complain about this behavior in
	// a remote future, then we can advise on a better solution.
	if (height_in_items < 0)
		height_in_items = ImMin(items_count, 7);
	const ImGuiStyle &style = GetStyle();
	float height_in_items_f = (height_in_items < items_count)
				      ? (height_in_items + 0.25f)
				      : (height_in_items + 0.00f);

	// We include ItemSpacing.y so that a list sized for the exact number of
	// items doesn't make a scrollbar appears. We could also enforce that by
	// passing a flag to BeginChild().
	ImVec2 size;
	size.x = 0.0f;
	size.y = ImFloor(GetTextLineHeightWithSpacing() * height_in_items_f +
			 style.FramePadding.y * 2.0f);
...
void ImGui::ListBoxFooter()
{
	...
	// Redeclare item size so that it includes the label (we have stored the
	// full size in LastItemRect) We call SameLine() to restore
	// DC.CurrentLine* data
	SameLine();
...
{
	const bool value_changed =
	    ListBox(label, current_item, Items_ArrayGetter, (void *)items,
		    items_count, height_items);
	return value_changed;
}
		    ...
		    void *data, int items_count, int height_in_items)
{
	...
	// Assume all items have even height (= 1 line of text). If you need
	// items of different or variable sizes you can create a custom version
	// of ListBox() in your code without using the clipper.
	ImGuiContext &g = *GImGui;
	bool value_changed = false;
	ImGuiListClipper clipper;
	clipper.Begin(
	    items_count,
	    GetTextLineHeightWithSpacing()); // We know exactly our line height
...
int ImGui::PlotEx(ImGuiPlotType plot_type, const char *label,
		  ...
		  float scale_min, float scale_max, ImVec2 frame_size)
{
	ImGuiContext &g = *GImGui;
	ImGuiWindow *window = GetCurrentWindow();
	if (window->SkipItems)
	...
	if (values_count >= values_count_min) {
		...
		if (hovered && inner_bb.Contains(g.IO.MousePos)) {
			const float t =
			    ImClamp((g.IO.MousePos.x - inner_bb.Min.x) /
					(inner_bb.Max.x - inner_bb.Min.x),
				    0.0f, 0.9999f);
			const int v_idx = (int)(t * item_count);
			IM_ASSERT(v_idx >= 0 && v_idx < values_count);

			const float v0 = values_getter(
		...
		float v0 =
		    values_getter(data, (0 + values_offset) % values_count);
		float t0 = 0.0f;
		ImVec2 tp0 = ImVec2(
		    t0, 1.0f - ImSaturate(
				   (v0 - scale_min) *
		...
		const ImU32 col_base =
		    GetColorU32((plot_type == ImGuiPlotType_Lines)
				    ? ImGuiCol_PlotLines
		...
		for (int n = 0; n < res_w; n++) {
			...
			const ImVec2 tp1 =
			    ImVec2(t1, 1.0f - ImSaturate((v1 - scale_min) *
							 inv_scale));

			// NB: Draw calls are merged together by the DrawList
			// system. Still, we should render our batch are lower
	...
	if (overlay_text)
		RenderTextClipped(ImVec2(frame_bb.Min.x,
					 frame_bb.Min.y + style.FramePadding.y),
				  frame_bb.Max, overlay_text, NULL, NULL,
				  ImVec2(0.5f, 0.0f));
	...
	// This is currently not exposed in the public API because we need a
	// larger redesign of the whole thing, but in the short-term we are
...
static float Plot_ArrayGetter(void *data, int idx)
{
	...
	const float v =
	    *(const float *)(const void *)((const unsigned char *)
					       plot_data->Values +
...
void ImGui::PlotHistogram(const char *label,
			  ...
			  float scale_max, ImVec2 graph_size)
{
	PlotEx(ImGuiPlotType_Histogram, label, values_getter, data,
	       values_count, values_offset, overlay_text, scale_min, scale_max,
	       graph_size);
}
    ...
    float w2) // not using va_arg because they promote float to double
{
	...
	for (int i = 0; i < IM_ARRAYSIZE(Pos); i++)
		NextWidth += NextWidths[i] +
			     ((i > 0 && NextWidths[i] > 0.0f) ? Spacing : 0.0f);
...
// rectangle, in which case ImGuiWindowFlags_MenuBar will become unnecessary.
...
bool ImGui::BeginMenuBar()
{
	...
	BeginGroup(); // Backup position on layer 0 // FIXME: Misleading to use
		      // a group for that backup/restore
	PushID("##menubar");

	// We don't clip with current window clipping rectangle as it is already
	// set to the area below. However we clip with window full rect. We
	// remove 1 worth of rounding to Max.x to that text in long menus and
	// small windows don't tend to display over the lower-right rounded
	...
	// analoguous here, maybe a BeginGroupEx() with flags).
...
void ImGui::EndMenuBar()
{
	...
	if (NavMoveRequestButNoResultYet() &&
		...
		if (nav_earliest_child->ParentWindow == window &&
			...
			// To do so we claim focus back, restore NavId and then
			// process the movement request for yet another frame.
			// This involve a one-frame delay which isn't very
			// problematic in this situation. We could remove it by
			// scoring in advance for multiple window (probably not
			// worth the hassle/cost)
			const ImGuiNavLayer layer = ImGuiNavLayer_Menu;
			IM_ASSERT(window->DC.NavLayerActiveMaskNext &
				  (1 << layer)); // Sanity check
			FocusWindow(window);
			SetNavIDWithRectRel(window->NavLastIds[layer], layer, 0,
					    window->NavRectRel[layer]);
			g.NavLayer = layer;
			g.NavDisableHighlight =
			    true; // Hide highlight for the current frame so we
				  // don't see the intermediary selection.
	...
	window->DC.MenuBarOffset.x =
	    window->DC.CursorPos.x -
	    window->MenuBarRect()
		.Min.x; // Save horizontal position so next append can reuse it.
...
bool ImGui::BeginMainMenuBar()
{
	...
	bool is_open =
	    Begin("##MainMenuBar", NULL, window_flags) && BeginMenuBar();
	PopStyleVar(2);
...
void ImGui::EndMainMenuBar()
{
	...
	// When the user has left the menu layer (typically: closed menus
	// through activation of an item), we restore focus to the previous
	// window
	// FIXME: With this strategy we won't be able to restore a NULL focus.
	ImGuiContext &g = *GImGui;
	if (g.CurrentWindow == g.NavWindow &&
	    g.NavLayer == ImGuiNavLayer_Main && !g.NavAnyRequest)
...
bool ImGui::BeginMenu(const char *label, bool enabled)
{
	...
	// (otherwise top-most popup menu would steal focus and not allow
	// hovering on parent menu)
	...
	// If a menu with same the ID was already submitted, we will append to
	// it, matching the behavior of Begin(). We are relying on a O(N) search
	// - so O(N log N) over the frame - which seems like the most efficient
	// for the expected small amount of BeginMenu() calls per frame. If
	// somehow this is ever becoming a problem we can switch to use e.g.
	// ImGuiStorage mapping key to last frame used.
	if (g.MenusIdSubmittedThisFrame.contains(id)) {
		if (menu_is_open)
			menu_is_open = BeginPopupEx(
			    id, flags); // menu_is_open can be 'false' when the
	...
	// Tag menu as used. Next time BeginMenu() with same ID is called it
	// will append to existing menu
	...
	if (menuset_is_open)
		g.NavWindow = window; // Odd hack to allow hovering across menus
				      // of a same menu-set (otherwise we
				      // wouldn't be able to hover parent)

	// The reference position stored in popup_pos will be used by Begin() to
	// find a suitable position for the child menu, However the final
	// position is going to be different! It is chosen by
	// FindBestWindowPosForPopup(). e.g. Menus tend to overlap each other
	// horizontally to amplify relative Z-ordering.
	ImVec2 popup_pos, pos = window->DC.CursorPos;
	if (window->DC.LayoutType == ImGuiLayoutType_Horizontal) {
		// Menu inside an horizontal menu bar
		// Selectable extend their highlight by half ItemSpacing in each
		// direction. For ChildMenu, the popup position will be
		// overwritten by the call to FindBestWindowPosForPopup() in
		// Begin()
		popup_pos = ImVec2(
		    pos.x - 1.0f - IM_FLOOR(style.ItemSpacing.x * 0.5f),
		...
		window->DC.CursorPos.x += IM_FLOOR(
		    ...
		    (-1.0f +
		     0.5f)); // -1 spacing to compensate the spacing added when
			     // Selectable() did a SameLine(). It would also
			     // work to call SameLine() ourselves after the
			     // PopStyleVar().
	} else {
		// Menu inside a menu
		// (In a typical menu window where all items are BeginMenu() or
		// MenuItem() calls, extra_w will always be 0.0f.
		//  Only when they are other items sticking out we're going to
		//  add spacing, yet only register minimum width into the layout
		//  system.
		...
		pressed = Selectable(
			...
			(!enabled ? ImGuiSelectableFlags_Disabled : 0),
		    ImVec2(min_w, 0.0f));
		ImU32 text_col = GetColorU32(enabled ? ImGuiCol_Text
						     : ImGuiCol_TextDisabled);
		RenderArrow(window->DrawList,
			    pos + ImVec2(window->DC.MenuColumns.Pos[2] +
					     extra_w + g.FontSize * 0.30f,
					 0.0f),
			    text_col, ImGuiDir_Right);
	}

	const bool hovered =
	    enabled && ItemHoverable(window->DC.LastItemRect, id);
	if (menuset_is_open)
	...
	if (window->DC.LayoutType ==
	    ImGuiLayoutType_Vertical) // (window->Flags &
				      // (ImGuiWindowFlags_Popup|ImGuiWindowFlags_ChildMenu))
	{
		// Close menu when not hovering it anymore unless we are moving
		// roughly in the direction of the menu Implement
		...
		if (g.HoveredWindow == window && child_menu_window != NULL &&
		    !(window->Flags & ImGuiWindowFlags_MenuBar)) {
			// FIXME-DPI: Values should be derived from a master
			// "scale" factor.
			ImRect next_window_rect = child_menu_window->Rect();
			ImVec2 ta = g.IO.MousePos - g.IO.MouseDelta;
			ImVec2 tb = (window->Pos.x < child_menu_window->Pos.x)
					? next_window_rect.GetTL()
					: next_window_rect.GetTR();
			ImVec2 tc = (window->Pos.x < child_menu_window->Pos.x)
					? next_window_rect.GetBL()
					: next_window_rect.GetBR();
			float extra =
			    ImClamp(ImFabs(ta.x - tb.x) * 0.30f, 5.0f,
			...
			tb.y = ta.y +
			       ImMax((tb.y - extra) - ta.y,
				     -100.0f); // triangle is maximum 200 high
					       // to limit the slope and the
					       // bias toward large sub-menus //
					       // FIXME: Multiply by fb_scale?
			tc.y = ta.y + ImMin((tc.y + extra) - ta.y, +100.0f);
			moving_toward_other_child_menu =
			    ImTriangleContainsPoint(ta, tb, tc, g.IO.MousePos);
			// GetForegroundDrawList()->AddTriangleFilled(ta, tb,
			// tc, moving_within_opened_triangle ?
			// IM_COL32(0,128,0,128) : IM_COL32(128,0,0,128)); //
			// [DEBUG]
		}
		if (menu_is_open && !hovered && g.HoveredWindow == window &&
		    g.HoveredIdPreviousFrame != 0 &&
		    g.HoveredIdPreviousFrame != id &&
		    !moving_toward_other_child_menu)
			want_close = true;
		...
		if (!menu_is_open && hovered && pressed) // Click to open
			want_open = true;
			 ...
			 !moving_toward_other_child_menu) // Hover to open
			want_open = true;
		    ...
		    menuset_is_open) // Click an open menu again to close it
		{
			...
			want_open = menu_is_open = false;
			   ...
			   (hovered && menuset_is_open &&
			    !menu_is_open)) // First click to open, then hover
					    // to open others
		{
			want_open = true;
	...
	if (!enabled) // explicitly close if an open menu becomes disabled,
		      // facilitate users code a lot in pattern such as 'if
		      // (BeginMenu("options", has_object)) { ..use object.. }'
		want_close = true;
	...
	if (!menu_is_open && want_open &&
	    g.OpenPopupStack.Size > g.BeginPopupStack.Size) {
		// Don't recycle same menu level in the same frame, first close
		// the other menu and yield for a frame.
		OpenPopup(label);
		return false;
	}
	...
	if (menu_is_open) {
		...
		menu_is_open = BeginPopupEx(
		    id, flags); // menu_is_open can be 'false' when the popup is
...
void ImGui::EndMenu()
{
	// Nav: When a left move request _within our child menu_ failed, close
	// ourselves (the _parent_ menu). A menu doesn't close itself because
	// EndMenuBar() wants the catch the last Left<>Right inputs. However, it
	// means that with the current code, a BeginMenu() from outside another
	// menu or a menu-bar won't be closable with the Left direction.
	ImGuiContext &g = *GImGui;
	ImGuiWindow *window = g.CurrentWindow;
	if (g.NavWindow && g.NavWindow->ParentWindow == window &&
		     ...
		     bool enabled)
{
	...
	// We've been using the equivalent of
	// ImGuiSelectableFlags_SetNavIdOnHover on all Selectable() since early
	// Nav system days (commit 43ee5d73), but I am unsure whether this
	// should be kept at all. For now moved it to be an opt-in feature used
	// by menus only.
	...
	bool pressed;
	if (window->DC.LayoutType == ImGuiLayoutType_Horizontal) {
		// Mimic the exact layout spacing of BeginMenu() to allow
		// MenuItem() inside a menu bar, which is a little misleading
		// but may be useful Note that in this situation we render
		...
		//  system.
		float shortcut_w =
		    shortcut ? CalcTextSize(shortcut, NULL).x : 0.0f;
		float min_w = window->DC.MenuColumns.DeclColumns(
		    label_size.x, shortcut_w,
		    IM_FLOOR(g.FontSize * 1.20f)); // Feedback for next frame
		float extra_w = ImMax(0.0f, GetContentRegionAvail().x - min_w);
		pressed = Selectable(
		    label, false, flags | ImGuiSelectableFlags_SpanAvailWidth,
		    ImVec2(min_w, 0.0f));
		if (shortcut_w > 0.0f) {
			PushStyleColor(ImGuiCol_Text,
				       g.Style.Colors[ImGuiCol_TextDisabled]);
			RenderText(pos + ImVec2(window->DC.MenuColumns.Pos[1] +
						    extra_w,
						0.0f),
				   shortcut, NULL, false);
			PopStyleColor();
		...
		if (selected)
			RenderCheckMark(
			    window->DrawList,
			    pos + ImVec2(window->DC.MenuColumns.Pos[2] +
					     extra_w + g.FontSize * 0.40f,
					 g.FontSize * 0.134f * 0.5f),
			    GetColorU32(enabled ? ImGuiCol_Text
						: ImGuiCol_TextDisabled),
			    g.FontSize * 0.866f);
	}
...
bool ImGui::MenuItem(const char *label, const char *shortcut, bool *p_selected,
		     bool enabled)
{
	if (MenuItem(label, shortcut, p_selected ? *p_selected : false,
		     enabled)) {
		if (p_selected)
			*p_selected = !*p_selected;
		return true;
...
struct ImGuiTabBarSection {
	...
	float Width;   // Sum of width of tabs in this section (after shrinking
		       // down)
						...
						const void *rhs)
{
	...
	const int a_section = (a->Flags & ImGuiTabItemFlags_Leading)	? 0
			      : (a->Flags & ImGuiTabItemFlags_Trailing) ? 2
									: 1;
	const int b_section = (b->Flags & ImGuiTabItemFlags_Leading)	? 0
			      : (b->Flags & ImGuiTabItemFlags_Trailing) ? 2
									: 1;
...
{
	ImGuiContext &g = *GImGui;
	return ref.Ptr ? (ImGuiTabBar *)ref.Ptr
...
bool ImGui::BeginTabBar(const char *str_id, ImGuiTabBarFlags flags)
{
	...
	tab_bar->ID = id;
	return BeginTabBarEx(tab_bar, tab_bar_bb,
			     flags | ImGuiTabBarFlags_IsFocused);
}

bool ImGui::BeginTabBarEx(ImGuiTabBar *tab_bar, const ImRect &tab_bar_bb,
			  ImGuiTabBarFlags flags)
{
	...
	if (tab_bar->CurrFrameVisible == g.FrameCount) {
		window->DC.CursorPos =
		    ImVec2(tab_bar->BarRect.Min.x,
			   tab_bar->BarRect.Max.y + tab_bar->ItemSpacingY);
		tab_bar->BeginCount++;
		return true;
	}

	// Ensure correct ordering when toggling ImGuiTabBarFlags_Reorderable
	// flag, or when a new tab was added while being not reorderable
	if ((flags & ImGuiTabBarFlags_Reorderable) !=
		(tab_bar->Flags & ImGuiTabBarFlags_Reorderable) ||
	    (tab_bar->TabsAddedNew && !(flags & ImGuiTabBarFlags_Reorderable)))
		if (tab_bar->Tabs.Size > 1)
			ImQsort(tab_bar->Tabs.Data, tab_bar->Tabs.Size,
				sizeof(ImGuiTabItem),
	...
	// Draw separator
	const ImU32 col = GetColorU32((flags & ImGuiTabBarFlags_IsFocused)
					  ? ImGuiCol_TabActive
	...
	const float y = tab_bar->BarRect.Max.y - 1.0f;
	{
		const float separator_min_x =
		    ...
		    IM_FLOOR(window->WindowPadding.x * 0.5f);
		const float separator_max_x =
		    tab_bar->BarRect.Max.x +
		    IM_FLOOR(window->WindowPadding.x * 0.5f);
		window->DrawList->AddLine(ImVec2(separator_min_x, y),
					  ImVec2(separator_max_x, y), col,
					  1.0f);
	}
	return true;
}
...
void ImGui::EndTabBar()
{
	...
	if (tab_bar->VisibleTabWasSubmitted || tab_bar->VisibleTabId == 0 ||
		...
		tab_bar->CurrTabsContentsHeight =
		    ImMax(window->DC.CursorPos.y - tab_bar->BarRect.Max.y,
			  tab_bar->CurrTabsContentsHeight);
		window->DC.CursorPos.y =
...
static void ImGui::TabBarLayout(ImGuiTabBar *tab_bar)
{
	...
	ImGuiTabBarSection
	    sections[3]; // Layout sections: Leading, Central, Trailing
	for (int tab_src_n = 0; tab_src_n < tab_bar->Tabs.Size; tab_src_n++) {
		...
		// We will need sorting if tabs have changed section (e.g. moved
		// from one of Leading/Central/Trailing to another)
		int curr_tab_section_n =
		    (tab->Flags & ImGuiTabItemFlags_Leading)	? 0
		    : (tab->Flags & ImGuiTabItemFlags_Trailing) ? 2
								: 1;
		if (tab_dst_n > 0) {
			ImGuiTabItem *prev_tab = &tab_bar->Tabs[tab_dst_n - 1];
			int prev_tab_section_n =
			    (prev_tab->Flags & ImGuiTabItemFlags_Leading) ? 0
			    : (prev_tab->Flags & ImGuiTabItemFlags_Trailing)
				? 2
				: 1;
	...
	sections[0].Spacing =
		...
		: 0.0f;
	sections[1].Spacing =
	    sections[1].TabCount > 0 && sections[2].TabCount > 0
		? g.Style.ItemInnerSpacing.x
		: 0.0f;
	...
	if (tab_bar->ReorderRequestTabId != 0) {
		if (TabBarProcessReorder(tab_bar))
			if (tab_bar->ReorderRequestTabId ==
	...
	// Tab List Popup (will alter tab_bar->BarRect and therefore the
	// available width!)
	const bool tab_list_popup_button =
	    (tab_bar->Flags & ImGuiTabBarFlags_TabListPopupButton) != 0;
	if (tab_list_popup_button)
		if (ImGuiTabItem *tab_to_select = TabBarTabListPopupButton(
			tab_bar)) // NB: Will alter BarRect.Min.x!
			scroll_track_selected_tab_id = tab_bar->SelectedTabId =
			    tab_to_select->ID;

	// Leading/Trailing tabs will be shrink only if central one aren't
	// visible anymore, so layout the shrink data as: leading, trailing,
	// central (whereas our tabs are stored as: leading, central, trailing)
	int shrink_buffer_indexes[3] = {
	    0, sections[0].TabCount + sections[2].TabCount,
	    sections[0].TabCount};
	...
	for (int tab_n = 0; tab_n < tab_bar->Tabs.Size; tab_n++) {
		...
		if (scroll_track_selected_tab_id == 0 &&
		    g.NavJustMovedToId == tab->ID)
			scroll_track_selected_tab_id = tab->ID;

		// Refresh tab width immediately, otherwise changes of style
		// e.g. style.FramePadding.x would noticeably lag in the tab
		// bar. Additionally, when using TabBarAddTab() to manipulate
		// tab bar order we occasionally insert new tabs that don't have
		// a width yet, and we cannot wait for the next BeginTabItem()
		// call. We cannot compute this width within TabBarAddTab()
		// because font size depends on the active window.
		const char *tab_name = tab_bar->GetTabName(tab);
		const bool has_close_button =
		    (tab->Flags & ImGuiTabItemFlags_NoCloseButton) ? false
								   : true;
		tab->ContentWidth =
		    TabItemCalcSize(tab_name, has_close_button).x;

		int section_n = (tab->Flags & ImGuiTabItemFlags_Leading)    ? 0
				: (tab->Flags & ImGuiTabItemFlags_Trailing) ? 2
									    : 1;
		ImGuiTabBarSection *section = &sections[section_n];
		section->Width +=
		    tab->ContentWidth + (section_n == curr_section_n
					     ? g.Style.ItemInnerSpacing.x
					     : 0.0f);
		curr_section_n = section_n;

		// Store data so we can build an array sorted by width if we
		// need to shrink tabs down
		int shrink_buffer_index = shrink_buffer_indexes[section_n]++;
		g.ShrinkWidthBuffer[shrink_buffer_index].Index = tab_n;
		g.ShrinkWidthBuffer[shrink_buffer_index].Width =
		    tab->ContentWidth;
	...
	if ((tab_bar->WidthAllTabsIdeal > tab_bar->BarRect.GetWidth() &&
		...
		if (ImGuiTabItem *scroll_track_selected_tab =
			...
			scroll_track_selected_tab_id =
			    scroll_track_selected_tab->ID;
			if (!(scroll_track_selected_tab->Flags &
	...
	if (central_section_is_visible)
		width_excess =
		    ImMax(section_1_w - (tab_bar->BarRect.GetWidth() -
					 section_0_w - section_2_w),
			  0.0f); // Excess used to shrink central section
	else
		width_excess =
		    (section_0_w + section_2_w) -
		    tab_bar->BarRect.GetWidth(); // Excess used to shrink
						 // leading/trailing section

	// With ImGuiTabBarFlags_FittingPolicyScroll policy, we will only shrink
	// leading/trailing if the central section is not visible anymore
	if (width_excess > 0.0f &&
	    ((tab_bar->Flags & ImGuiTabBarFlags_FittingPolicyResizeDown) ||
		...
		for (int tab_n = shrink_data_offset;
			...
			ImGuiTabItem *tab =
			    &tab_bar->Tabs[g.ShrinkWidthBuffer[tab_n].Index];
			float shrinked_width =
			    IM_FLOOR(g.ShrinkWidthBuffer[tab_n].Width);
	...
	for (int section_n = 0; section_n < 3; section_n++) {
		...
		if (section_n == 2)
			tab_offset =
			    ImMin(ImMax(0.0f, tab_bar->BarRect.GetWidth() -
						  section->Width),
				  tab_offset);

		for (int tab_n = 0; tab_n < section->TabCount; tab_n++) {
			ImGuiTabItem *tab =
			    &tab_bar->Tabs[section_tab_index + tab_n];
			tab->Offset = tab_offset;
			tab_offset +=
			    tab->Width + (tab_n < section->TabCount - 1
					      ? g.Style.ItemInnerSpacing.x
					      : 0.0f);
		}
		tab_bar->WidthAllTabs +=
		    ImMax(section->Width + section->Spacing, 0.0f);
	...
	if (scroll_track_selected_tab_id)
		if (ImGuiTabItem *scroll_track_selected_tab = TabBarFindTabByID(
			tab_bar, scroll_track_selected_tab_id))
	...
	if (tab_bar->ScrollingAnim != tab_bar->ScrollingTarget) {
		...
		// target in 1/3 seconds. Teleport if we are aiming far off the
		// visible line
		tab_bar->ScrollingSpeed =
		    ImMax(tab_bar->ScrollingSpeed, 70.0f * g.FontSize);
		tab_bar->ScrollingSpeed = ImMax(
		    tab_bar->ScrollingSpeed,
		    ImFabs(tab_bar->ScrollingTarget - tab_bar->ScrollingAnim) /
			0.3f);
		const bool teleport =
		    (tab_bar->PrevFrameVisible + 1 < g.FrameCount) ||
	...
	// Actual layout in host window (we don't do it in BeginTabBar() so as
	// not to waste an extra frame)
	ImGuiWindow *window = g.CurrentWindow;
	window->DC.CursorPos = tab_bar->BarRect.Min;
	ItemSize(
	    ImVec2(tab_bar->WidthAllTabsIdeal, tab_bar->BarRect.GetHeight()),
	    tab_bar->FramePadding.y);
}

// Dockables uses Name/ID in the global namespace. Non-dockable items use the ID
// stack.
...
// The *TabId fields be already set by the docking system _before_ the actual
...
void ImGui::TabBarCloseTab(ImGuiTabBar *tab_bar, ImGuiTabItem *tab)
{
	...
	if (!(tab->Flags & ImGuiTabItemFlags_UnsavedDocument)) {
		// This will remove a frame of lag for selecting another tab on
		// closure. However we don't run it in the case where the
		// 'Unsaved' flag is set, so user gets a chance to fully undo
		// the closure
		tab->WantClose = true;
		if (tab_bar->VisibleTabId == tab->ID) {
		...
		// Actually select before expecting closure attempt (on an
		// UnsavedDocument tab user is expect to e.g. show a popup)
		if (tab_bar->VisibleTabId != tab->ID)
				     ...
				     ImGuiTabBarSection *sections)
{
	...
	float margin = g.FontSize *
		       1.0f; // When to scroll to make Tab N+1 visible always
			     // make a bit of N visible to suggest more
			     // scrolling area (since we don't have a scrollbar)
	int order = tab_bar->GetTabOrder(tab);

	// Scrolling happens only in the central section (leading/trailing
	// sections are not scrolling)
	// FIXME: This is all confusing.
	float scrollable_width = tab_bar->BarRect.GetWidth() -
				 sections[0].Width - sections[2].Width -
				 sections[1].Spacing;

	// We make all tabs positions all relative Sections[0].Width to make
	// code simpler
	float tab_x1 = tab->Offset - sections[0].Width +
		       (order > sections[0].TabCount - 1 ? -margin : 0.0f);
	float tab_x2 =
	    tab->Offset - sections[0].Width + tab->Width +
	    (order + 1 < tab_bar->Tabs.Size - sections[2].TabCount ? margin
								   : 1.0f);
	...
	if (tab_bar->ScrollingTarget > tab_x1 ||
		...
		// Scroll to the left
		tab_bar->ScrollingTargetDistToVisibility =
		    ImMax(tab_bar->ScrollingAnim - tab_x2, 0.0f);
...
void ImGui::TabBarQueueReorder(ImGuiTabBar *tab_bar, const ImGuiTabItem *tab,
			       int dir)
{
...
{
	ImGuiTabItem *tab1 =
	    TabBarFindTabByID(tab_bar, tab_bar->ReorderRequestTabId);
	if (tab1 == NULL || (tab1->Flags & ImGuiTabItemFlags_NoReorder))
		return false;

	// IM_ASSERT(tab_bar->Flags & ImGuiTabBarFlags_Reorderable); // <- this
	// may happen when using debug tools
	int tab2_order =
	    tab_bar->GetTabOrder(tab1) + tab_bar->ReorderRequestDir;
...
static ImGuiTabItem *ImGui::TabBarScrollingButtons(ImGuiTabBar *tab_bar)
{
	...
	const ImVec2 arrow_button_size(
	    g.FontSize - 2.0f, g.FontSize + g.Style.FramePadding.y * 2.0f);
	const float scrolling_buttons_width = arrow_button_size.x * 2.0f;

	const ImVec2 backup_cursor_pos = window->DC.CursorPos;
	// window->DrawList->AddRect(ImVec2(tab_bar->BarRect.Max.x -
	// scrolling_buttons_width, tab_bar->BarRect.Min.y),
	// ImVec2(tab_bar->BarRect.Max.x, tab_bar->BarRect.Max.y),
	...
	float x = ImMax(tab_bar->BarRect.Min.x,
	...
	if (ArrowButtonEx("##<", ImGuiDir_Left, arrow_button_size,
		...
		select_dir = -1;
	window->DC.CursorPos =
	    ImVec2(x + arrow_button_size.x, tab_bar->BarRect.Min.y);
	...
	if (select_dir != 0)
		if (ImGuiTabItem *tab_item =
			TabBarFindTabByID(tab_bar, tab_bar->SelectedTabId)) {
			int selected_order = tab_bar->GetTabOrder(tab_item);
			int target_order = selected_order + select_dir;

			// Skip tab item buttons until another tab item is found
			// or end is reached
			while (tab_to_scroll_to == NULL) {
				// If we are at the end of the list, still
				...
				// (even if first/last item is a button, return
				// it so we can update the scroll)
				if (tab_to_scroll_to->Flags &
				    ImGuiTabItemFlags_Button) {
...
static ImGuiTabItem *ImGui::TabBarTabListPopupButton(ImGuiTabBar *tab_bar)
{
	...
	const float tab_list_popup_button_width =
	    g.FontSize + g.Style.FramePadding.y;
	const ImVec2 backup_cursor_pos = window->DC.CursorPos;
	window->DC.CursorPos =
	    ImVec2(tab_bar->BarRect.Min.x - g.Style.FramePadding.y,
...
bool ImGui::BeginTabItem(const char *label, bool *p_open,
			 ImGuiTabItemFlags flags)
{
	...
	ImGuiTabBar *tab_bar = g.CurrentTabBar;
	if (tab_bar == NULL) {
		IM_ASSERT_USER_ERROR(tab_bar, "Needs to be called between "
					      "BeginTabBar() and EndTabBar()!");
		return false;
	}
	IM_ASSERT(
	    !(flags & ImGuiTabItemFlags_Button)); // BeginTabItem() Can't be
						  // used with button flags, use
	...
	if (ret && !(flags & ImGuiTabItemFlags_NoPushId)) {
		...
		PushOverrideID(
		    tab->ID); // We already hashed 'label' so push into the ID
...
void ImGui::EndTabItem()
{
	...
	if (tab_bar == NULL) {
		IM_ASSERT_USER_ERROR(tab_bar != NULL,
				     "Needs to be called between BeginTabBar() "
				     "and EndTabBar()!");
		return;
	}
		      ...
		      ImGuiTabItemFlags flags)
{
	...
	// render. We make a call to ItemAdd() so that attempts to use a
	// contextual popup menu with an implicit ID won't use an older ID.
	IMGUI_TEST_ENGINE_ITEM_INFO(id, label, window->DC.LastItemStatusFlags);
	if (p_open && !*p_open) {
		PushItemFlag(ImGuiItemFlags_NoNav |
	...
	IM_ASSERT((flags &
		  ...
		  (ImGuiTabItemFlags_Leading |
		   ImGuiTabItemFlags_Trailing)); // Can't use both Leading and
						 // Trailing

	// Store into ImGuiTabItemFlags_NoCloseButton, also honor
	// ImGuiTabItemFlags_NoCloseButton passed by user (although not
	// documented)
	...
	// Update selected tab
	if (tab_appearing &&
	    (tab_bar->Flags & ImGuiTabBarFlags_AutoSelectNewTabs) &&
	    tab_bar->NextSelectedTabId == 0)
	...
	if ((flags & ImGuiTabItemFlags_SetSelected) &&
	    (tab_bar->SelectedTabId !=
	     id)) // SetSelected can only be passed on explicit tab bar
		if (!is_tab_button)
			tab_bar->NextSelectedTabId = id;

	// Lock visibility
	// (Note: tab_contents_visible != tab_selected... because CTRL+TAB
	// operations may preview some tabs without selecting them!)
	...
	// Note that tab_is_new is not necessarily the same as tab_appearing!
	// When a tab bar stops being submitted and then gets submitted again,
	// the tabs will have 'tab_appearing=true' but 'tab_is_new=false'.
	...
	const bool is_central_section =
	    (tab->Flags &
	     (ImGuiTabItemFlags_Leading | ImGuiTabItemFlags_Trailing)) == 0;
	size.x = tab->Width;
	if (is_central_section)
		window->DC.CursorPos =
		    tab_bar->BarRect.Min +
		    ImVec2(IM_FLOOR(tab->Offset - tab_bar->ScrollingAnim),
			   0.0f);
	else
		window->DC.CursorPos =
	...
	if (want_clip_rect)
		PushClipRect(ImVec2(ImMax(bb.Min.x, tab_bar->ScrollingRectMinX),
				    bb.Min.y - 1),
	...
	ImGuiButtonFlags button_flags =
	    ((is_tab_button ? ImGuiButtonFlags_PressedOnClickRelease
			    : ImGuiButtonFlags_PressedOnClick) |
	     ImGuiButtonFlags_AllowItemOverlap);
	if (g.DragDropActive)
		button_flags |= ImGuiButtonFlags_PressedOnDragDropHold;
	...
	if (held && !tab_appearing && IsMouseDragging(0)) {
		if (!g.DragDropActive &&
		    (tab_bar->Flags & ImGuiTabBarFlags_Reorderable)) {
			// While moving a tab it will jump on the other side of
			// the mouse, so we also test for MouseDelta.x
			if (g.IO.MouseDelta.x < 0.0f &&
			    g.IO.MousePos.x < bb.Min.x) {
				if (tab_bar->Flags &
				    ImGuiTabBarFlags_Reorderable)
					TabBarQueueReorder(tab_bar, tab, -1);
			} else if (g.IO.MouseDelta.x > 0.0f &&
	...
	const ImU32 tab_col = GetColorU32(
		...
		? (tab_bar_focused ? ImGuiCol_TabActive
				   : ImGuiCol_TabUnfocusedActive)
	...
	const ImGuiID close_button_id =
	    p_open ? GetIDWithSeed("#CLOSE", NULL, id) : 0;
	bool just_closed;
	bool text_clipped;
	TabItemLabelAndCloseButton(
	    display_draw_list, bb, flags, tab_bar->FramePadding, label, id,
	...
	// Tooltip (FIXME: Won't work over the close button because ItemOverlap
	// systems messes up with HoveredIdTimer) We test IsItemHovered() to
	// discard e.g. when another item is active or drag and drop over the
	// tab bar (which g.HoveredId ignores)
...
void ImGui::SetTabItemClosed(const char *label)
{
	...
	if (is_within_manual_tab_bar) {
		...
		if (ImGuiTabItem *tab = TabBarFindTabByID(tab_bar, tab_id))
			tab->WantClose = true; // Will be processed by next call
					       // to TabBarLayout()
	}
}
...
ImVec2 ImGui::TabItemCalcSize(const char *label, bool has_close_button)
{
	ImGuiContext &g = *GImGui;
	ImVec2 label_size = CalcTextSize(label, NULL, true);
	ImVec2 size = ImVec2(label_size.x + g.Style.FramePadding.x,
			     label_size.y + g.Style.FramePadding.y * 2.0f);
	if (has_close_button)
		size.x += g.Style.FramePadding.x +
			  (g.Style.ItemInnerSpacing.x +
			   g.FontSize); // We use Y intentionally to fit the
					// close button circle.
	else
...
void ImGui::TabItemBackground(ImDrawList *draw_list, const ImRect &bb,
			      ImGuiTabItemFlags flags, ImU32 col)
{
	// While rendering tabs, we trim 1 pixel off the top of our bounding box
	// so they can fit within a regular frame height while looking
	// "detached" from it.
	ImGuiContext &g = *GImGui;
	...
	const float rounding =
	    ImMax(0.0f, ImMin((flags & ImGuiTabItemFlags_Button)
				  ? g.Style.FrameRounding
				  : g.Style.TabRounding,
			      width * 0.5f - 1.0f));
	...
	draw_list->PathArcToFast(ImVec2(bb.Min.x + rounding, y1 + rounding),
				 rounding, 6, 9);
	draw_list->PathArcToFast(ImVec2(bb.Max.x - rounding, y1 + rounding),
				 rounding, 9, 12);
	...
	if (g.Style.TabBorderSize > 0.0f) {
		...
		draw_list->PathArcToFast(
		    ImVec2(bb.Min.x + rounding + 0.5f, y1 + rounding + 0.5f),
		    rounding, 6, 9);
		draw_list->PathArcToFast(
		    ImVec2(bb.Max.x - rounding - 0.5f, y1 + rounding + 0.5f),
		    rounding, 9, 12);
		draw_list->PathLineTo(ImVec2(bb.Max.x - 0.5f, y2));
		draw_list->PathStroke(GetColorU32(ImGuiCol_Border), false,
...
void ImGui::TabItemLabelAndCloseButton(ImDrawList *draw_list, const ImRect &bb,
				       ImGuiTabItemFlags flags,
				       ImVec2 frame_padding, const char *label,
				       ImGuiID tab_id, ImGuiID close_button_id,
				       ...
				       bool *out_text_clipped)
{
	...
	if (bb.GetWidth() <= 1.0f)
		...
		// In Style V2 we'll have full override of all colors per state
		// (e.g. focused, selected) But right now if you want to alter
		// text color of tabs this is what you need to do.
#if 0
	...
	ImRect text_pixel_clip_bb(bb.Min.x + frame_padding.x,
				  bb.Min.y + frame_padding.y,
				  bb.Max.x - frame_padding.x, bb.Max.y);
	if (flags & ImGuiTabItemFlags_UnsavedDocument) {
		text_pixel_clip_bb.Max.x -=
		    CalcTextSize(TAB_UNSAVED_MARKER, NULL, false).x;
		ImVec2 unsaved_marker_pos(
		    ImMin(bb.Min.x + frame_padding.x + label_size.x + 2,
			  text_pixel_clip_bb.Max.x),
		    bb.Min.y + frame_padding.y + IM_FLOOR(-g.FontSize * 0.25f));
		RenderTextClippedEx(draw_list, unsaved_marker_pos,
				    bb.Max - frame_padding, TAB_UNSAVED_MARKER,
				    NULL, NULL);
	}
	...
	if (out_text_clipped) {
		*out_text_clipped = (text_ellipsis_clip_bb.Min.x +
				     label_size.x) > text_pixel_clip_bb.Max.x;
		// draw_list->AddCircle(text_ellipsis_clip_bb.Min, 3.0f,
		// *out_text_clipped ? IM_COL32(255, 0, 0, 255) : IM_COL32(0,
		// 255, 0, 255));
	}
	...
	//  'hovered' will be true when hovering the Tab but NOT when hovering
	//  the close button 'g.HoveredId==id' will be true when hovering the
	//  Tab including when hovering the close button
	//  'g.ActiveId==close_button_id' will be true when we are holding on
	//  the close button, in which case both hovered booleans are false
	bool close_button_pressed = false;
	bool close_button_visible = false;
	if (close_button_id != 0)
		if (is_contents_visible ||
		    bb.GetWidth() >= g.Style.TabMinWidthForCloseButton)
	...
	if (close_button_visible) {
		...
		if (CloseButton(close_button_id,
				ImVec2(bb.Max.x - frame_padding.x * 2.0f -
					   close_button_sz,
				       bb.Min.y)))
	...
	// FIXME: if FramePadding is noticeably large, ellipsis_max_x will be
	// wrong here (e.g. #3497), maybe for consistency that parameter of
